...
import {
  flattenGameEntryPages,
  useInfiniteFundraisingGameEntrySearch,
  type GameEntrySearchSort,
} from '@/features/fundraisingGames/searchApi';
...
export default function FundraisingGameScreen() {
  const { organizationId = '', campaignId = '', slug = '' } =
    useLocalSearchParams<{ organizationId: string; campaignId: string; slug: string }>();
  const game = useFundraisingGame(organizationId || null, campaignId || null);

  if (game.isLoading) {
    return <><ScreenHeader title="Free Fundraising Game" /><LoadingState label="Loading game…" /></>;
  }
  if (game.isError) {
    return <><ScreenHeader title="Free Fundraising Game" /><ErrorState message="Could not load this fundraiser's free game." onRetry={() => game.refetch()} /></>;
  }

  return (
    <GameEditor
      key={game.data?.id ?? 'new-game'}
      organizationId={organizationId}
      campaignId={campaignId}
      slug={slug}
      existing={game.data ?? null}
    />
  );
}
...
function GameEditor({
  ...
  const [entrySort, setEntrySort] = useState<GameEntrySearchSort>('NEWEST');
  const [entryFilterOpen, setEntryFilterOpen] = useState(false);
  const [entrySortOpen, setEntrySortOpen] = useState(false);
  const entries = useInfiniteFundraisingGameEntrySearch(
    organizationId,
    campaignId,
    { q: entryQuery, winnerOnly, sort: entrySort },
    !!existing,
  );
  const entryItems = useMemo(() => flattenGameEntryPages(entries.data?.pages), [entries.data?.pages]);
  const entryTotal = entries.data?.pages[0]?.totalElements ?? existing?.entryCount ?? 0;
  ...
  const [instructions, setInstructions] = useState(
    existing?.instructions ??
      'Choose an open square for free. Supporting the fundraiser is optional and never changes your odds or number of entries.',
  );
  const [prizeDescription, setPrizeDescription] = useState(
    existing?.prizeDescription ?? 'Bragging rights / sponsor-provided recognition',
  );
  const [maxEntries, setMaxEntries] = useState(
    existing?.maxEntries == null ? '' : String(existing.maxEntries),
  );
  const [entriesPerPerson, setEntriesPerPerson] = useState(
    String(existing?.entriesPerPerson ?? 1),
  );
  ...
  const disclosure =
    'No purchase or donation is necessary to enter. Donating does not improve your odds or provide additional entries.';
  ...
  const busy =
    ...
    open.isPending ||
    close.isPending ||
    drawWinner.isPending;

  const entrySortLabel =
  ...
  async function save() {
    ...
    if (!Number.isInteger(perPerson) || perPerson < 1 || perPerson > 20) {
      return toast.show('Free entries per person must be between 1 and 20.', 'error');
    }
    if (limit != null && (!Number.isInteger(limit) || limit < 1)) {
      return toast.show('Entry limit must be a positive whole number, or leave it blank for unlimited.', 'error');
    }
    if (
      gameType === 'BIG_GAME_SQUARES' &&
      (!Number.isInteger(r) || !Number.isInteger(c) || !r || !c || r < 1 || c < 1 || r > 26 || c > 26)
    ) {
      return toast.show('Squares grids can be 1–26 rows and columns.', 'error');
    }
  ...
  async function doAction(success: string, action: () => Promise<unknown>) {
    try {
      ...
      toast.show('That free-game action could not be completed.', 'error');
    }
  }
  ...
  return (
    <ThemedView style={styles.container}>
      ...
      <ScrollView
              ...
              {GAME_TYPES.map((option) => (
                <Pressable
                  ...
                  style={[
                    ...
                    gameType === option.value && styles.typeCardSelected,
                  ]}>
                  <ThemedText
                    type="smallBold"
                    style={gameType === option.value ? styles.selectedText : undefined}>
                    {option.label}
                  </ThemedText>
                  <ThemedText
                    type="small"
                    themeColor={gameType === option.value ? undefined : 'textSecondary'}
                    style={gameType === option.value ? styles.selectedText : undefined}>
                    {option.description}
                  </ThemedText>
                </Pressable>
          ...
          <View style={styles.statusRow}>
            <View style={styles.flexOne}>
              ...
              <ThemedText type="small" themeColor="textSecondary">
                {existing.status} · {existing.entryCount} entries
              </ThemedText>
            </View>
        ...
        {canConfigure && (
          <>
            ...
            <Field label="Prize / recognition (optional)">
              <TextInput
                ...
                style={[
                  styles.input,
                  styles.smallMultiline,
                  { color: theme.text, backgroundColor: theme.backgroundElement },
                ]}
              />
            </Field>
            <Field label="Free entries per person">
              <TextInput
                value={entriesPerPerson}
                onChangeText={setEntriesPerPerson}
                keyboardType="number-pad"
                style={[styles.input, { color: theme.text, backgroundColor: theme.backgroundElement }]}
              />
            </Field>
            {gameType !== 'BIG_GAME_SQUARES' && (
              <Field label="Total entries allowed (blank = unlimited)">
                <TextInput
                  value={maxEntries}
            ...
            <Button disabled={busy} onPress={save}>
              {existing ? 'Save Game Setup' : 'Create Free Game'}
            </Button>
          </>
        )}
        ...
        {existing && (
          <ThemedView type="backgroundElement" style={styles.actionsCard}>
            <View style={styles.flexOne}>
              <ThemedText type="smallBold">Game actions</ThemedText>
              <ThemedText type="small" themeColor="textSecondary">
                Opening a game allows public free entries. Close it before a random prize drawing is selected.
              </ThemedText>
            </View>
            <View style={styles.actions}>
              {existing.permissions.canOpen && (
                <Button disabled={busy} onPress={() => doAction('Free game opened.', () => open.mutateAsync())}>
                  Open Game
                </Button>
              )}
              {existing.permissions.canClose && (
                <Button variant="secondary" disabled={busy} onPress={() => doAction('Free game closed.', () => close.mutateAsync())}>
                  Close Game
                </Button>
              )}
              {existing.permissions.canDrawWinner && (
                <Button disabled={busy} onPress={() => doAction('Winner selected.', () => drawWinner.mutateAsync())}>
                  Draw Winner
                </Button>
              )}
            </View>
        ...
        {existing && (
          <>
            ...
            <ListControls
              query={entryQuery}
              onChangeQuery={setEntryQuery}
              searchPlaceholder="Search player, email, square, or answer"
              resultCount={entryTotal}
              activeFilters={winnerOnly ? ['Winner only'] : []}
              onRemoveFilter={() => setWinnerOnly(false)}
              onClearFilters={() => {
                setEntryQuery('');
                setWinnerOnly(false);
                setEntrySort('NEWEST');
              }}
              onPressFilter={() => setEntryFilterOpen(true)}
              onPressSort={() => setEntrySortOpen(true)}
              sortLabel={entrySortLabel}
            />
            ...
            {!entries.isLoading && !entries.isError && entryItems.length === 0 && (
              <EmptyState
                title={entryQuery.trim() || winnerOnly ? 'No results found' : 'No free entries yet'}
                description={
                  entryQuery.trim() || winnerOnly
                    ? 'Try changing your search or filters.'
                    : 'Free game entries will appear here after participants join.'
                }
              />
            )}
            {entryItems.map((entry) => (
              <ThemedView
                ...
                style={[styles.entryRow, entry.isWinner && styles.winnerRow]}>
                <View style={styles.flexOne}>
                  <ThemedText type="smallBold">
                    {entry.displayName}{entry.isWinner ? ' · Winner' : ''}
                  </ThemedText>
                  <ThemedText type="small" themeColor="textSecondary">{entry.email}</ThemedText>
                  {entry.selectionKey && (
                    <ThemedText type="small" themeColor="textSecondary">Square: {entry.selectionKey}</ThemedText>
                  )}
                  {entry.selectionText && (
                    <ThemedText type="small" themeColor="textSecondary">Pick: {entry.selectionText}</ThemedText>
                  )}
                </View>
              </ThemedView>
            ))}
            <ListFooter
              loadedCount={entryItems.length}
      ...
      <Modal visible={entryFilterOpen} onClose={() => setEntryFilterOpen(false)}>
        <ThemedText type="smallBold" style={styles.modalTitle}>Filter entries</ThemedText>
        <Option selected={!winnerOnly} label="All entries" onPress={() => { setWinnerOnly(false); setEntryFilterOpen(false); }} />
        <Option selected={winnerOnly} label="Winner only" onPress={() => { setWinnerOnly(true); setEntryFilterOpen(false); }} />
      </Modal>

      <Modal visible={entrySortOpen} onClose={() => setEntrySortOpen(false)}>
        <ThemedText type="smallBold" style={styles.modalTitle}>Sort entries</ThemedText>
        <Option selected={entrySort === 'NEWEST'} label="Newest" onPress={() => { setEntrySort('NEWEST'); setEntrySortOpen(false); }} />
        <Option selected={entrySort === 'OLDEST'} label="Oldest" onPress={() => { setEntrySort('OLDEST'); setEntrySortOpen(false); }} />
        <Option selected={entrySort === 'NAME_ASC'} label="Player A–Z" onPress={() => { setEntrySort('NAME_ASC'); setEntrySortOpen(false); }} />
      </Modal>
    </ThemedView>
  );
...
const styles = StyleSheet.create({
  ...
  disclosure: {
    ...
    borderWidth: 1,
    borderColor: Brand.victoryGreen,
