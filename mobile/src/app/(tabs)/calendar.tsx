...
/** Calendar (Calendar tab) — real events for the selected team (ADR-102). */
export default function CalendarScreen() {
  ...
  if (coach.teams.length === 0) {
    return (
      <ThemedView style={styles.container}>
        <PlatformStatusSpacer />
        <EmptyState title="No team selected" description="You aren't assigned to a team as a coach in this organization." />
      </ThemedView>
    );
  }
    ...
    <ThemedView style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.agenda}>
        {eventsQuery.data && datesWithEvents.length === 0 && (
          <EmptyState title="No events scheduled" description="Nothing on this team's schedule yet." />
        )}
