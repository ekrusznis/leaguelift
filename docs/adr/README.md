...
- ADR-100: Phase 33/36 native mobile scaffold kickoff — created and customized the mobile/ Expo+TypeScript project (rebranded, web target removed, real ios/android platform-split example, EAS build profiles, build-release script), typecheck/lint/expo-doctor all clean (2026-08-10)
- ADR-101: mobile coach-persona screen set built from docs/design/mobile_sample_design.png — onboarding gate, 5-tab shell, Dashboard/Calendar/Team Chat/Event Details/Announcements/Team Roster/Settings, static mock data (no auth yet), typecheck/lint/expo-doctor all clean (2026-08-10)
- ADR-102: mobile real backend integration — real login for any role (only COACH built out), secure token storage, real DTOs/API layer for teams/events/RSVP/messaging/announcements/roster/settings, shared Toast/Modal/ConfirmDialog, real loading/error/empty states throughout, no refresh-token backend work (matches web's forced-relogin), typecheck/lint (incl. React Compiler purity rules)/expo-doctor all clean (2026-08-10)
- ADR-103: mobile Parent/Guardian persona — new /parent route segment (coach's (tabs) stays at root), one-time role-routing via useEffect+router.replace (fixed a real redirect-loop risk in the declarative-<Redirect> approach), real household/fees/credits/documents API layers, persona-agnostic event-details.tsx + guardian RSVP, Messages/Announcements/Settings reused unchanged, typecheck/lint/expo-doctor all clean (2026-08-10)
