...
| Search | Global / feature search | ☑ | ☐ | ☑ | ☑ | PASS (documented scope) | Live-tested as Owner: real debounced search (real `/search?q=` calls, both `Maya` → "Maya Johnson · Athlete" and `Johnson` → "Maya Johnson · Athlete" + "Johnson Family · Household" returned correctly). Household result click correctly navigates to the real household detail page. Team/Athlete/Organization result clicks intentionally do nothing beyond closing the dropdown — confirmed via source comment this is documented, current scope ("no detail route exists for them yet"), not a bug. Worth a UX polish pass eventually (a result that looks clickable but silently does nothing), but not a functional defect |
