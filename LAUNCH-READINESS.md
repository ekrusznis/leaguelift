...
| Subscription | Free if launch-enabled | ☑ | ☐ | ☑ | ☐ | PASS (built this session) | See LR-034 — FREE is now a real self-serve tier (Phase 45/46, DESIGN-DOC.md §14.1T/U), not just launch-enabled scaffolding. Registration bypasses Stripe Checkout entirely; verified via a real-Postgres integration test (`FreeSubscriptionIntegrationTest`) that a fresh owner reaches an `ACTIVE` org with `plan_code='FREE'` and no Stripe identifiers. Not yet live-browser-walked against the running docker-compose stack this session |
| Subscription | Starter $49 | ☑ | ☐ | ☑ | ☐ | PASS (unchanged, gating extended) | Existing $49 checkout flow untouched; its entitlement gates extended downward to also apply to FREE (team cap, fees, SMS, integrations, campaigns, sponsorships, family credits, advanced reporting — see LR-034). Full backend test suite (1080+ tests) green after the change |
| Subscription | Club $149 | ☑ | ☐ | ☑ | ☐ | PASS (unchanged) | Existing $149 checkout flow untouched; Club remains the tier every new gate in LR-034 unlocks |
| Subscription | Capacity / upgrade / downgrade | ☑ | ☐ | ☑ | ☐ | PASS (built this session) | See LR-034 — real in-app upgrade/downgrade now exists (previously nothing did; every plan change went through the Stripe Billing Portal only). A downgrade that would violate the target tier's limits (over team/campaign cap, existing fees/sponsorships/family-credit program, connected gated integration) is blocked with a specific per-violation message, a link to fix it, and a support-ticket link — never silent data loss, matching §13's explicit requirement. Verified via real-Postgres integration tests and a component test covering both the blocked-violation panel and a successful paid-to-paid apply; not yet live-browser-walked |
