...
**2026-08-21 review**: 4 of these 6 rows are already resolved by code that shipped in earlier sessions — filled in below with what's actually built, so only the 2 genuinely open rows need a founder call before launch.

| Item | Decision | Recommendation | Founder Decision | Date |
|---|---|---|---|---|
| Founding duration | Confirm exact duration | ~~6 months~~ | **3 months** — already decided and implemented (`founding_org_promo_code.pilot_ends_at` = redeemed_at + 90 days, marketing copy corrected to match) | 2026-08-12 |
| Free plan at soft launch | Confirm implementation readiness | Include only if hardened | **Ready** — LR-034 PASS, built and tested; still needs the founder's go/no-go on actually exposing it at soft launch, not an implementation gap | |
| Starter exact team cap | Confirm approved rule | Use configured approved rule | **Implemented as 3 teams** (`PlanEntitlementService.maxTeamsForPlan`, FREE=1/STARTER=3/Club+League=unlimited) — needs the founder to confirm 3 is the intended number, not a placeholder | |
| Founding grace period | Confirm | 14 days suggested | **Still genuinely open, and it's two different things** — (a) the Founding pilot's own suspend timeline is decided and built (day-30/60 check-ins, warnings day 63-89, real suspend day 90); (b) a *regular paid* subscription with a failed Stripe payment has **no grace period at all** today — `OrganizationSubscriptionService.handleInvoicePaymentFailed` leaves access open indefinitely while PAST_DUE with a code comment saying exactly that no policy has been chosen. This second one needs a real decision before launch if failed-payment handling matters for the first cohort | |
| Incomplete integrations visible? | Hide vs disabled | Hide or honest disabled state | **Done** — `IntegrationsPanel.tsx`: "These providers are scaffolded but remain disabled until Rally26 verifies official access, credentials, scopes, and contracts." Visible, not hidden, honestly labeled | |
| P1 exception | Case-by-case | Prefer fix | No P1s currently open that need an exception — see §35, only P2s and LR-006 (downgraded, see below) remain unresolved | |
...
|---|---|---|---|---|---|---|
| LR-042 | P0 | Account & organization deletion — ownership transfer + organization closure (slices 1 & 3 of 3) | Founder-directed, App Store Guideline 5.1.1(v) launch blocker, same initiative as LR-041. **Slice 1 (ownership transfer, Owner-only)**: `MembershipService.transferOwnership` (direct, target must already be an active Administrator) plus an invite-by-email path (`ownership_transfer_invitation` table) for handing the org to someone not yet a member — both demote the old owner to Administrator and audit both sides. **Slice 3 (organization closure, Owner-only, the alternative to transfer)**: cascades a real delete across the org's owned-data tree. This schema deliberately never relies on DB cascade (675 FK constraints, 667 `NO ACTION`, confirmed via direct `pg_constraint` introspection this session) — so a plain `delete from organization` fails immediately. Instead, a compile-time-generated, dependency-ordered table list (`ORGANIZATION_DELETION_SCOPE`, derived from the real live FK graph, not guessed from migration files) drives one generic executor: 14 financial tables (fee/order/contribution/ledger/etc.) are archived into a new `organization_financial_archive` snapshot table before deletion; the one real dependency cycle (`fundraising_game.winner_entry_id` ↔ `fundraising_game_entry`) is broken with a targeted null-out first; `audit_event`/`platform_support_access` are deliberately excluded and survive untouched (founder decision — oversight records outlive the org they're about). The `organization` row itself is never deleted, only tombstoned (`status → ARCHIVED`, reusing an existing-but-previously-unset enum value rather than adding a new one; contact/address fields cleared). Every remaining member's own `app_user` login is untouched — only their link to the closed org is removed. **Two real, previously-undiscovered bugs found and fixed by this session's own integration test, not anticipated by the FK-graph plan**: (1) 7 messaging tables (`message_entry`, `message_thread`, `message_thread_member`, `message_recipient`, `message_contact_restriction`, `message_moderation_event`, `message_safety_report`) have `reject_messaging_history_mutation()` triggers (V57/V58/V60) that unconditionally block UPDATE/DELETE to keep messaging safety history append-only — a plain hard-delete sweep would have failed closure for any org with messaging history at all. Founder direction: keep the append-only guarantee rather than bypass it wholesale — `message_entry`/`message_recipient` are redacted in place (body/display-name/email/phone replaced with a placeholder, row kept) via a narrow, transaction-scoped GUC bypass (`V104` migration) needed only for `message_entry`'s UPDATE block; the other 5 tables are left completely untouched, same as `audit_event`. (2) Both this scanner and the already-shipped `AccountDeletionLifecycleScanner` (LR-041) call their own `@Transactional finalize()` via `pastDue.forEach(::finalize)` — a **self-invocation that silently bypasses Spring's proxy, meaning `@Transactional` had no effect in either scanner since the day it was written**: every repository call ran in its own auto-committed mini-transaction, not one atomic sweep, so a mid-cascade failure would have left permanently corrupted partial-deletion state with no rollback. Caught only because a `set local` GUC from one statement wasn't visible to the next in the integration test. Fixed in both scanners via `@Lazy` self-injection (nullable, defaulted, so existing MockK unit tests are unaffected) | PASS (built and live-verified this session) | `V103__organization_deletion.sql`, `V104__organization_deletion_messaging_bypass.sql`, `organization/domain/OrganizationDeletionRequest.kt`/`OrganizationDeletionScope.kt`, `organization/persistence/OrganizationDeletionRequestRepository.kt`/`OrganizationDeletionExecutorRepository.kt`, `organization/application/OrganizationDeletionService.kt`/`OrganizationDeletionLifecycleScanner.kt`, `organization/web/OrganizationDeletionController.kt`; slice 1 (already committed `648de62`): `MembershipService.transferOwnership`/`finalizeOwnershipTransfer`, `V101__ownership_transfer_invitations.sql`, `invitation/*OwnershipTransferInvitation*`. Web: `OrganizationMembersPanel.tsx` "Close this organization" section (pending-closure banner + confirm modal), next to slice 1's existing "Transfer ownership" section. Mobile: same in `owner/(tabs)/members.tsx`. Self-invocation fix applied to both `OrganizationDeletionLifecycleScanner` and `AccountDeletionLifecycleScanner`. New backend tests: `OrganizationDeletionServiceTest`/`OrganizationDeletionLifecycleScannerTest` (MockK unit), `OrganizationDeletionCascadeIntegrationTest` (real Testcontainers Postgres — two real orgs populated across ~25 representative tables spanning every scope mode/financial/cycle case, one closed, then every one of the 93 scoped tables asserted empty for the closed org and untouched for the other — not a spot-check). Full backend suite + ktlint green | Live-verified against the rebuilt docker-compose backend (migrations `103`/`104` applied cleanly): real register→verify→login→create-org→request-closure→pending→cancel cycle over the real `/organizations/{id}/deletion-request` endpoints; duplicate-request correctly rejected (400); a real non-member correctly blocked (403). The actual cascade/redaction/archive/tombstone logic verified end-to-end via the Testcontainers integration test against a real Postgres instance with the real Spring-wired beans (including the self-invocation fix) rather than re-derived by hand over curl — the `@Scheduled` cron firing itself not separately re-verified, same as LR-041 |
| LR-041 | P0 | Account & organization deletion — personal account deletion (slice 2 of 3) | Founder-directed, App Store Guideline 5.1.1(v) launch blocker — the app supports account creation but had zero account-deletion code anywhere (confirmed via full-repo search this session). Any user can now request deletion of their own login from Settings; a 7-day cancelable grace period follows (real, not cosmetic — a scheduled daily scanner only finalizes past-due requests). An Owner of any active organization is blocked with a clear message pointing at slice 1's transfer/close options instead — deletion never orphans a live org. Finalization reuses `identityintegrity`'s existing `IdentityResolutionRepository` (built for platform-admin account merges) rather than duplicating its revoke logic — every membership/role-assignment/guardian-relationship is revoked, every message-thread membership closed, auth tokens invalidated, then the `app_user` row is anonymized in place (never hard-deleted — confirmed this session that ~70 tables reference `app_user.id` with only 4 cascading). New `DELETED` `AppUserStatus` value, already correctly rejected by the pre-existing `JwtCurrentUserConverter` non-ACTIVE check. **Founder-requested failsafe**: `message_recipient`/`announcement_recipient` store a display-name snapshot at delivery time rather than a live join — confirmed via a full sweep this session that these are the *only* two such snapshot tables (messages sender, audit actor/target, and membership lists are all live-joined and self-heal from the anonymization alone) — both get an explicit `[User Deleted]` update at finalize time | PASS (built and live-verified this session) | `V102__account_deletion.sql`, `identity/domain/AccountDeletionRequest.kt`, `identity/persistence/AccountDeletionRequestRepository.kt`, `identity/application/AccountDeletionService.kt`/`AccountDeletionLifecycleScanner.kt`, `identity/web/AccountDeletionController.kt`, 3 new methods on `identityintegrity/persistence/IdentityResolutionRepository.kt` (`retireDeletedUser`, `anonymizeMessageRecipientDisplayName`, `anonymizeAnnouncementRecipientDisplayName`). Web: `SettingsPage.tsx` "Danger zone" (request/cancel + pending-deletion banner). Mobile: same in `settings.tsx`. 7 new backend unit tests (service + scanner), full suite + ktlint green | Live-verified against the rebuilt docker-compose backend: real request→pending→cancel→re-request cycle over the real `/me/deletion-request` endpoints; a real Owner correctly blocked with the transfer/close message; duplicate-request correctly rejected. The scheduled scanner's SQL itself (not just the mocked orchestration) verified by running the exact `retireDeletedUser`/`anonymizeMessageRecipientDisplayName`/`anonymizeAnnouncementRecipientDisplayName` statements directly against a real test user row — status/email/display_name/password all changed exactly as designed, constraint accepted the new `DELETED` value. The `@Scheduled` cron firing itself not separately re-verified — identical trigger mechanism to the already-production-proven `FoundingPilotLifecycleScanner` |
| LR-040 | P0 | Mobile — EAS build profile / backend targeting | `mobile/eas.json`'s `development` build profile hardcodes `EXPO_PUBLIC_API_BASE_URL`/`EXPO_PUBLIC_FRONTEND_BASE_URL` to `http://localhost:8080`/`http://localhost:5173`. On a real physical device (the founder's own live-QA setup, built via expo.dev/EAS), "localhost" resolves to the phone itself, not the developer's machine — this profile cannot ever reach a real backend from a physical device. The `preview`/`production` profiles correctly point at `https://api.rally26.com`/`https://rally26.com`, so a physical-device build almost certainly used one of those instead — meaning any backend errors observed on a real device (see the "Unable to load plans / An unexpected error occurred" screenshot investigated this session) are against the **actual deployed backend**, not a local/network misconfiguration. **Fixed and confirmed**: `mobile/README.md` (already documented, lines 222-225, ADR-061) confirms `preview` and `production` deliberately share one single production backend — no staging exists by design. `development`'s `env` updated to match (`https://api.rally26.com`/`https://rally26.com`) instead of `localhost`, since this profile produces an installable dev-client APK/IPA meant for real-device testing, not tethered same-machine use. Separately confirmed live via curl: `https://api.rally26.com/api/v1/actuator/health` and `.../owner-onboarding/plans` both return a real `401` (reachable, running, correctly rejecting unauthenticated requests) and `https://rally26.com/` returns `200` — the production deployment is up and responding, ruling out "server is down/unreachable" as the explanation. This confirms the founder's screenshot was hitting the **real live production backend**, and the "Unable to load plans" 500 is a genuine server-side bug there — most likely either a stale/older deployed backend build (this session's fixes live only on this feature branch, not yet deployed) or a production-specific data issue (e.g. `subscription_plan` table not seeded/configured the same way production's real Stripe-linked catalog was set up, vs. this session's local dev seed data) causing an unhandled exception in `planRepository.listActive()`'s row-mapping. Confirmed NOT a code bug in the currently-checked-out branch — local docker-compose reproduces the full onboarding + Plan step + real Stripe test-mode checkout cleanly end-to-end | FOUNDER DECISION REQUIRED — need access to production backend logs/deployment status (correlate by `requestId` from the error response) to find the actual thrown exception; `GlobalExceptionHandler.handleUnexpected` deliberately never puts exception detail in the client response, so this cannot be diagnosed further from the client side alone | `mobile/eas.json` | production domain liveness verified via curl this session; the underlying 500 itself not yet reproduced or fixed — needs prod access |
