...
| Events | Sport terminology | ☑ | ☐ | ☑ | ☐ | NOT BUILT | See LR-011 — the entire sport-configuration foundation this depends on doesn't exist (`team.sport` is unconstrained free text, no canonical sport codes, zero terminology-derivation logic anywhere). Event type labels are the generic `EventType` enum (COMPETITION/TOURNAMENT/PRACTICE/MEETING/OTHER), never sport-derived (no "Match" for soccer vs "Meet" for swimming, etc.). Same root cause and same FOUNDER DECISION REQUIRED status as LR-011, not a separate gap |
...
| LR-032 | P1 | Search / cross-cutting authorization (web + API) | Founder-directed investigation: does global search correctly scope results by the caller's role, and does the *category-labeling* half of that ask already work? Category labeling: **confirmed working correctly, no bug** — a "Johnson" search returns both "Maya Johnson" (labeled "Athlete") and "Johnson Family" (labeled "Household") as clearly distinct, typed results; verified live. **Role-based scoping: a real, confirmed gap.** `SearchService.searchOrganization()`'s own doc comment states the intent precisely: global search is "deliberately not offered to Parent/Coach/Athlete dashboards — an org-wide search of every household would cross the household-privacy boundary... this is scoped to the same audience that already sees every team/household in the org via the organization detail page" (i.e., Owner/Administrator only). In practice, the backend never verified that — it called `membershipService.requireActiveMembership()`, which passes for **any** active membership role, including a team-scoped Coach (`TEAM_ADMINISTRATOR`). Proved live: signed in as the real Coach account (jordan.ellis, `TEAM_ADMINISTRATOR`, confirmed via direct DB query) and called `GET .../search?q=Chen` directly (bypassing the UI, which correctly never renders a search box on the Coach dashboard) — it returned "Chen Family," a household with zero team affiliation, completely outside this coach's legitimate scope. **Founder confirmed the fix direction directly** ("the person searching has role/security expectations"). **Fixed 2026-08-20**: `SearchService.searchOrganization`, `HouseholdSearchService.search`/`count`, and `TeamSearchService.search`/`count` — the exact 3 services the search UI itself calls, matching `canManageOrganization`'s own frontend gate precisely — now require `requireManagerRole` (OWNER/ADMINISTRATOR) instead of `requireActiveMembership`. New unit tests for all three (`HouseholdSearchServiceTest`/`TeamSearchServiceTest` didn't exist before this fix). **Deliberately NOT touched, flagged as a separate decision**: the plain, non-search `TeamService.list()`/`count()` (and its Household equivalent) have the identical weaker gate, but — unlike the 3 search services — are genuine, broadly-used dependencies of real lower-privilege features (a Coach's team-fundraiser filter in `CampaignList.tsx`, CSV import's team-matching, a guardian's own `HouseholdDetailPage`, several others) with 8+ real frontend call sites. Tightening those to manager-only would break legitimate functionality, not just close a leak — this remains the same "materially larger, needs an explicit design decision" item the original finding described, now scoped precisely to just the plain list endpoints rather than search | PASS (fixed, search endpoints) / OPEN (plain list endpoints, by design pending a decision) | `2fee560`-adjacent, this session — see `SearchService.kt`/`HouseholdSearchService.kt`/`TeamSearchService.kt` doc comments for the exact finding. Full backend test suite + ktlint clean | reproduced live pre-fix (curl as jordan.ellis returned "Chen Family") and post-fix against the rebuilt docker-compose backend: same call now returns `403 MEMBERSHIP_MANAGEMENT_DENIED` for all 3 endpoints (global search, household search, team search); confirmed the real Owner (mike.anderson) still gets correct results with zero regression |
