...
| Sponsorships | Package / publish / QR / payment | ☑ | ☐ | ☑ | ☑ | PASS (list, fixed) | See LR-026 — the Sponsorship Packages list 500'd for every organization until fixed this session; verified via curl (real "Gold Sponsor" package returned correctly). Publish/QR/payment sub-flows and the separate "Review pending sponsorships" search endpoint not yet exercised — see LR-026 note flagging the latter as an unverified same-class risk |
