...
| Onboarding | Founding Organization entitlement | ☐ | ☐ | ☐ | ☐ | NOT BUILT | Confirmed via source search: none of §14's required model exists anywhere — no unique one-use invite/promo code table, no `foundingStartAt`/`foundingEndAt`, no program-status field, no 30/14/7-day expiration reminder lifecycle, no grace/read-only state. The only related thing that exists is `/founding-organizations` (`FoundingOrganizationsPage.tsx`, added in LR-021), a marketing landing page whose CTA routes into the same generic `/auth/register` self-serve flow everyone else uses — it grants no actual Club-at-$0 entitlement, unique code, or expiration tracking. This is a real, separate feature that hasn't been started; do not confuse it with the FREE tier built in LR-034, which is a different, permanent tier, not a time-limited promo |
