				...
				<li key={request.id} className="rounded-lg border border-slate-gray/20 bg-white p-4">
					<div className="flex flex-wrap items-start justify-between gap-3">
						<div className="min-w-0 flex-1">
							...
							<p className="mt-2 break-words text-sm text-slate-gray">{request.reason}</p>
							{request.reviewNote && <p className="mt-2 text-sm text-slate-gray"><span className="font-medium">Review note:</span> {request.reviewNote}</p>}
						</div>
						<span className="shrink-0 rounded-full bg-navy/10 px-2 py-1 text-xs font-medium text-navy">
							{PROFILE_CORRECTION_STATUS_LABELS[request.status]}
						</span>
					</div>
					<div className="mt-3 flex flex-wrap items-center justify-between gap-2 text-xs text-slate-gray">
