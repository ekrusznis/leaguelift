...
export function PlatformSubscriptionsPage() {
	...
	return (
		<div className="flex flex-col gap-6">
			<div>
				<h1 className="font-heading text-2xl font-bold text-navy-900">Subscriptions</h1>
				<p className="mt-1 text-slate-500">Read-only organization billing visibility for onboarding, recovery, and support triage.</p>
			</div>
			<form onSubmit={(event) => { event.preventDefault(); setPage(0); setQuery(queryInput); }} className="flex flex-wrap items-end gap-3 rounded-xl border border-slate-200 bg-white p-4">
				<label className="flex min-w-64 flex-1 flex-col gap-1 text-sm font-medium text-navy-900">Search
					<input value={queryInput} onChange={(event) => setQueryInput(event.target.value)} placeholder="Organization name or slug" className="min-h-11 rounded-md border border-slate-300 px-3 py-2 font-normal" />
				</label>
				<label className="flex flex-col gap-1 text-sm font-medium text-navy-900">Subscription status
					...
					<tbody className="divide-y divide-slate-100">
						{subscriptions.data.items.map((item) => (
							<tr key={item.organizationId}>
								<td className="px-4 py-3"><p className="font-semibold text-navy-900">{item.organizationName}</p><p className="text-xs text-slate-500">{item.organizationStatus}</p></td>
								<td className="px-4 py-3 text-slate-700">{item.planName ?? "—"}{item.amountMinor != null && item.currency ? <span className="block text-xs text-slate-500">{formatMoney(item.amountMinor, item.currency)} / month</span> : null}</td>
								...
								<td className="px-4 py-3 text-slate-600">{item.recoveryState.replaceAll("_", " ")}</td>
								<td className="px-4 py-3 text-slate-600">Customer {item.hasStripeCustomer ? "✓" : "—"} · Subscription {item.hasStripeSubscription ? "✓" : "—"}</td>
								<td className="px-4 py-3 text-xs text-slate-600"><span className="block">Failure: {formatDate(item.lastPaymentFailureAt)}</span><span className="block">Success: {formatDate(item.lastPaymentSuccessAt)}</span></td>
								<td className="px-4 py-3 text-right"><Link to={`/app/platform/organizations/${item.organizationId}`} className="rounded-md border border-slate-300 px-3 py-2 font-medium text-navy-900 hover:border-green-500 hover:text-green-700">Open org</Link></td>
							</tr>
						))}
					</tbody>
				</table>
				{subscriptions.data.items.length === 0 && <p className="p-6 text-center text-sm text-slate-500">No organizations match these filters.</p>}
			</div>
			<div className="flex items-center justify-between text-sm text-slate-600"><p>{subscriptions.data.totalElements} organizations</p><div className="flex items-center gap-2"><button type="button" disabled={page === 0} onClick={() => setPage((value) => Math.max(0, value - 1))} className="rounded-md border border-slate-300 px-3 py-2 disabled:opacity-40">Previous</button><span>Page {page + 1} of {totalPages}</span><button type="button" disabled={page + 1 >= totalPages} onClick={() => setPage((value) => value + 1)} className="rounded-md border border-slate-300 px-3 py-2 disabled:opacity-40">Next</button></div></div>
		</div>
	);
}

function StatusPill({ status }: { status: string }) {
	const classes = status === "ACTIVE" || status === "TRIALING" ? "bg-green-100 text-green-800" : status === "PAST_DUE" ? "bg-amber-100 text-amber-900" : "bg-slate-100 text-slate-700";
