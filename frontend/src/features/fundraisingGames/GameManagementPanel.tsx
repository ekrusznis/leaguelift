...
import { useFundraisingGameEntrySearch, type GameEntrySearchSort } from "./searchApi";
...
const DEFAULTS: FundraisingGameFormValues = {
	...
	title: "Big Game Squares",
	instructions: "Pick an open square for free and follow along with the game.",
	prizeDescription: "",
	maxEntries: "",
	entriesPerPerson: 1,
	rows: 10,
...
export function GameManagementPanel({ organizationId, campaignId, canCreateGame }: { organizationId: string; campaignId: string; canCreateGame: boolean }) {
	...
	const [entrySize, setEntrySize] = useState(25);
	const [entryQuery, setEntryQuery] = useState("");
	const [winnerOnly, setWinnerOnly] = useState(false);
	const [entrySort, setEntrySort] = useState<GameEntrySearchSort>("NEWEST");
	const entries = useFundraisingGameEntrySearch(
		organizationId,
		campaignId,
		{ page: entryPage, size: entrySize, q: entryQuery, winnerOnly, sort: entrySort },
		!!game.data,
	);
	...
	useEffect(() => {
		...
		setValues({
			...
			instructions: game.data.instructions ?? "",
			prizeDescription: game.data.prizeDescription ?? "",
			maxEntries: game.data.maxEntries == null ? "" : String(game.data.maxEntries),
			entriesPerPerson: game.data.entriesPerPerson,
			rows: game.data.rows ?? 10,
			cols: game.data.cols ?? 10,
		});
	}, [game.data]);
	...
	return (
		<div className="flex flex-col gap-3 rounded-lg bg-ice-white p-3 dark:bg-[#0f172a]">
			<div className="flex flex-wrap items-start justify-between gap-3">
				<div><p className="font-semibold text-navy dark:text-[#f8fafc]">{item.title}</p><p className="text-xs text-slate-gray dark:text-[#94a3b8]">{GAME_LABELS[item.gameType]} · {item.status} · {item.entryCount} entr{item.entryCount === 1 ? "y" : "ies"}</p></div>
				<div className="flex flex-wrap gap-2">
					{item.permissions.canConfigure && <Button type="button" variant="secondary" onClick={() => setEditing((v) => !v)}>{editing ? "Cancel edit" : "Edit game"}</Button>}
					{item.permissions.canOpen && <Button type="button" onClick={() => open.mutate()} disabled={open.isPending}>Open free game</Button>}
					{item.permissions.canClose && <Button type="button" variant="secondary" onClick={() => close.mutate()} disabled={close.isPending}>Close game</Button>}
					{item.permissions.canDrawWinner && <Button type="button" onClick={() => draw.mutate()} disabled={draw.isPending}>{draw.isPending ? "Drawing…" : "Draw winner"}</Button>}
				</div>
			</div>
			...
			<div className="mt-2 border-t border-slate-gray/20 pt-3">
				<h4 className="mb-3 font-heading text-sm font-semibold text-navy dark:text-[#f8fafc]">Game entries</h4>
				<ListToolbar
					searchValue={entryQuery}
					onSearchChange={(value) => { setEntryQuery(value); setEntryPage(0); }}
					searchPlaceholder="Search player, email, square, or answer"
					resultCount={entries.data?.totalElements ?? item.entryCount}
					sortValue={entrySort}
					sortOptions={[
					...
					onSortChange={(value) => { setEntrySort(value as GameEntrySearchSort); setEntryPage(0); }}
					hasActiveFilters={winnerOnly}
					onClear={() => { setEntryQuery(""); setWinnerOnly(false); setEntrySort("NEWEST"); setEntryPage(0); }}
					filters={
						<label className="flex min-h-11 items-center gap-2 rounded-lg border border-slate-300 px-3 text-sm text-navy dark:border-[#334155] dark:text-[#f8fafc]">
							<input type="checkbox" checked={winnerOnly} onChange={(event) => { setWinnerOnly(event.target.checked); setEntryPage(0); }} />
							Winner only
						</label>
					}
				/>
				{entries.isLoading && <LoadingState label="Loading entries…" />}
				{entries.isError && <ErrorState message="Could not load game entries." onRetry={() => entries.refetch()} />}
				{entries.data?.items.length === 0 && (
					<EmptyState
						compact
						title={entryQuery.trim() || winnerOnly ? "No results found" : "No entries yet"}
						description={entryQuery.trim() || winnerOnly ? "Try changing your search or filters." : "Free game entries will appear here after participants join."}
					/>
				)}
				{entries.data && entries.data.items.length > 0 && (
					<div className="overflow-x-auto">
						<table className="w-full text-left text-sm">
							<thead><tr><th className="p-2">Player</th><th className="p-2">Entry</th><th className="p-2">Email</th><th className="p-2">Result</th></tr></thead>
							<tbody>
								{entries.data.items.map((entry) => (
									<tr key={entry.id} className="border-t border-slate-gray/20">
										<td className="p-2">{entry.displayName}</td>
										<td className="p-2">{entry.selectionKey ?? entry.selectionText ?? "Free drawing entry"}</td>
										<td className="p-2">{entry.email}</td>
										<td className="p-2">{entry.isWinner ? "Winner" : "—"}</td>
									</tr>
								))}
							</tbody>
						</table>
				...
				{entries.data && (
					<Pagination
						...
						onSizeChange={(value) => { setEntrySize(value); setEntryPage(0); }}
					/>
				)}
			</div>
...
function GameForm({ values, setValues, onSubmit, submitLabel, disabled, error, lockType = false }: {
	values: FundraisingGameFormValues;
	setValues: (value: FundraisingGameFormValues) => void;
	onSubmit: () => void;
	submitLabel: string;
	disabled: boolean;
	error: boolean;
	...
	return <form className="grid gap-3 rounded-lg border border-slate-gray/20 bg-pure-white p-3 dark:bg-[#111827] md:grid-cols-2" onSubmit={(event) => { event.preventDefault(); onSubmit(); }}>
		...
		<p className="rounded-md bg-victory-green/10 p-2 text-xs md:col-span-2">Entry is always free. Donation checkout is separate and never changes entry limits or odds.</p>
		{error && <p role="alert" className="text-sm text-error-red md:col-span-2">Could not save the free game. Check the configuration and try again.</p>}
