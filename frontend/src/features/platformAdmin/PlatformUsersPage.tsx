...
export function PlatformUsersPage() {
	...
	return (
		<div className="flex flex-col gap-6">
			<div><h1 className="font-heading text-2xl font-bold text-navy-900 dark:text-[#f8fafc]">Users</h1><p className="mt-1 text-slate-500 dark:text-[#cbd5e1]">Platform-wide identity and organization-membership directory. Passwords and tokens are never displayed.</p></div>
			<form onSubmit={(event) => { event.preventDefault(); setPage(0); setQuery(queryInput); }} className="flex flex-wrap items-end gap-3 rounded-xl border border-slate-200 dark:border-[#334155] bg-white dark:bg-[#111827] p-4">
				<label className="flex min-w-64 flex-1 flex-col gap-1 text-sm font-medium text-navy-900 dark:text-[#f8fafc]">Search<input value={queryInput} onChange={(event) => setQueryInput(event.target.value)} placeholder="Name or email" className="min-h-11 rounded-md border border-slate-300 dark:border-[#334155] px-3 py-2 font-normal" /></label>
				<label className="flex flex-col gap-1 text-sm font-medium text-navy-900 dark:text-[#f8fafc]">Status<select value={status} onChange={(event) => { setStatus(event.target.value); setPage(0); }} className="min-h-11 rounded-md border border-slate-300 dark:border-[#334155] bg-white dark:bg-[#111827] px-3 py-2 font-normal"><option value="">All statuses</option><option value="ACTIVE">Active</option><option value="SUSPENDED">Suspended</option></select></label>
				...
				<table className="w-full min-w-[900px] text-left text-sm">
					...
					<tbody className="divide-y divide-slate-100 dark:divide-[#334155]">
						{users.data.items.map((user) => (
							<tr key={user.userId}>
								<td className="px-4 py-3"><p className="font-semibold text-navy-900 dark:text-[#f8fafc]">{user.displayName}</p><p className="text-xs text-slate-500 dark:text-[#cbd5e1]">{user.email}</p><p className="mt-1 font-mono text-[11px] text-slate-400">{user.userId}</p></td>
										...
										<ul className="flex flex-col gap-1.5">
											{user.organizationMemberships.map((membership) => (
												<li key={membership.organizationId}>
													...
													<span className="ml-2 text-xs text-slate-500 dark:text-[#cbd5e1]">{membership.role}</span>
												</li>
											))}
										</ul>
									) : "None"}
								</td>
								<td className="px-4 py-3 text-slate-600 dark:text-[#cbd5e1]">{new Date(user.createdAt).toLocaleDateString()}</td>
							</tr>
						))}
					</tbody>
				</table>
				{users.data.items.length === 0 && <p className="p-6 text-center text-sm text-slate-500 dark:text-[#cbd5e1]">No users match these filters.</p>}
			</div>
			<div className="flex items-center justify-between text-sm text-slate-600 dark:text-[#cbd5e1]"><p>{users.data.totalElements} users</p><div className="flex items-center gap-2"><button type="button" disabled={page === 0} onClick={() => setPage((value) => Math.max(0, value - 1))} className="rounded-md border border-slate-300 dark:border-[#334155] px-3 py-2 disabled:opacity-40">Previous</button><span>Page {page + 1} of {totalPages}</span><button type="button" disabled={page + 1 >= totalPages} onClick={() => setPage((value) => value + 1)} className="rounded-md border border-slate-300 dark:border-[#334155] px-3 py-2 disabled:opacity-40">Next</button></div></div>
		</div>
	);
}
