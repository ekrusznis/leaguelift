			...
			<div className="grid gap-5 xl:grid-cols-[minmax(300px,0.8fr)_minmax(0,1.7fr)]">
				<section className="overflow-hidden rounded-xl border border-slate-200 bg-white">
					<div className="border-b border-slate-200 px-4 py-3">
						<h2 className="font-semibold text-navy-900">Candidate groups</h2>
						<p className="text-xs text-slate-500">{candidates.data.items.length} potential duplicate keys</p>
					</div>
					<div className="max-h-[720px] divide-y divide-slate-100 overflow-y-auto">
						{candidates.data.items.map((group, index) => (
							<button
								...
								className={`w-full px-4 py-3 text-left hover:bg-ice-50 ${index === selectedIndex ? "bg-ice-50" : ""}`}
							>
								<div className="flex items-center justify-between gap-2">
									<span className="rounded-full bg-slate-100 px-2 py-0.5 text-[11px] font-semibold text-slate-600">{group.matchType}</span>
						...
						<>
							<div className="rounded-xl border border-slate-200 bg-white p-5">
								<div className="flex flex-wrap items-start justify-between gap-3">
									<div>
										<h2 className="font-semibold text-navy-900">Side-by-side identity review</h2>
										<p className="text-sm text-slate-500">Choose which identity is the source and which app user should survive.</p>
									</div>
									<span className="rounded-full bg-ice-50 px-3 py-1 text-xs font-semibold text-navy-900">{selected.matchType}: {selected.normalizedValue}</span>
								</div>
							...
							{preview.data && (
								<div className="rounded-xl border border-slate-200 bg-white p-5">
									<div className="flex flex-wrap items-start justify-between gap-3">
										<div>
											<h2 className="font-semibold text-navy-900">Deterministic resolution plan</h2>
											<p className="text-sm text-slate-500">Strategy: <span className="font-semibold text-navy-900">{preview.data.strategy.replaceAll("_", " ")}</span></p>
									...
									<div className="mt-5">
										<h3 className="text-sm font-semibold text-navy-900">Dependency inventory</h3>
										{preview.data.dependencies.length > 0 ? (
											<div className="mt-2 overflow-x-auto rounded-lg border border-slate-200">
												<table className="w-full text-left text-xs">
													<thead className="bg-slate-50 text-slate-500"><tr><th className="px-3 py-2">Table</th><th className="px-3 py-2">Column</th><th className="px-3 py-2">Rows</th><th className="px-3 py-2">Treatment</th></tr></thead>
										...
										) : <p className="mt-2 text-sm text-slate-500">No foreign-key dependencies were found for the source identity.</p>}
									</div>
									...
									<div className="mt-5 rounded-xl border border-amber-200 bg-amber-50 p-4">
										...
										<label className="mt-4 flex flex-col gap-1 text-sm font-medium text-amber-950">
											Resolution reason
											<textarea value={reason} onChange={(event) => setReason(event.target.value)} maxLength={500} rows={3} placeholder="Why these records are confirmed duplicates and why this surviving account is correct" className="rounded-md border border-amber-300 bg-white px-3 py-2 font-normal text-slate-800" />
										...
										<label className="mt-3 flex flex-col gap-1 text-sm font-medium text-amber-950">
											...
											<input value={confirmedTargetEmail} onChange={(event) => setConfirmedTargetEmail(event.target.value)} placeholder={targetEmail || "Target must have an email"} className="min-h-11 rounded-md border border-amber-300 bg-white px-3 py-2 font-normal text-slate-800" />
										</label>
	...
	return (
		<div className="rounded-lg border border-slate-200 p-4">
			<div className="flex items-start justify-between gap-2">
				<div><p className="font-semibold text-navy-900">{identity.displayName}</p><p className="text-xs text-slate-500">{identity.ref.kind === "APP_USER" ? "App user" : "Guardian shell"} · {identity.status}</p></div>
			...
			<dl className="mt-3 grid gap-2 text-xs">
				<div><dt className="text-slate-400">Email</dt><dd className="break-all text-slate-700">{identity.email ?? "—"}</dd></div>
				<div><dt className="text-slate-400">Phone</dt><dd className="text-slate-700">{identity.phone ?? "—"}</dd></div>
				{identity.organizationName && <div><dt className="text-slate-400">Organization / household</dt><dd className="text-slate-700">{identity.organizationName} · {identity.householdName ?? "—"}</dd></div>}
				<div><dt className="text-slate-400">External IDs</dt><dd className="break-all text-slate-700">{identity.externalIds.join(", ") || "—"}</dd></div>
				<div><dt className="text-slate-400">Identity ID</dt><dd className="break-all font-mono text-[10px] text-slate-500">{identity.ref.id}</dd></div>
			</dl>
			...
			{identity.roleAssignments.length > 0 && <p className="mt-2 text-xs text-slate-500">Active resource roles: {identity.roleAssignments.length}</p>}
			{identity.guardianLinks.length > 0 && <p className="mt-1 text-xs text-slate-500">Active guardian links: {identity.guardianLinks.length}</p>}
			{identity.mergedIntoUserId && <p className="mt-2 rounded bg-red-50 px-2 py-1 text-xs text-red-800">Already merged into {identity.mergedIntoUserId}</p>}
			<div className="mt-4 flex gap-2">
				<button type="button" onClick={onSource} className={`rounded-md border px-3 py-1.5 text-xs font-semibold ${source && refKey(source) === key ? "border-navy-900 bg-navy-900 text-white" : "border-slate-300 text-slate-600"}`}>Source</button>
				<button type="button" onClick={onTarget} className={`rounded-md border px-3 py-1.5 text-xs font-semibold ${target && refKey(target) === key ? "border-azure-blue bg-ice-50 text-navy-900" : "border-slate-300 text-slate-600"}`}>Surviving target</button>
			</div>
		</div>
	);
}
