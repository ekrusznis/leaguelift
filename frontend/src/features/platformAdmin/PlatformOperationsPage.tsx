...
export function PlatformOperationsPage() {
	...
	return (
		<div className="flex flex-col gap-6">
			...
			<section className="rounded-xl border border-slate-200 bg-white p-5">
				<div><h2 className="font-heading text-lg font-semibold text-navy-900">Provider contract hardening</h2><p className="mt-1 text-sm text-slate-500">Code and configuration readiness only. Live probes remain disabled until Phase 20 credentialed verification.</p></div>
			...
			<section className="rounded-xl border border-slate-200 bg-white p-5">
				<div><h2 className="font-heading text-lg font-semibold text-navy-900">Recent provider runs</h2><p className="mt-1 text-sm text-slate-500">Durable read/write/webhook/health runs with redacted errors and immutable counts.</p></div>
				{syncRuns.data.length === 0 ? <p className="mt-4 text-sm text-slate-500">No integration sync runs recorded.</p> : <ul className="mt-4 space-y-2">{syncRuns.data.slice(0, 10).map((run) => <li key={run.id} className="flex flex-wrap items-center justify-between gap-3 rounded-lg border border-slate-200 px-3 py-2 text-sm"><span className="font-medium text-navy-900">{run.provider.replaceAll("_", " ")} · {run.direction}</span><span className="text-slate-500">{run.status} · {run.createdCount} created · {run.failedCount} failed</span></li>)}</ul>}
			</section>
...
function ProviderContractCard({ contract }: { contract: PlatformProviderContract }) {
	return <article className="rounded-lg border border-slate-200 p-4"><div className="flex items-start justify-between gap-3"><h3 className="font-semibold text-navy-900">{contract.provider.replaceAll("_", " ")}</h3><span className="rounded-full bg-slate-100 px-2 py-1 text-[11px] font-semibold text-slate-600">{contract.configurationStatus.replaceAll("_", " ")}</span></div><dl className="mt-3 space-y-2 text-xs"><div><dt className="font-medium text-slate-500">Credential ownership</dt><dd className="text-slate-700">{contract.credentialOwnership}</dd></div><div><dt className="font-medium text-slate-500">Webhook verification</dt><dd className="text-slate-700">{contract.webhookVerification}</dd></div><div><dt className="font-medium text-slate-500">Health mode</dt><dd className="text-slate-700">{contract.healthCheckMode}</dd></div></dl><ul className="mt-3 space-y-2">{contract.capabilities.map((capability) => <li key={capability.name} className="rounded-md bg-ice-50 p-2 text-xs"><div className="flex justify-between gap-2"><span className="font-medium text-navy-900">{capability.name}</span><span className="text-slate-500">{capability.status}</span></div><p className="mt-1 text-slate-600">{capability.summary}</p></li>)}</ul></article>;
}
