	...
	return (
		<div className="mx-auto max-w-4xl px-4 py-10 sm:py-14">
			...
			<div className="mt-6 grid gap-7 lg:grid-cols-[1fr_0.75fr]">
				<section className="rounded-2xl border border-slate-200 dark:border-[#334155] bg-white dark:bg-[#111827] p-6 shadow-sm sm:p-8">
					<h1 className="font-heading text-3xl font-extrabold text-navy-900 dark:text-[#f8fafc]">Contact Rally26 Support</h1>
					<p className="mt-3 text-slate-600 dark:text-[#cbd5e1]">Submitting creates a durable support case before email is attempted. This is ticket/email support, not live chat, and no response-time promise is implied.</p>
					{authenticated && <p className="mt-4 rounded-lg bg-ice-50 dark:bg-[#0f172a] p-3 text-sm text-slate-700 dark:text-[#cbd5e1]">Submitting as <strong>{user?.displayName}</strong> ({user?.email}).</p>}
					<form onSubmit={submit} className="mt-6 flex flex-col gap-4">
						{!authenticated && <>
							<label className="text-sm font-semibold text-navy-900 dark:text-[#f8fafc]">Your name<input required minLength={2} maxLength={120} value={requesterName} onChange={(e) => setRequesterName(e.target.value)} className="mt-1 min-h-11 w-full rounded-lg border border-slate-300 dark:border-[#334155] px-3 font-normal" /></label>
							<label className="text-sm font-semibold text-navy-900 dark:text-[#f8fafc]">Email<input required type="email" maxLength={320} value={requesterEmail} onChange={(e) => setRequesterEmail(e.target.value)} className="mt-1 min-h-11 w-full rounded-lg border border-slate-300 dark:border-[#334155] px-3 font-normal" /></label>
						</>}
						{authenticated && organizationOptions.length > 0 && <label className="text-sm font-semibold text-navy-900 dark:text-[#f8fafc]">Organization (optional)<select value={organizationId} onChange={(e) => setOrganizationId(e.target.value)} className="mt-1 min-h-11 w-full rounded-lg border border-slate-300 dark:border-[#334155] px-3 font-normal"><option value="">No organization selected</option>{organizationOptions.map((org) => <option key={org.id} value={org.id}>{org.name}</option>)}</select></label>}
						<label className="text-sm font-semibold text-navy-900 dark:text-[#f8fafc]">Category<select value={category} onChange={(e) => setCategory(e.target.value as SupportCaseCategory)} className="mt-1 min-h-11 w-full rounded-lg border border-slate-300 dark:border-[#334155] px-3 font-normal">{SUPPORT_CASE_CATEGORIES.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}</select></label>
						<label className="text-sm font-semibold text-navy-900 dark:text-[#f8fafc]">Subject<input required minLength={5} maxLength={200} value={subject} onChange={(e) => setSubject(e.target.value)} className="mt-1 min-h-11 w-full rounded-lg border border-slate-300 dark:border-[#334155] px-3 font-normal" /></label>
						<label className="text-sm font-semibold text-navy-900 dark:text-[#f8fafc]">What happened?<textarea required minLength={20} maxLength={5000} rows={7} value={description} onChange={(e) => setDescription(e.target.value)} className="mt-1 w-full rounded-lg border border-slate-300 dark:border-[#334155] px-3 py-2 font-normal" placeholder="Include the page, action, what you expected, and what happened instead." /></label>
						...
						{createCase.isSuccess && <div role="status" className="rounded-lg border border-green-300 bg-green-50 dark:bg-green-950 p-4 text-sm text-green-900">Support case <strong>{createCase.data.id}</strong> was created. A confirmation email will be attempted through the notification outbox.</div>}
