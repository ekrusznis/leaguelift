...
const PRIORITY_CLASSES: Record<ActionCenterPriority, string> = {
	...
	HIGH: "bg-championship-gold/20 text-navy dark:text-[#f8fafc]",
	NORMAL: "bg-info-blue/10 text-info-blue",
	INFO: "bg-slate-gray/10 text-slate-gray dark:text-[#cbd5e1]",
};
	...
	return (
		<div className="flex flex-col gap-6">
			<header>
				...
				<h1 className="mt-1 font-heading text-3xl font-bold text-navy dark:text-[#f8fafc]">Action Center</h1>
			...
			{actionCenter.data && (
				<>
					<div className="grid gap-3 sm:grid-cols-2">
						<div className="rounded-xl border border-slate-gray/20 bg-pure-white dark:bg-[#111827] p-5">
							<p className="text-sm text-slate-gray dark:text-[#cbd5e1]">Open items</p>
							<p className="mt-1 font-heading text-3xl font-bold text-navy dark:text-[#f8fafc]">{actionCenter.data.totalCount}</p>
						</div>
						<div className="rounded-xl border border-slate-gray/20 bg-pure-white dark:bg-[#111827] p-5">
							<p className="text-sm text-slate-gray dark:text-[#cbd5e1]">High or urgent</p>
					...
					{actionCenter.data.items.length === 0 ? (
						...
						<ul className="flex flex-col gap-3" aria-label="Action center items">
							{actionCenter.data.items.map((item) => {
								...
								return (
									<li key={item.id} className="rounded-xl border border-slate-gray/20 bg-pure-white dark:bg-[#111827] p-5 shadow-sm">
										<div className="flex flex-wrap items-start justify-between gap-3">
											<div className="min-w-0 flex-1">
												...
												<h2 className="mt-2 font-heading text-lg font-semibold text-navy dark:text-[#f8fafc]">{item.title}</h2>
												<p className="mt-1 text-sm text-slate-gray dark:text-[#cbd5e1]">{item.description}</p>
												{date && <p className="mt-2 text-xs text-slate-gray dark:text-[#cbd5e1]">Relevant date: {date}</p>}
											</div>
