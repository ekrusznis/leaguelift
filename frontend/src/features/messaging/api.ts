...
/** PATCH .../messaging/safe-sport-policy (Phase 37 slice 37.2) — Platform-Admin-only server-side (MessageSafeSportService.recordReview); an org owner/admin can read the policy via useMessageSafeSportPolicy but not call this. */
export function useRecordSafeSportReview(organizationId: string | undefined) { const qc=useQueryClient(); return useMutation({ mutationFn: (body: { reviewStatus: MessageSafeSportReviewStatus; athleteMessagingEnabled: boolean; reviewReference?: string }) => apiFetch<MessageSafeSportPolicy>(`/organizations/${organizationId}/messaging/safe-sport-policy`, { method: "PATCH", body }), onSuccess: () => qc.invalidateQueries({ queryKey: ["messaging", "safe-sport-policy", organizationId] }) }); }
