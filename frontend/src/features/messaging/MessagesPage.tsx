...
export function MessagesPage() {
	...
	return (
		<div className="flex flex-col gap-8">
			<header>
				<p className="text-sm font-semibold uppercase tracking-wide text-victory-green">Phase 25.3 · Messaging safety</p>
				<h1 className="mt-1 font-heading text-3xl font-bold text-navy">Messages</h1>
				<p className="mt-2 max-w-3xl text-slate-gray">Broadcasts and coach-to-family conversations now include message reporting, retained safety evidence, manager moderation history, and reversible safety locks. Athlete-created direct messaging remains disabled pending safe-sport review.</p>
			</header>
			...
			<section aria-labelledby="message-inbox-heading" className="rounded-xl border border-slate-gray/20 bg-pure-white p-5">
				<div className="flex flex-wrap items-center justify-between gap-3">
					<div><h2 id="message-inbox-heading" className="font-heading text-xl font-semibold text-navy">Your message threads</h2><p className="mt-1 text-sm text-slate-gray">Broadcasts and conversations visible to your account appear here.</p></div>
						...
						<div className="min-w-0 rounded-lg border border-slate-gray/20 p-4">
							...
							{inboxMessages.data && <ul className="flex flex-col gap-3" aria-label="Messages in selected thread">{inboxMessages.data.items.map(({ message, readAt, accessReason }) => (
								<li key={message.id} className={`rounded-lg border p-4 ${readAt ? "border-slate-gray/20 bg-ice-white" : "border-victory-green/40 bg-victory-green/5"}`}>
									<div className="flex flex-wrap items-start justify-between gap-3"><div className="min-w-0 flex-1"><p className="text-xs text-slate-gray">{message.senderDisplayName} · {formatDate(message.sentAt)}</p><p className="mt-2 whitespace-pre-wrap text-sm text-navy">{message.body}</p>{accessReason === "GUARDIAN_VISIBILITY" && <p className="mt-2 text-xs text-slate-gray">Guardian visibility access · read-only unless you were also explicitly selected.</p>}<ReportMessageControl messageId={message.id} /></div>{!readAt && <Button type="button" variant="secondary" disabled={markRead.isPending} onClick={() => markRead.mutate(message.id)}>Mark read</Button>}</div>
								</li>
							))}</ul>}
			...
			{teamScopes.length > 0 && <section aria-labelledby="new-conversation-heading" className="rounded-xl border border-slate-gray/20 bg-pure-white p-5">
				...
				<p className="mt-1 text-sm text-slate-gray">Choose active guardians or athletes connected to one team. Selected members can reply; guardians automatically observing an athlete are read-only unless selected too.</p>
				<form onSubmit={(event) => void submitConversation(event)} className="mt-4 grid gap-4">
					<div className="max-w-xl"><label htmlFor="conversation-team" className="text-sm font-medium text-navy">Team</label><select id="conversation-team" value={conversationTeam ? `${conversationTeam.organizationId}:${conversationTeam.scopeId}` : ""} onChange={(event) => { setConversationTeamKey(event.target.value); setSelectedTargets(new Set()); }} className="mt-1 min-h-11 w-full rounded-md border border-slate-gray/30 px-3 py-2">{teamScopes.map((scope) => <option key={`${scope.organizationId}:${scope.scopeId}`} value={`${scope.organizationId}:${scope.scopeId}`}>{scope.label}</option>)}</select></div>
					<div className="grid gap-4 md:grid-cols-2"><div><label htmlFor="conversation-title" className="text-sm font-medium text-navy">Conversation title</label><input id="conversation-title" value={conversationTitle} onChange={(event) => setConversationTitle(event.target.value)} required maxLength={180} className="mt-1 min-h-11 w-full rounded-md border border-slate-gray/30 px-3 py-2" /></div><fieldset><legend className="text-sm font-medium text-navy">Fallback delivery</legend><label className="mt-2 flex items-center gap-2 text-sm text-slate-gray"><input type="checkbox" checked={conversationEmail} onChange={(event) => setConversationEmail(event.target.checked)} /> Email</label><label className="mt-2 flex items-center gap-2 text-sm text-slate-gray"><input type="checkbox" checked={conversationSms} onChange={(event) => setConversationSms(event.target.checked)} /> SMS to opted-in guardians</label></fieldset></div>
					<div><div className="flex flex-wrap items-center justify-between gap-2"><p className="text-sm font-medium text-navy">Recipients</p>{contacts.data && contacts.data.length > 0 && <button type="button" className="text-sm font-semibold text-victory-green" onClick={() => setSelectedTargets(selectedTargets.size === contacts.data.length ? new Set() : new Set(contacts.data.map((contact) => contact.userId)))}>{selectedTargets.size === contacts.data.length ? "Clear all" : "Select all"}</button>}</div>{contacts.isLoading && <LoadingState label="Loading team contacts…" />}{contacts.isError && <ErrorState message="Could not load eligible team contacts." onRetry={() => contacts.refetch()} />}{contacts.data?.length === 0 && <EmptyState title="No activated family contacts" description="Invite guardians or eligible athletes before starting a two-way conversation." />}{contacts.data && contacts.data.length > 0 && <div className="mt-2 max-h-64 overflow-y-auto rounded-lg border border-slate-gray/20 p-2">{contacts.data.map((contact) => <label key={contact.userId} className="flex items-center gap-3 rounded-md px-2 py-2 hover:bg-ice-white"><input type="checkbox" checked={selectedTargets.has(contact.userId)} onChange={() => toggleTarget(contact.userId)} /><span className="min-w-0"><span className="block font-medium text-navy">{contact.displayName}</span><span className="block text-xs text-slate-gray">{contact.recipientType === "GUARDIAN" ? "Guardian" : "Athlete"}</span></span></label>)}</div>}</div>
					<div><label htmlFor="conversation-first-message" className="text-sm font-medium text-navy">First message</label><textarea id="conversation-first-message" value={conversationBody} onChange={(event) => setConversationBody(event.target.value)} required maxLength={5000} rows={5} className="mt-1 w-full rounded-md border border-slate-gray/30 px-3 py-2" /></div>
			...
			{scopes.length > 0 && <section aria-labelledby="message-management-heading" className="rounded-xl border border-slate-gray/20 bg-pure-white p-5">
				<h2 id="message-management-heading" className="font-heading text-xl font-semibold text-navy">Manage messaging</h2>
				<p className="mt-1 text-sm text-slate-gray">Create one-way broadcasts here and review/archive both broadcast and conversation threads for the scopes you manage.</p>
				...
				<form onSubmit={(event) => void createBroadcastThread(event)} className="mt-5 grid gap-4 rounded-lg bg-ice-white p-4">
					<div><label htmlFor="thread-title" className="text-sm font-medium text-navy">New broadcast title</label><input id="thread-title" required minLength={3} maxLength={180} value={title} onChange={(event) => setTitle(event.target.value)} className="mt-1 min-h-11 w-full rounded-md border border-slate-gray/30 px-3 py-2" /></div>
					<div className="grid gap-4 md:grid-cols-2"><div><label htmlFor="thread-audience" className="text-sm font-medium text-navy">Audience</label><select id="thread-audience" value={audience} onChange={(event) => setAudience(event.target.value as Exclude<MessageAudience, "SELECTED">)} className="mt-1 min-h-11 w-full rounded-md border border-slate-gray/30 px-3 py-2"><option value="ALL">Everyone in scope</option><option value="STAFF">Staff</option><option value="GUARDIANS">Guardians</option><option value="ATHLETES">Activated athletes</option></select></div><fieldset><legend className="text-sm font-medium text-navy">Fallback delivery</legend><label className="mt-2 flex items-center gap-2 text-sm text-slate-gray"><input type="checkbox" checked={emailEnabled} onChange={(event) => setEmailEnabled(event.target.checked)} /> Email</label><label className="mt-2 flex items-center gap-2 text-sm text-slate-gray"><input type="checkbox" checked={smsEnabled} onChange={(event) => setSmsEnabled(event.target.checked)} /> SMS to opted-in households</label></fieldset></div>
