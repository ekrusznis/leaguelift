...
const REASON_SEVERITY_CLASSES: Record<MessageSafetyReportReason, string> = {
	...
	SPAM: "bg-slate-gray/10 text-slate-gray dark:text-[#cbd5e1]",
	OTHER: "bg-slate-gray/10 text-slate-gray dark:text-[#cbd5e1]",
};
...
export function ReportMessageControl({ messageId }: { messageId: string }) {
	...
	return (
		<details className="mt-3 text-xs text-slate-gray dark:text-[#cbd5e1]">
			<summary className="cursor-pointer font-semibold text-navy dark:text-[#f8fafc]">Report message</summary>
			<form onSubmit={(event) => void submit(event)} className="mt-2 grid gap-2 rounded-md border border-slate-gray/20 bg-pure-white dark:bg-[#111827] p-3">
				<label>Reason<select value={reason} onChange={(event) => setReason(event.target.value as MessageSafetyReportReason)} className="mt-1 min-h-10 w-full rounded-md border border-slate-gray/30 px-2 py-1 text-sm text-navy dark:text-[#f8fafc]">{Object.entries(REASON_LABELS).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label>
				<label>Optional context<textarea value={details} onChange={(event) => setDetails(event.target.value)} maxLength={2000} rows={3} className="mt-1 w-full rounded-md border border-slate-gray/30 px-2 py-1 text-sm text-navy dark:text-[#f8fafc]" /></label>
...
export function MyMessageReportsPanel() {
	...
	return (
		<section aria-labelledby="my-message-reports-heading" className="rounded-xl border border-slate-gray/20 bg-pure-white dark:bg-[#111827] p-5">
			<h2 id="my-message-reports-heading" className="font-heading text-xl font-semibold text-navy dark:text-[#f8fafc]">Your safety reports</h2>
			<p className="mt-1 text-sm text-slate-gray dark:text-[#cbd5e1]">Reports remain private to you and authorized safety reviewers.</p>
			<ul className="mt-4 grid gap-3">{reports.data.items.map((item) => <li key={item.id} className="rounded-lg bg-ice-white dark:bg-[#0f172a] p-3"><div className="flex flex-wrap justify-between gap-2"><span className="font-semibold text-navy dark:text-[#f8fafc]">{item.threadTitle}</span><span className="text-xs font-semibold text-slate-gray dark:text-[#cbd5e1]">{item.status.replace("_", " ")}</span></div><p className="mt-1 text-xs text-slate-gray dark:text-[#cbd5e1]">{REASON_LABELS[item.reason]} · {formatDate(item.createdAt)}</p><p className="mt-2 line-clamp-2 text-sm text-navy dark:text-[#f8fafc]">{item.messageBody}</p>{item.resolutionNote && <p className="mt-2 text-xs text-slate-gray dark:text-[#cbd5e1]">Review note: {item.resolutionNote}</p>}</li>)}</ul>
		</section>
	);
}
...
export function ManagedMessageSafetyPanel({ scope }: { scope: MessageManagementScope | undefined }) {
	...
	return (
		<div className="mt-8 border-t border-slate-gray/20 pt-6">
			<div className="flex flex-wrap items-center justify-between gap-3"><div><h3 className="font-heading text-lg font-semibold text-navy dark:text-[#f8fafc]">Safety review</h3><p className="mt-1 text-sm text-slate-gray dark:text-[#cbd5e1]">Open reports sort by severity first, then oldest first. Reported messages and append-only moderation history for this scope.</p></div><label className="text-sm text-slate-gray dark:text-[#cbd5e1]">Report status<select value={status} onChange={(event) => setStatus(event.target.value as MessageSafetyReportStatus | "")} className="ml-2 min-h-10 rounded-md border border-slate-gray/30 px-2"><option value="">All</option><option value="OPEN">Open</option><option value="IN_REVIEW">In review</option><option value="RESOLVED">Resolved</option><option value="DISMISSED">Dismissed</option></select></label></div>
			...
			{reports.data && reports.data.items.length > 0 && <div className="mt-4 grid gap-4 lg:grid-cols-[18rem_minmax(0,1fr)]"><ul className="flex flex-col gap-2">{reports.data.items.map((item) => <li key={item.id}><button type="button" onClick={() => setSelectedReportId(item.id)} className={`w-full rounded-lg border p-3 text-left ${selected?.id === item.id ? "border-victory-green bg-victory-green/5" : "border-slate-gray/20"}`}><div className="flex justify-between gap-2"><span className={`rounded-full px-2 py-0.5 text-xs font-semibold ${REASON_SEVERITY_CLASSES[item.reason]}`}>{REASON_LABELS[item.reason]}</span><span className="text-xs text-slate-gray dark:text-[#cbd5e1]">{item.status.replace("_", " ")}</span></div><p className="mt-1 text-xs text-slate-gray dark:text-[#cbd5e1]">{item.reporterDisplayName} · {item.threadTitle}</p><p className="mt-2 line-clamp-2 text-sm text-slate-gray dark:text-[#cbd5e1]">{item.messageBody}</p></button></li>)}</ul><div className="min-w-0 rounded-lg border border-slate-gray/20 p-4">{selected && <><div className="flex flex-wrap items-start justify-between gap-3"><div><h4 className="font-heading font-semibold text-navy dark:text-[#f8fafc]">{selected.threadTitle}</h4><p className="mt-1 text-xs text-slate-gray dark:text-[#cbd5e1]">Reported {formatDate(selected.createdAt)} · message from {selected.messageSenderDisplayName}</p></div><span className="text-xs font-semibold text-slate-gray dark:text-[#cbd5e1]">{selected.status.replace("_", " ")}</span></div><blockquote className="mt-4 rounded-lg bg-ice-white dark:bg-[#0f172a] p-3 text-sm text-navy dark:text-[#f8fafc]">{selected.messageBody}</blockquote>{selected.details && <p className="mt-3 text-sm text-slate-gray dark:text-[#cbd5e1]">Reporter context: {selected.details}</p>}{selected.threadSafetyLockedAt && <p role="status" className="mt-3 rounded-md border border-slate-gray/30 bg-ice-white dark:bg-[#0f172a] p-2 text-sm font-semibold text-navy dark:text-[#f8fafc]">Thread is safety-locked. {selected.threadSafetyLockReason}</p>}<label className="mt-4 block text-sm font-medium text-navy dark:text-[#f8fafc]">Review / lock note<textarea value={note} onChange={(event) => setNote(event.target.value)} maxLength={2000} rows={3} className="mt-1 w-full rounded-md border border-slate-gray/30 px-3 py-2" /></label><div className="mt-3 flex flex-wrap gap-2">{selected.status === "OPEN" && <Button type="button" variant="secondary" disabled={review.isPending} onClick={() => void setReviewStatus("IN_REVIEW")}>Start review</Button>}<Button type="button" variant="secondary" disabled={lock.isPending || unlock.isPending} onClick={() => void toggleLock()}>{selected.threadSafetyLockedAt ? "Unlock thread" : "Safety-lock thread"}</Button>{selected.status !== "RESOLVED" && <Button type="button" variant="secondary" disabled={review.isPending || !note.trim()} onClick={() => void setReviewStatus("RESOLVED")}>Resolve</Button>}{selected.status !== "DISMISSED" && <Button type="button" variant="secondary" disabled={review.isPending || !note.trim()} onClick={() => void setReviewStatus("DISMISSED")}>Dismiss</Button>}</div>{notice && <p role="status" className="mt-3 text-sm text-slate-gray dark:text-[#cbd5e1]">{notice}</p>}<div className="mt-5 border-t border-slate-gray/20 pt-4"><p className="text-xs font-semibold uppercase tracking-wide text-slate-gray dark:text-[#cbd5e1]">Moderation history</p>{events.isLoading && <LoadingState label="Loading moderation history…" />}{events.data && <ul className="mt-2 grid gap-2">{events.data.map((event) => <li key={event.id} className="text-xs text-slate-gray dark:text-[#cbd5e1]"><span className="font-semibold text-navy dark:text-[#f8fafc]">{event.eventType.replace("_", " ")}</span> · {event.actorDisplayName} · {formatDate(event.createdAt)}{event.note ? ` · ${event.note}` : ""}</li>)}</ul>}</div></>}</div></div>}
		</div>
	);
}
