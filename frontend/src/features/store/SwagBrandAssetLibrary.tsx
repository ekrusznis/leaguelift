...
export function SwagBrandAssetLibrary({ organizationId, teamId }: { organizationId: string; teamId: string | null }) {
	...
	return (
		<section className="flex flex-col gap-3 rounded-lg border border-slate-gray/20 bg-ice-white dark:bg-[#0f172a] p-4" aria-labelledby="swag-brand-assets-heading">
			<div className="flex flex-wrap items-center justify-between gap-3">
				<div><h3 id="swag-brand-assets-heading" className="font-heading text-base font-semibold text-navy dark:text-[#f8fafc]">Swag Brand Asset Library</h3><p className="text-sm text-slate-gray dark:text-[#cbd5e1]">Reusable approved marks. Products keep a snapshot when an asset is selected.</p></div>
				<label className="flex items-center gap-2 text-sm text-navy dark:text-[#f8fafc]"><input type="checkbox" checked={showArchived} onChange={(event) => setShowArchived(event.target.checked)} /> Show archived</label>
			</div>
			<p className="text-xs text-slate-gray dark:text-[#cbd5e1]">New uploads are saved to {teamId ? "this team" : "the organization"}. Team workspaces can also select active organization-wide assets.</p>
			<div className="grid items-end gap-3 md:grid-cols-3">
				<label className="flex flex-col gap-1 text-sm font-medium text-navy dark:text-[#f8fafc]">Asset name<input value={name} maxLength={120} onChange={(event) => setName(event.target.value)} className="min-h-11 rounded-md border border-slate-gray/30 px-3 py-2" /></label>
				<label className="flex flex-col gap-1 text-sm font-medium text-navy dark:text-[#f8fafc]">Asset type<select value={category} onChange={(event) => setCategory(event.target.value as SwagBrandAssetCategory)} className="min-h-11 rounded-md border border-slate-gray/30 px-3 py-2">{CATEGORIES.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}</select></label>
				<label className="flex flex-col gap-1 text-sm font-medium text-navy dark:text-[#f8fafc]">Upload PNG, JPEG, or WebP<input type="file" accept="image/png,image/jpeg,image/webp" disabled={state.status === "uploading"} onChange={(event) => upload(event.target.files?.[0])} className="text-sm" /></label>
			</div>
			{state.status === "uploading" && <p className="text-sm text-slate-gray dark:text-[#cbd5e1]">Uploading and validating…</p>}
			...
			{assets.data && assets.data.length === 0 && <p className="text-sm text-slate-gray dark:text-[#cbd5e1]">No brand assets yet.</p>}
			{assets.data && assets.data.length > 0 && <ul className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">{assets.data.map((asset) => {
				...
				return (
					<li key={asset.id} className="flex gap-3 rounded-md border border-slate-gray/20 bg-pure-white dark:bg-[#111827] p-3">
						<img src={asset.previewUrl} alt="" className="h-16 w-16 rounded-md border border-slate-gray/20 object-contain" />
						<div className="min-w-0 flex-1"><p className="truncate font-medium text-navy dark:text-[#f8fafc]">{asset.name}</p><p className="text-xs text-slate-gray dark:text-[#cbd5e1]">{asset.category.replaceAll("_", " ")} · {asset.teamId ? "Team" : "Organization"}</p><p className="text-xs text-slate-gray dark:text-[#cbd5e1]">{asset.status}</p></div>
...
export function ProductBrandAssetAssignment({ organizationId, storeId, teamId, product }: { organizationId: string; storeId: string; teamId: string | null; product: ManagedProduct }) {
	...
	return (
		<div className="flex flex-col gap-2 rounded-md border border-slate-gray/20 bg-ice-white dark:bg-[#0f172a] p-3">
			<label htmlFor={`brand-asset-${product.id}`} className="text-sm font-medium text-navy dark:text-[#f8fafc]">Swag Brand Asset</label>
			<div className="flex flex-wrap items-center gap-2"><select id={`brand-asset-${product.id}`} value={selected} onChange={(event) => setSelected(event.target.value)} className="min-h-11 min-w-64 rounded-md border border-slate-gray/30 px-3 py-2"><option value="">Select an approved asset…</option>{assets.data?.filter((asset) => asset.status === "ACTIVE").map((asset) => <option key={asset.id} value={asset.id}>{asset.name} ({asset.category.replaceAll("_", " ")})</option>)}</select><Button type="button" variant="secondary" onClick={save} disabled={assign.isPending}>{assign.isPending ? "Saving…" : "Use asset"}</Button>{product.hasSwagBrandAsset && <span className="rounded-full bg-pure-white dark:bg-[#111827] px-2 py-1 text-xs text-slate-gray dark:text-[#cbd5e1]">Snapshot assigned</span>}</div>
			<p className="text-xs text-slate-gray dark:text-[#cbd5e1]">Archiving the library source later will not change this product or any order history.</p>
