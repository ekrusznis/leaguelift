...
export function FinancialCorrectionsPanel({ organizationId }: { organizationId: string }) {
	...
	return <section className="rounded-xl border border-slate-gray/20 bg-pure-white p-5" aria-labelledby="corrections-heading">
		<div className="flex flex-wrap items-start justify-between gap-3">
			<div><h3 id="corrections-heading" className="font-heading text-lg font-semibold text-navy">Controlled refunds &amp; reversals</h3><p className="mt-1 text-sm text-slate-gray">Preview the exact provider and ledger effect before confirming. Original records remain visible.</p></div>
		...
		<form onSubmit={createPreview} className="mt-4 grid gap-3 lg:grid-cols-2" noValidate>
			<label className="grid gap-1 text-sm text-navy">Record type<select className={inputClass} value={targetType} onChange={(e) => { setTargetType(e.target.value as FinancialCorrectionTargetType); setPreview(null); }}><option value="CONTRIBUTION">Contribution</option><option value="SPONSORSHIP">Sponsorship</option><option value="ORDER">Order</option><option value="OFFLINE_FINANCIAL_RECORD">Offline record</option></select></label>
			<label className="grid gap-1 text-sm text-navy">Record ID<input className={inputClass} value={targetId} onChange={(e) => { setTargetId(e.target.value); setPreview(null); }} placeholder="UUID from the related record" /></label>
			<label className="grid gap-1 text-sm text-navy">Amount (leave blank for full)<input className={inputClass} type="number" min="0.01" step="0.01" value={amount} onChange={(e) => { setAmount(e.target.value); setPreview(null); }} /></label>
			<label className="grid gap-1 text-sm text-navy">Reason<textarea className={inputClass} rows={2} value={reason} onChange={(e) => { setReason(e.target.value); setPreview(null); }} /></label>
		...
		{preview && <div className="mt-4 rounded-lg border border-amber-300 bg-amber-50 p-4">
			<h4 className="font-semibold text-navy">Confirm {preview.correctionType.toLowerCase()}</h4>
			<p className="mt-1 text-sm text-navy">{preview.targetLabel}: {money(preview.requestedAmountMinor, preview.currency)}. Remaining correctable amount: {money(preview.remainingAfterMinor, preview.currency)}.</p>
			<ul className="mt-2 list-disc pl-5 text-sm text-slate-gray">{preview.warnings.map((warning) => <li key={warning}>{warning}</li>)}</ul>
		...
		<div className="mt-6"><h4 className="font-semibold text-navy">Correction history</h4>{list.data?.items.length ? <ul className="mt-2 grid gap-2">{list.data.items.map((item) => <li key={item.id} className="rounded-md bg-ice-white p-3 text-sm text-slate-gray"><strong className="text-navy">{item.correctionType}</strong> · {item.targetType} · {money(item.amountMinor, item.currency)} · {new Date(item.createdAt).toLocaleString()}<div className="break-all text-xs">Target: {item.targetId}</div><div>{item.reason}</div></li>)}</ul> : <EmptyState title="No financial corrections" description="Completed refunds and reversals will appear here." />}</div>
	</section>;
}
