...
export function DashCard({ title, action, children, className = "", tone = "light" }: DashCardProps) {
	const surface =
		tone === "dark"
					...
					{action &&
						(action.to ? (
							...
							<button
								...
								className="flex items-center gap-1 text-sm font-semibold text-green-600 hover:text-green-500"
							>
								{action.label}
								<ChevronMini />
							</button>
