...
export function OwnerOnboardingPage() {
	...
	return (
		<main className="min-h-screen bg-slate-50 dark:bg-[#1e293b] px-4 py-10 sm:px-6">
			<div className="mx-auto max-w-5xl">
				<div className="mb-8">
					<p className="text-sm font-semibold uppercase tracking-[0.18em] text-green-700">Rally26 setup</p>
					<h1 className="mt-2 font-heading text-3xl font-extrabold text-navy-900 dark:text-[#f8fafc]">Create your organization</h1>
					<p className="mt-2 max-w-2xl text-slate-600 dark:text-[#cbd5e1]">Your progress is saved after each step. You can close this page and resume after signing in again.</p>
				</div>
				...
				<ol className="mb-8 grid gap-3 sm:grid-cols-4" aria-label="Onboarding progress">
					{STEP_ORDER.map((label, index) => {
						...
						const className = `block rounded-xl border px-4 py-3 text-sm font-semibold ${index <= step ? "border-green-300 bg-green-50 dark:bg-green-950 text-green-900" : "border-slate-200 dark:border-[#334155] bg-white dark:bg-[#111827] text-slate-500 dark:text-[#cbd5e1]"}`;
				...
				{routeStep === "organization" && <section className="rounded-2xl border border-slate-200 dark:border-[#334155] bg-white dark:bg-[#111827] p-6 shadow-sm">
					<h2 className="text-lg font-bold text-navy-900 dark:text-[#f8fafc]">2. Organization</h2>
					<p className="mt-1 text-sm text-slate-600 dark:text-[#cbd5e1]">Create the draft club profile and confirm its timezone.</p>
					<div className="mt-5 grid gap-4 sm:grid-cols-2">
						<label className="text-sm font-semibold text-slate-700 dark:text-[#cbd5e1]">Organization name<input className="mt-1 w-full rounded-lg border border-slate-300 dark:border-[#334155] px-3 py-2" value={organizationForm.name} onChange={(e) => { updateField("name", e.target.value); if (!slugTouched) updateField("slug", slugify(e.target.value)); }} /></label>
						<label className="text-sm font-semibold text-slate-700 dark:text-[#cbd5e1]">Organization URL<input disabled={Boolean(onboarding?.organization)} className="mt-1 w-full rounded-lg border border-slate-300 dark:border-[#334155] px-3 py-2 disabled:bg-slate-100 disabled:dark:bg-slate-800" value={organizationForm.slug} onChange={(e) => { setSlugTouched(true); updateField("slug", slugify(e.target.value)); }} /></label>
						<label className="text-sm font-semibold text-slate-700 dark:text-[#cbd5e1]">Organization type<select className="mt-1 w-full rounded-lg border border-slate-300 dark:border-[#334155] px-3 py-2" value={organizationForm.organizationType} onChange={(e) => updateField("organizationType", e.target.value)}>{ORGANIZATION_TYPES.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label>
						<div className="sm:col-span-2">
							<span className="text-sm font-semibold text-slate-700 dark:text-[#cbd5e1]">Sports</span>
							<div className="mt-1 grid grid-cols-2 gap-2 sm:grid-cols-3">
								{SPORT_OPTIONS.map((sport) => (
									<label key={sport} className="flex items-center gap-2 text-sm text-slate-700 dark:text-[#cbd5e1]">
										<input type="checkbox" checked={organizationForm.sports.includes(sport)} onChange={() => toggleSport(sport)} className="size-4 rounded border-slate-300 dark:border-[#334155]" />
										{sport}
									</label>
								))}
							</div>
						</div>
						<label className="text-sm font-semibold text-slate-700 dark:text-[#cbd5e1]">Contact email<input type="email" className="mt-1 w-full rounded-lg border border-slate-300 dark:border-[#334155] px-3 py-2" value={organizationForm.contactEmail} onChange={(e) => updateField("contactEmail", e.target.value)} /></label>
						<label className="text-sm font-semibold text-slate-700 dark:text-[#cbd5e1]">Contact phone<input className="mt-1 w-full rounded-lg border border-slate-300 dark:border-[#334155] px-3 py-2" value={organizationForm.contactPhone ?? ""} onChange={(e) => updateField("contactPhone", e.target.value)} /></label>
						<label className="text-sm font-semibold text-slate-700 dark:text-[#cbd5e1] sm:col-span-2">Address<input className="mt-1 w-full rounded-lg border border-slate-300 dark:border-[#334155] px-3 py-2" value={organizationForm.addressLine1} onChange={(e) => updateField("addressLine1", e.target.value)} /></label>
						<label className="text-sm font-semibold text-slate-700 dark:text-[#cbd5e1]">City<input className="mt-1 w-full rounded-lg border border-slate-300 dark:border-[#334155] px-3 py-2" value={organizationForm.addressCity} onChange={(e) => updateField("addressCity", e.target.value)} /></label>
						<label className="text-sm font-semibold text-slate-700 dark:text-[#cbd5e1]">State / province<input className="mt-1 w-full rounded-lg border border-slate-300 dark:border-[#334155] px-3 py-2" value={organizationForm.addressState} onChange={(e) => updateField("addressState", e.target.value)} /></label>
						<label className="text-sm font-semibold text-slate-700 dark:text-[#cbd5e1]">Postal code<input className="mt-1 w-full rounded-lg border border-slate-300 dark:border-[#334155] px-3 py-2" value={organizationForm.addressPostalCode} onChange={(e) => updateField("addressPostalCode", e.target.value)} /></label>
						<label className="text-sm font-semibold text-slate-700 dark:text-[#cbd5e1]">Country code<input maxLength={2} className="mt-1 w-full rounded-lg border border-slate-300 dark:border-[#334155] px-3 py-2 uppercase" value={organizationForm.addressCountry} onChange={(e) => updateField("addressCountry", e.target.value.toUpperCase())} /></label>
						<div className="sm:col-span-2">
							<div className="flex flex-wrap items-end gap-3">
								<label className="min-w-0 flex-1 text-sm font-semibold text-slate-700 dark:text-[#cbd5e1]">Confirmed timezone<input className="mt-1 w-full rounded-lg border border-slate-300 dark:border-[#334155] px-3 py-2" value={organizationForm.timezone} onChange={(e) => updateField("timezone", e.target.value)} /><span className="mt-1 block text-xs font-normal text-slate-500 dark:text-[#cbd5e1]">Use an IANA timezone such as America/New_York.</span></label>
								<button type="button" onClick={suggestTimezone} className="rounded-lg border border-slate-300 dark:border-[#334155] bg-white dark:bg-[#111827] px-3 py-2 text-sm font-bold text-slate-800 dark:text-slate-200">Suggest from address</button>
							</div>
							{timezoneSuggestion && <div className="mt-3 rounded-lg border border-blue-200 bg-blue-50 dark:bg-blue-950 p-3 text-sm text-blue-900">Suggested timezone: <strong>{timezoneSuggestion}</strong>. <button type="button" onClick={() => { updateField("timezone", timezoneSuggestion); setTimezoneSuggestion(null); }} className="font-bold underline">Use this timezone</button></div>}
						</div>
					</div>
