	...
	return <main className="min-h-screen bg-slate-50 dark:bg-[#1e293b] px-4 py-10 sm:px-6"><div className="mx-auto max-w-5xl">
		...
		{routeStep === "organization" && <section className="rounded-2xl border border-slate-200 dark:border-[#334155] bg-white dark:bg-[#111827] p-6 shadow-sm">
			...
			<div className="mt-5 grid gap-4 sm:grid-cols-2">
				...
				<div className="sm:col-span-2"><span className={label}>Sports{requiredMark(organizationForm.sports.length > 0)}</span><div className="mt-1 grid grid-cols-1 gap-x-3 gap-y-2 sm:grid-cols-3">{SPORT_OPTIONS.map((sport) => <label key={sport} className="flex items-start gap-2 text-sm text-slate-700 dark:text-[#cbd5e1]"><input type="checkbox" checked={organizationForm.sports.includes(sport)} onChange={() => toggleSport(sport)} className="mt-0.5 size-4 shrink-0 rounded border-slate-300 dark:border-[#334155]" /><span>{sport}</span></label>)}</div></div>
