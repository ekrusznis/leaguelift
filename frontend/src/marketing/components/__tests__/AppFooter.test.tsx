...
describe("AppFooter", () => {
	it("shows help, support, legal, copyright, and a public home logo", () => {
		...
		expect(screen.getByRole("link", { name: "LeagueLift home" })).toHaveAttribute("href", "/");
		...
		expect(screen.getByText(/LeagueLift\. All rights reserved\./)).toBeInTheDocument();
	});

	it("uses authenticated help routes inside the app", () => {
		renderWithProviders(<AppFooter authenticated />);

		expect(screen.getByRole("link", { name: "LeagueLift home" })).toHaveAttribute("href", "/app");
