...
import { PREVIEW_NAV_LINKS, type PreviewSectionId } from "../content/landingPreviewNav";
...
/**
 * Single-page header for the /landing-preview experiment: flat anchor nav instead
 * of the real site's Product/Solutions/Resources dropdowns, since every
 * destination here is a scroll target on the same page rather than a route.
 * Highlights the section currently in view via IntersectionObserver.
 */
export function LandingPreviewHeader() {
	const [scrolled, setScrolled] = useState(false);
	const [mobileOpen, setMobileOpen] = useState(false);
	const [activeHash, setActiveHash] = useState<PreviewSectionId>(PREVIEW_NAV_LINKS[0].hash);
	...
	useEffect(() => {
		const sections = PREVIEW_NAV_LINKS.map((link) => document.getElementById(link.hash)).filter(
		...
		const observer = new IntersectionObserver(
			(entries) => {
				...
				setActiveHash(topMost.target.id as PreviewSectionId);
			},
		...
		<>
			...
			>
				<PageContainer className="flex h-full items-center justify-between gap-4">
					<Logo tone="dark" to="/landing-preview" />
					...
					<nav aria-label="Primary" className="hidden items-center gap-1 lg:flex">
						{PREVIEW_NAV_LINKS.map((link) => (
							<button
								...
								className={`rounded-md px-3 py-2 text-sm font-medium transition-colors focus-visible:outline focus-visible:outline-2 focus-visible:outline-green-400 ${
					...
					<button
						...
						className="rounded-md p-2 text-white hover:bg-white/10 focus-visible:outline focus-visible:outline-2 focus-visible:outline-green-400 lg:hidden"
					>
