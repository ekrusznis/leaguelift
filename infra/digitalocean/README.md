...
- DigitalOcean DNS for `leaguelift.io` (`@`, `www`, `api` A records).
...
3. Add the GitHub Actions deploy SSH public key to the droplet's
   `~/.ssh/authorized_keys` (root and/or the `leaguelift` deploy user created by the
   bootstrap script).
...
5. Run the `infra-bootstrap` workflow by hand once (creates the registry + DNS
   records + firewall).
6. Point `leaguelift.io`'s registrar nameservers at DigitalOcean's
