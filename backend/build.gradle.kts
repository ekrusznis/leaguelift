...
plugins {
    ...
    // Lints Kotlin sources against ktlint's standard style. `check` (and therefore
    // `build`) depends on `ktlintCheck` by default, so every build fails on style
    // violations; `ktlintFormat` auto-fixes them and shows up as a runnable Gradle
    // task in IntelliJ's Gradle tool window.
