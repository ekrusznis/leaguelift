...
create table google_calendar_connection_setting (
    ...
    sync_direction            text not null default 'RALLY26_TO_GOOGLE',
    automatic_sync_enabled    boolean not null default false,
    last_calendar_listed_at   timestamptz,
    created_at                timestamptz not null default now(),
    updated_at                timestamptz not null default now(),
    constraint google_calendar_setting_direction_check check (sync_direction in ('RALLY26_TO_GOOGLE'))
);
...
values
    ('10000000-0000-4000-8000-000000000016', 'connecting-google-calendar',
     ...
     E'## Personal connection\n\nGoogle Calendar is a personal integration. When Rally26 has activated its verified Google OAuth application, any signed-in adult or permitted athlete can open **My Integrations**, authorize a Google account, and select one destination calendar. Organization owners do not connect Google Calendar on behalf of everyone else.\n\n## Current availability\n\nUntil the provider is activated and verified, Rally26 shows **Not configured** and does not request Google credentials. Standard ICS downloads and Add to Calendar files continue to work without a Google connection.\n\n## Planned first sync direction\n\nThe first supported direction is Rally26 to Google Calendar. Rally26 event identity is preserved through a durable mapping so updates and cancellations can target the same external event rather than creating duplicates. Automatic synchronization remains disabled until credentialed provider testing is complete.\n\n## Disconnecting\n\nDisconnecting removes Rally26 access to the stored provider credential. Existing events already written to Google are not silently deleted unless the user explicitly chooses a supported removal workflow.',
     'Events and RSVP', 'PUBLIC', 'PUBLISHED', 56, now()),
    ('10000000-0000-4000-8000-000000000017', 'platform-managed-integrations',
     'Why some integrations are managed by Rally26',
     'Why organizations never enter Rally26 platform credentials for payments, fulfillment, communications, or storage.',
     E'## Platform-operated providers\n\nStripe, Printify, Resend, Twilio, and DigitalOcean Spaces are Rally26 infrastructure. Their credentials belong to the Rally26 environment, not to an organization account. Organization users should never be asked to paste these provider secrets into Rally26.\n\n## What organizations can see\n\nCustomer-facing pages show the resulting Rally26 workflow, such as checkout, fulfillment status, email or SMS delivery results, and media uploads. Only authorized Rally26 Platform Administrators can see sanitized configuration readiness and provider-operation health. Raw keys, tokens, webhook secrets, and permanent storage credentials are never returned to a browser.',
