...
values
    ('10000000-0000-4000-8000-000000000018', 'quickbooks-online-readiness',
     'Preparing QuickBooks Online for LeagueLift',
     'What owners and administrators can prepare before QuickBooks authorization and exports are activated.',
     E'## Who manages the connection\n\nOnly an organization owner or administrator can connect QuickBooks Online. The connection belongs to the organization, not an individual bookkeeper account inside LeagueLift.\n\n## Phase 19 readiness\n\nLeagueLift models the company realm, chart-of-accounts mapping, export batches, idempotency, token refresh, revocation, and reconciliation results. Until LeagueLift activates and verifies its Intuit application, the card remains **Not configured** and no accounting data is sent.\n\n## Account mapping\n\nBefore an export can be enabled, review mappings for sales income, contributions, sponsorship income, refunds, fee receivables, bank clearing, and payout clearing. Phase 19 is preview/read-only. Accounting writes require sandbox verification and an approved accounting policy in Phase 20.\n\n## Existing financial history\n\nQuickBooks export never replaces LeagueLift ledger history. Export batches retain source identity and payload hashes so retries remain idempotent.',
    ...
    ('10000000-0000-4000-8000-000000000019', 'sportsengine-integration-readiness',
     ...
     E'## Current status\n\nSportsEngine remains **Not configured** until LeagueLift verifies the current official product, endpoints, scopes, rate limits, and terms. Legacy API documentation is not treated as current access.\n\n## Planned workflow\n\nAn owner or administrator authorizes the organization, reviews mapped organizations, teams, participants, roster memberships, and events, resolves conflicts, and confirms an idempotent import. Every imported record retains provider and external identity.\n\n## Safety\n\nPhase 19 provides preview and mapping contracts only. It does not silently create or overwrite LeagueLift records and it never asks for an undocumented token.',
    ...
    ('10000000-0000-4000-8000-000000000020', 'gamechanger-maxpreps-imports',
     ...
     E'## Partner-pending providers\n\nLeagueLift does not claim a direct GameChanger or MaxPreps API connection without an official supported contract. It does not scrape or automate either consumer website.\n\n## Supported paths\n\nUse provider-supported CSV or calendar exports where available, then preview mappings and validation results before importing. Existing LeagueLift CSV event import and ICS feed workflows remain available.\n\n## Future activation\n\nIf an official partner interface becomes available, the existing external-ID, mapping, sync-run, issue, disconnect, and recovery scaffolding can be connected to the verified provider client without redesigning the user workflow.',
