...
values
    ...
    ('MAXPREPS', 'MaxPreps', 'SPORTS_DATA', 'ORGANIZATION', 'FILE_IMPORT', '["FILE_IMPORT"]',
     'PARTNER_PENDING', 'PARTNER_PENDING',
     'File-import and partner workflow scaffold only; LeagueLift does not scrape the consumer website.',
     'Requires a formal partner mechanism or supported export/import contract.',
     '[]', 50, true),
    ('ICS_FEED', 'ICS Feed', 'CALENDAR', 'ORGANIZATION', 'URL', '["URL"]',
     'AVAILABLE', 'EXISTING',
     'Stateful schedule feed subscription already available in LeagueLift.',
    ...
    ('STRIPE', 'Stripe', 'PAYMENTS', 'PLATFORM', 'PLATFORM_SECRET', '["PLATFORM_SECRET","WEBHOOK"]',
     'PLATFORM_MANAGED', 'PLATFORM_MANAGED', 'LeagueLift-managed payment and payout infrastructure.',
     'Configured and verified by LeagueLift; organizations never enter platform Stripe credentials.', '[]', 100, false),
    ('PRINTIFY', 'Printify', 'FULFILLMENT', 'PLATFORM', 'PLATFORM_SECRET', '["PLATFORM_SECRET","WEBHOOK"]',
     'PLATFORM_MANAGED', 'PLATFORM_MANAGED', 'LeagueLift-managed apparel catalog and fulfillment infrastructure.',
     'Configured and verified by LeagueLift; organizations never enter platform Printify credentials.', '[]', 110, false),
    ('RESEND', 'Resend', 'COMMUNICATIONS', 'PLATFORM', 'PLATFORM_SECRET', '["PLATFORM_SECRET","WEBHOOK"]',
     'PLATFORM_MANAGED', 'PLATFORM_MANAGED', 'LeagueLift-managed transactional email delivery.',
     'Configured and verified by LeagueLift; organizations never enter email-provider credentials.', '[]', 120, false),
    ('TWILIO', 'Twilio', 'COMMUNICATIONS', 'PLATFORM', 'PLATFORM_SECRET', '["PLATFORM_SECRET","WEBHOOK"]',
     'PLATFORM_MANAGED', 'PLATFORM_MANAGED', 'LeagueLift-managed opted-in transactional SMS delivery.',
     'Configured and verified by LeagueLift; organizations never enter SMS-provider credentials.', '[]', 130, false),
    ('DIGITALOCEAN_SPACES', 'DigitalOcean Spaces', 'STORAGE', 'PLATFORM', 'PLATFORM_SECRET', '["PLATFORM_SECRET"]',
     'PLATFORM_MANAGED', 'PLATFORM_MANAGED', 'LeagueLift-managed private media and document storage.',
     'Configured and verified by LeagueLift; permanent storage credentials are never browser-visible.', '[]', 140, false),
...
values
    ('10000000-0000-4000-8000-000000000015', 'understanding-integration-statuses',
     ...
     'What Available, Not configured, Partner pending, Connected, Needs attention, and Disconnected mean in LeagueLift.',
     E'## Availability is not a connection\n\n**Available** means LeagueLift has a supported connection or import workflow. **Not configured** means the safe adapter is present but LeagueLift has not activated the required provider application, credentials, callback, scopes, or contract verification. **Partner pending** means no supported direct API has been verified; use the displayed CSV, ICS, or manual workflow instead.\n\n## Connection states\n\n**Authorization pending** means an OAuth attempt started but did not complete. **Connected** is shown only after a provider authorization or verified platform configuration succeeds. **Needs attention** means the stored connection exists but refresh, health, or sync failed. **Disconnected** and **Revoked** are terminal states until a new authorization starts.\n\n## Credentials\n\nLeagueLift never returns stored access or refresh tokens to the browser. Organization users should never enter LeagueLift platform credentials for Stripe, Printify, Resend, Twilio, or DigitalOcean Spaces.',
