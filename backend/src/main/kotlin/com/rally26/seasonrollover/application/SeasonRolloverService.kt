...
import com.rally26.seasonrollover.domain.SeasonRolloverPreview
import com.rally26.seasonrollover.domain.SeasonRolloverResult
...
class SeasonRolloverService(
    ...
    private fun analyze(
        ...
        val source =
            teamRepository.findById(command.sourceTeamId, organizationId)
        ...
        if (source.name.equals(command.newTeamName, ignoreCase = true)) {
            throw ValidationException(
                "The new team name must differ from the source team name because archived team names remain reserved.",
            )
        }
        ...
        return SeasonRolloverPreview(
            ...
            warnings =
                buildList {
                    if (command.copyRoster) {
                        add(
                            "Roster copy reuses active participant records and creates new team links; prior joined dates are not carried forward.",
                        )
                    }
                    if (command.copyStaff) {
                        add(
                            "Staff copy includes only explicit active TEAM role assignments. Organization owner/administrator inheritance already applies automatically.",
                        )
                    }
                    if (command.copyBranding) {
                        add(
                            "Branding copy reuses the current ready logo/cover assets; it does not duplicate uploaded files.",
                        )
                    }
                    if (command.archiveSourceTeam) {
                        add(
                            "Archiving changes only the source team's status. Its historical records remain intact and are not moved or deleted.",
                        )
                    }
                },
    ...
    private fun normalize(command: SeasonRolloverCommand): SeasonRolloverCommand {
        ...
        if (name.length >
            ...
            throw ValidationException("New team name must be $MAX_TEAM_NAME_LENGTH characters or fewer.")
        }
    ...
    private fun stableValue(value: Any?): Any? =
        when (value) {
    ...
    private fun completedResult(
        organizationId: UUID,
        run: SeasonRolloverRun,
    ): SeasonRolloverResult {
        val destination =
            teamRepository.findById(run.destinationTeamId, organizationId)
