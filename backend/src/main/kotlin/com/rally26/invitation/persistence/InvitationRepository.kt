...
class InvitationRepository(
    ...
    fun findById(id: UUID): Invitation? =
        jdbcClient
            .sql("select $INVITATION_COLUMNS from invitation where id = :id")
            .param("id", id)
            .query(::mapRow)
            .optional()
            .orElse(null)
    ...
    fun findByTokenHash(tokenHash: String): Invitation? =
        jdbcClient
            .sql("select $INVITATION_COLUMNS from invitation where token_hash = :tokenHash")
            .param("tokenHash", tokenHash)
            .query(::mapRow)
            .optional()
            .orElse(null)

    fun listPendingForOrganization(
        organizationId: UUID,
        offset: Int,
        limit: Int,
    ): List<Invitation> =
    ...
    fun countPendingForOrganization(organizationId: UUID): Long =
        jdbcClient
            .sql("select count(*) from invitation where organization_id = :organizationId and status = 'PENDING'")
            .param("organizationId", organizationId)
            .query(Long::class.java)
            .single()

    fun findPendingForOrganizationAndEmail(
        organizationId: UUID,
        email: String,
    ): Invitation? =
        jdbcClient
