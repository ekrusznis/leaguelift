...
import com.rally26.reporting.persistence.CampaignRevenueRow
import com.rally26.reporting.persistence.FeeCollectionRow
import com.rally26.reporting.persistence.ProductPerformanceRow
import com.rally26.reporting.persistence.RefundRow
...
data class RevenueReport(
    val from: LocalDate,
    val to: LocalDate,
    val bySourceType: List<SourceTypeRevenueRow>,
    val byTeam: List<TeamRevenueRow>,
) {
    val totalMinor: Long get() = bySourceType.sumOf { it.amountMinor }
}

data class RefundsReport(
    val from: LocalDate,
    val to: LocalDate,
...
data class FeeCollectionsReport(
    val from: LocalDate,
    val to: LocalDate,
    val payments: List<FeeCollectionRow>,
...
class ReportingService(
    ...
    fun exportRevenueReportCsv(
        ...
        for (row in report.byTeam) {
            lines +=
                listOf(
                    "",
                    row.teamName ?: "Organization-wide",
                    CsvUtil.formatMinor(row.amountMinor),
                ).joinToString(",") { CsvUtil.escape(it) }
        }
        return lines.joinToString("\r\n")
    }

    fun getCampaignBreakdown(
        organizationId: UUID,
        from: LocalDate?,
        to: LocalDate?,
        currentUser: CurrentUser,
    ...
    private fun householdOutstandingBalance(
        organizationId: UUID,
        householdId: UUID,
    ): Long {
        val outstandingStatuses = setOf(FeeAssignmentStatus.OPEN, FeeAssignmentStatus.PARTIALLY_PAID)
        return feeRepository
            .findByHousehold(householdId, organizationId, offset = 0, limit = HOUSEHOLD_FEE_ASSIGNMENT_LIMIT)
    ...
    private fun toInstantRange(
        from: LocalDate,
        to: LocalDate,
    ): Pair<Instant, Instant> = from.atStartOfDay(ZoneOffset.UTC).toInstant() to to.plusDays(1).atStartOfDay(ZoneOffset.UTC).toInstant()
}
