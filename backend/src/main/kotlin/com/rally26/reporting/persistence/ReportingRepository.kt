...
data class SourceTypeRevenueRow(
    val sourceType: String,
    val amountMinor: Long,
)

data class TeamRevenueRow(
    val teamId: UUID?,
    val teamName: String?,
    val amountMinor: Long,
)

data class CampaignRevenueRow(
    val campaignId: UUID,
...
data class FeeCollectionRow(
    val feePaymentId: UUID,
    val householdId: UUID,
    val householdName: String,
    val amountMinor: Long,
    val paidAt: java.time.LocalDate,
)
...
class ReportingRepository(
    ...
    fun revenueBySourceType(
        organizationId: UUID,
        from: Instant,
        to: Instant,
    ): List<SourceTypeRevenueRow> =
        jdbcClient
            .sql(
                """
    ...
    fun revenueByCampaign(
        ...
        jdbcClient
            ...
            .query {
                ...
                _,
                ->
                CampaignRevenueRow(rs.getObject("campaign_id", UUID::class.java), rs.getString("campaign_name"), rs.getLong("amount_minor"))
            }.list()
        ...
        jdbcClient
            ...
            .query { rs, _ ->
                ProductPerformanceRow(
                    rs.getObject("product_id", UUID::class.java),
    ...
    fun countNewOrganizations(
        from: Instant,
        to: Instant,
    ): Long =
        jdbcClient
            .sql("select count(*) from organization where created_at >= :from and created_at < :to")
            .param("from", Timestamp.from(from))
            .param("to", Timestamp.from(to))
    ...
    fun countNewCustomers(
        from: Instant,
        to: Instant,
    ): Long =
        jdbcClient
            .sql("select count(*) from app_user where created_at >= :from and created_at < :to")
            .param("from", Timestamp.from(from))
            .param("to", Timestamp.from(to))
            .query(Long::class.java)
            .single()
    ...
    fun platformGrossTransactionVolume(
        from: Instant,
        to: Instant,
    ): Long =
        jdbcClient
    ...
    fun feeCollections(
        ...
        jdbcClient
            ...
            .query { rs, _ ->
                FeeCollectionRow(
                    rs.getObject("fee_payment_id", UUID::class.java),
