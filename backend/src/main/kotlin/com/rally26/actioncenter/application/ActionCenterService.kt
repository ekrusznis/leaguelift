...
class ActionCenterService(
    ...
    fun get(currentUser: CurrentUser): ActionCenter {
        ...
        managerOrganizationIds.forEach { organizationId ->
            ...
            if (corrections > 0) {
                items +=
                    aggregate(
                        "org:$organizationId:corrections",
                        ActionCenterType.PROFILE_CORRECTION_REVIEW,
                        ActionCenterPriority.HIGH,
                        "Review profile corrections",
                        "$corrections pending request${plural(corrections)} need an owner or administrator decision.",
                        "/app/organizations/$organizationId/corrections",
                        organizationId,
                    )
            }
            ...
            if (overdueFees > 0) {
                items +=
                    aggregate(
                        "org:$organizationId:overdue-fees",
                        ActionCenterType.OVERDUE_FEES,
                        ActionCenterPriority.HIGH,
                        "Follow up on overdue fees",
                        "$overdueFees fee assignment${plural(overdueFees)} are past due.",
                        "/app/organizations/$organizationId/fees",
                        organizationId,
                    )
            }
            val fulfillmentExceptions = repository.countFulfillmentExceptions(organizationId)
            if (fulfillmentExceptions > 0) {
                items +=
                    aggregate(
                        "org:$organizationId:fulfillment-exceptions",
                        ActionCenterType.FULFILLMENT_EXCEPTION,
                        ActionCenterPriority.URGENT,
                        "Resolve fulfillment exceptions",
                        "$fulfillmentExceptions order fulfillment item${plural(fulfillmentExceptions)} failed or require attention.",
            ...
            if (pendingOfflineRecords > 0) {
                items +=
                    aggregate(
                        "org:$organizationId:offline-financial-review",
                        ActionCenterType.OFFLINE_FINANCIAL_REVIEW,
                        ActionCenterPriority.HIGH,
                        "Verify offline financial records",
                        "$pendingOfflineRecords externally received transaction${plural(pendingOfflineRecords)} still need verification.",
            ...
            if (overdueInstallments > 0) {
                items +=
                    aggregate(
                        "org:$organizationId:overdue-installments",
                        ActionCenterType.PAYMENT_PLAN_OVERDUE,
                        ActionCenterPriority.HIGH,
                        "Follow up on overdue installments",
                        "$overdueInstallments payment-plan installment${plural(overdueInstallments)} are overdue.",
                        "/app/organizations/$organizationId/fees",
                        organizationId,
                    )
            }
            ...
            if (reconciliationIssues > 0) {
                items +=
                    aggregate(
                        "org:$organizationId:reconciliation-review",
                        ActionCenterType.RECONCILIATION_REVIEW,
                        ActionCenterPriority.HIGH,
                        "Review reconciliation exceptions",
                        "$reconciliationIssues issue${plural(reconciliationIssues)} were found in the latest reconciliation run.",
                        "/app/organizations/$organizationId/financial-operations",
                        organizationId,
                    )
            }
            ...
            if (events > 0) {
                items +=
                    aggregate(
                        "org:$organizationId:event-review",
                        ActionCenterType.EVENT_REVIEW,
                        ActionCenterPriority.NORMAL,
                        "Review unpublished events",
                        "$events draft or tentative event${plural(events)} still need review.",
        ...
        contexts
            .filter { it.contextType == ContextType.TEAM && it.organizationId !in managerOrganizationIds }
            ...
            .forEach { context ->
                ...
                if (count > 0) {
                    items +=
                        aggregate(
                            "team:$teamId:event-review",
                            ActionCenterType.EVENT_REVIEW,
                            ActionCenterPriority.NORMAL,
                            "Review ${context.label} events",
                            "$count draft or tentative event${plural(count)} still need review.",
        ...
        contexts
            .filter { it.contextType == ContextType.TOURNAMENT && it.organizationId !in managerOrganizationIds }
            .filter { Capabilities.EVENT_PUBLISH in it.capabilities || Capabilities.EVENT_UPDATE in it.capabilities }
            .forEach { context ->
                val organizationId = context.organizationId ?: return@forEach
                ...
                if (count > 0) {
                    items +=
                        aggregate(
                            "tournament:$tournamentId:event-review",
                            ActionCenterType.EVENT_REVIEW,
                            ActionCenterPriority.NORMAL,
                            "Review ${context.label} events",
                            "$count draft or tentative event${plural(count)} still need review.",
        ...
        repository.listGuardianFeeActions(currentUser.userId, ACTION_LIMIT).forEach { fee ->
            ...
            items +=
                ActionCenterItem(
                    id = "fee:${fee.id}",
                    type = ActionCenterType.FEE_PAYMENT,
        ...
        repository.listGuardianDocumentActions(currentUser.userId, ACTION_LIMIT).forEach { document ->
            items +=
                ActionCenterItem(
                    id = "document:${document.assignmentId}",
                    type = ActionCenterType.DOCUMENT_ACKNOWLEDGMENT,
                    priority = ActionCenterPriority.HIGH,
                    title = "Document acknowledgment needed",
                    ...
                    contextType = "HOUSEHOLD",
                    contextId = document.householdId,
        ...
        (
            repository.listGuardianRsvpActions(currentUser.userId, ACTION_LIMIT) +
                repository.listAthleteRsvpActions(currentUser.userId, ACTION_LIMIT)
        ).forEach { rsvp ->
            ...
            items +=
                ActionCenterItem(
                    id = id,
                    type = ActionCenterType.EVENT_RSVP,
                    ...
                    organizationId = rsvp.organizationId,
                    contextType = "PARTICIPANT",
                    contextId = rsvp.participantId,
                    dueAt = rsvp.startAt,
        ...
        repository.listSupportCaseActions(currentUser.userId, ACTION_LIMIT).forEach { supportCase ->
            items +=
                ActionCenterItem(
                    ...
                    type = ActionCenterType.SUPPORT_CASE_RESPONSE,
                    priority = ActionCenterPriority.HIGH,
                    title = "Support is waiting for you",
                    description = supportCase.subject,
                    actionPath = "/app/help/support",
            ...
            if (supportCount > 0) {
                items +=
                    aggregate(
                        ...
                        ActionCenterType.PLATFORM_SUPPORT_QUEUE,
                        ActionCenterPriority.URGENT,
                        "Triage priority support cases",
                        "$supportCount high-priority open case${plural(supportCount)} need attention.",
            ...
            if (failures > 0) {
                items +=
                    aggregate(
                        ...
                        ActionCenterType.PLATFORM_DELIVERY_FAILURES,
                        ActionCenterPriority.URGENT,
                        "Review delivery failures",
                        "$failures failed or dead-letter delivery item${plural(failures)} need operational review.",
        ...
        val sorted =
            items.distinctBy { it.id }.sortedWith(
        ...
        return ActionCenter(
            sorted,
            sorted.size,
            sorted.count { it.priority in setOf(ActionCenterPriority.HIGH, ActionCenterPriority.URGENT) },
        )
    }
    ...
    private fun priorityRank(priority: ActionCenterPriority) =
        when (priority) {
