...
class SupportCaseStatusChangedEmailHandler(
    ...
    override fun handle(event: OutboxEvent) {
        ...
        val resolutionHtml = supportCase.resolution?.takeIf { it.isNotBlank() }?.let { resolution ->
        ...
        emailProvider.send(
            EmailMessage(
                ...
                body = "Your Rally26 support case \"${supportCase.subject}\" is now $statusLabel.$resolutionLine\n\n" +
                    "View your cases: $caseUrl\n\n— Rally26 Support",
                template = resendTemplateProperties.supportCaseUpdateId.takeIf { it.isNotBlank() }?.let { templateId ->
                    EmailTemplateRef(
                        id = templateId,
                        variables = mapOf(
    ...
    private fun statusLabel(status: SupportCaseStatus): String = when (status) {
