...
object WelcomeEmailFeatures {
    private val OWNER_AND_ADMINISTRATOR_FEATURES =
        listOf(
    ...
    private val TEAM_ADMINISTRATOR_FEATURES =
        listOf(
    ...
    private val TOURNAMENT_ADMINISTRATOR_FEATURES =
        listOf(
            "Tournament schedules & events",
            "Tournament communications",
        )

    private val VIEWER_FEATURES =
        listOf(
    ...
    fun featuresFor(role: MembershipRole): List<String> =
        when (role) {
    ...
    fun featuresHtml(role: MembershipRole): String = featuresFor(role).joinToString(separator = "") { feature -> featureRowHtml(feature) }
