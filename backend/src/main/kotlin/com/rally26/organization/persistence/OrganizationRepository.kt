...
class OrganizationRepository(
    ...
    fun findById(id: UUID): Organization? =
        jdbcClient
            .sql("select $ORGANIZATION_COLUMNS from organization where id = :id")
            .param("id", id)
            .query(::mapRow)
            .optional()
            .orElse(null)
    ...
    fun findBySlug(slug: String): Organization? =
        jdbcClient
            .sql("select $ORGANIZATION_COLUMNS from organization where slug = :slug")
            .param("slug", slug)
            .query(::mapRow)
            .optional()
            .orElse(null)
    ...
    fun countAllForPlatformAdmin(): Long = jdbcClient.sql("select count(*) from organization").query(Long::class.java).single()
