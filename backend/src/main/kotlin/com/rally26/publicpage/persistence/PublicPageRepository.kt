package com.rally26.publicpage.persistence
...
class PublicPageRepository(
    ...
    fun findById(
        ...
        jdbcClient
            .sql("select $PAGE_COLUMNS from public_page where id = :id and organization_id = :organizationId")
            .param("id", id)
    ...
    fun findBySlug(slug: String): PublicPage? =
        jdbcClient
            .sql("select $PAGE_COLUMNS from public_page where lower(slug) = lower(:slug)")
            .param("slug", slug)
            .query(::mapRow)
            .optional()
            .orElse(null)
    ...
    fun findByEntityId(entityId: UUID): PublicPage? =
        jdbcClient
            .sql("select $PAGE_COLUMNS from public_page where entity_id = :entityId")
    ...
    fun countForOrg(organizationId: UUID): Long =
        jdbcClient
            .sql("select count(*) from public_page where organization_id = :organizationId")
            .param("organizationId", organizationId)
