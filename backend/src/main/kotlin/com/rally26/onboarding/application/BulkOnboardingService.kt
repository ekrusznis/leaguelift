...
import com.rally26.onboarding.domain.BulkActionItemResult
...
data class BulkInvitationCommand(
    val email: String,
...
class BulkOnboardingService(
    ...
    fun assignParticipantsToTeam(
        ...
        val results =
            participantIds.distinct().map { participantId ->
                ...
                if (participant == null || participant.status != ParticipantStatus.ACTIVE) {
                    BulkActionItemResult(
                        ...
                        message = "The active participant could not be found.",
                    )
                } else {
    ...
    fun assignFeeTemplate(
        ...
        val results =
            householdIds.distinct().map { householdId ->
                ...
                when {
                    household == null || household.status != HouseholdStatus.ACTIVE ->
                        BulkActionItemResult(
                            ...
                            BulkActionStatus.FAILED,
                            message = "The active household could not be found.",
                        )
                    feeRepository.hasActiveTemplateAssignment(organizationId, householdId, feeTemplateId, dueDate) ->
                        BulkActionItemResult(
                            household.displayName,
                            BulkActionStatus.SKIPPED,
                            message = "This fee is already assigned for the selected due date.",
                        )
                    else -> {
                        try {
                            val assignment =
                ...
                when {
                    ...
                    mediaAssignmentRepository.findActiveByAssetAndEntity(
                        ...
                        BulkActionItemResult(
                            household.displayName,
                            BulkActionStatus.SKIPPED,
                            message = "This document is already assigned.",
                        )
                    else -> {
                        try {
                            val assignment =
                                documentService.assignToHousehold(
                                    organizationId = organizationId,
                                    householdId = householdId,
                                    assetId = assetId,
                                    title = title,
    ...
    private fun List<BulkActionItemResult>.toBulkResult() =
        BulkActionResult(
            requestedCount = size,
