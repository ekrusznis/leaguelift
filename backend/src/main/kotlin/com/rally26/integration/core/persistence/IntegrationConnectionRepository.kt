...
import com.rally26.integration.core.domain.IntegrationHealthCheck
import com.rally26.integration.core.domain.IntegrationHealthStatus
...
class IntegrationConnectionRepository(
    ...
    fun listForUser(userId: UUID): List<IntegrationConnection> =
        jdbcClient
            .sql("select $COLUMNS from integration_connection where user_id = :userId order by updated_at desc")
            .param("userId", userId)
            .query(::mapRow)
            .list()
    ...
    fun findById(id: UUID): IntegrationConnection? =
        jdbcClient
            .sql("select $COLUMNS from integration_connection where id = :id")
            .param("id", id)
            .query(::mapRow)
            .optional()
            .orElse(null)

    fun findByIdForOrganization(
        id: UUID,
        organizationId: UUID,
    ): IntegrationConnection? =
        jdbcClient
            .sql("select $COLUMNS from integration_connection where id = :id and organization_id = :organizationId")
            .param("id", id)
            .param("organizationId", organizationId)
            .query(::mapRow)
            .optional()
    ...
    fun findByIdForUser(
        ...
        jdbcClient
            .sql("select $COLUMNS from integration_connection where id = :id and user_id = :userId")
            .param("id", id)
            .param("userId", userId)
            .query(::mapRow)
            .optional()
    ...
    private companion object {
        const val COLUMNS =
            "id, provider, category, owner_type, organization_id, user_id, auth_mode, status, granted_scopes, external_account_id, " +
                "external_account_name, credential_id, access_token_expires_at, refresh_locked_at, refresh_locked_by_user_id, " +
                "last_successful_sync_at, last_health_check_at, last_error_code, last_error_message, legacy_resource_type, " +
