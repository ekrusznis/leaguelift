...
@Repository
class EventSourceConnectionRepository(
    ...
    fun findById(
        ...
        jdbcClient
            .sql("select $COLS from event_source_connection where id = :id and organization_id = :organizationId")
            .param("id", id)
    ...
    fun recordSyncResult(
        id: UUID,
        syncStatus: EventSourceSyncStatus,
        error: String?,
    ): Int {
        val now = Instant.now()
        return jdbcClient
            ...
            .param("syncStatus", syncStatus.name)
            .param("error", error)
            .param("id", id)
            .update()
    }
