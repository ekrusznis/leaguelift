...
@Repository
class FeePaymentPlanRepository(
    private val jdbcClient: JdbcClient,
) {
    fun findLatestByAssignment(
        organizationId: UUID,
    ...
    fun findByIdForUpdate(
        ...
        jdbcClient
            .sql("select $PLAN_COLUMNS from fee_payment_plan where organization_id = :organizationId and id = :planId for update")
            .param("organizationId", organizationId)
            .param("planId", planId)
            .query(::mapPlan)
            .optional()
    ...
    fun refreshStatus(
        ...
        val status =
            if (plan.status == FeePaymentPlanStatus.CANCELLED) {
                FeePaymentPlanStatus.CANCELLED
            } else if (paid >= plan.totalMinor) {
                FeePaymentPlanStatus.COMPLETED
            } else {
                FeePaymentPlanStatus.ACTIVE
            }
        if (status != plan.status) {
            jdbcClient
                .sql(
                    "update fee_payment_plan set status = :status, updated_at = :now where id = :planId and organization_id = :organizationId",
