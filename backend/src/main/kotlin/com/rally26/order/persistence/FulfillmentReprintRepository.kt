...
private const val REPRINT_COLUMNS =
    "id, organization_id, fulfillment_id, order_id, status, reason, vendor_order_reference, carrier, tracking_number, tracking_url, " +
        "internal_notes, requested_by_user_id, created_at, updated_at, shipped_at, delivered_at"

@Repository
class FulfillmentReprintRepository(
    ...
    fun listByOrder(orderId: UUID): List<FulfillmentReprint> =
        jdbcClient
            .sql(
    ...
    fun findOpenByFulfillment(fulfillmentId: UUID): FulfillmentReprint? =
        jdbcClient
            .sql(
