...
class FulfillmentRepository(private val jdbcClient: JdbcClient) {
	fun findByOrder(orderId: UUID): Fulfillment? = jdbcClient.sql(
		"""
	...
	fun countExceptionsByOrganization(organizationId: UUID): Long = jdbcClient.sql(
		"""
		select count(*)
		from fulfillment f
	...
	private fun mapRow(rs: java.sql.ResultSet, _rowNum: Int): Fulfillment = Fulfillment(
