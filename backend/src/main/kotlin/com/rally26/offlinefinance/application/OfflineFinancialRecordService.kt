...
data class OfflineOrderLineItem(
...
class OfflineFinancialRecordService(
    ...
    fun createContribution(
        ...
        val contribution =
            contributionRepository.insertOfflinePending(
                organizationId,
                campaign.id,
                amountMinor,
                campaign.currency,
    ...
    fun createSponsorship(
        ...
        val sponsorship =
            sponsorshipRepository.insertOfflinePending(
                organizationId,
                sponsorshipPackage.id,
                sponsor.id,
                sponsorshipPackage.priceMinor,
    ...
    fun createOrder(
        ...
        validateReceivedAt(receivedAt)
        if (items.isEmpty() ||
            items.size > MAX_ORDER_LINES
        ) {
            throw ValidationException("An offline order must contain 1 to $MAX_ORDER_LINES line items.")
        }
        if (items.any {
                it.quantity !in 1..MAX_ORDER_QUANTITY
            }
        ) {
            throw ValidationException("Each order quantity must be between 1 and $MAX_ORDER_QUANTITY.")
        }
        val store =
        ...
        val resolved =
            items.map { line ->
                val variant =
                    productVariantRepository.findById(line.productVariantId, organizationId)
    ...
    fun verify(
        ...
        membershipService.requireManagerRole(organizationId, currentUser)
        val record =
            repository.findByIdForUpdate(recordId, organizationId)
                ?: throw NotFoundException("OFFLINE_FINANCIAL_RECORD_NOT_FOUND", "The offline financial record could not be found.")
        return verifyInternal(record, currentUser)
    }

    private fun verifyInternal(
        record: OfflineFinancialRecord,
        currentUser: CurrentUser,
    ): OfflineFinancialRecord {
        ...
        when (record.recordType) {
            OfflineFinancialRecordType.CONTRIBUTION -> {
                val contribution =
                    contributionRepository.findById(record.recordId)
                ...
                if (contribution.status !=
                    ...
                    throw ValidationException("Only a pending offline contribution can be verified.")
                }
            ...
            OfflineFinancialRecordType.SPONSORSHIP -> {
                val sponsorship =
                    sponsorshipRepository.findById(record.recordId)
                ...
                if (sponsorship.status !=
                    ...
                    throw ValidationException("Only a pending offline sponsorship can be verified.")
                }
                val sponsorshipPackage =
                    sponsorshipPackageRepository.findById(sponsorship.packageId, record.organizationId)
                        ?: throw NotFoundException("SPONSORSHIP_PACKAGE_NOT_FOUND", "The sponsorship package could not be found.")
                ensureSponsorshipCapacity(sponsorshipPackage.id, sponsorshipPackage.effectiveMaxQuantity())
            ...
            OfflineFinancialRecordType.ORDER -> {
                val order =
                    orderRepository.findById(record.recordId, record.organizationId)
                ...
                ledgerService.recordOfflineOrder(
                    order.copy(status = OrderStatus.CONFIRMED, confirmedAt = record.receivedAt),
                    items,
                    record.paymentReference,
                )
                if (fulfillmentRepository.findByOrder(order.id) == null) {
                    val vendorIds =
                        items
                            .map { item ->
                                val variant =
                                    productVariantRepository.findById(item.productVariantId, order.organizationId)
                                        ?: error("order_item ${item.id} references a missing product_variant")
                                val product =
                                    productRepository.findById(variant.productId, order.organizationId)
                    ...
                    fulfillmentHistoryRepository.insert(
                        ...
                        "Manual fulfillment created when the offline order was verified.",
                        currentUser.userId,
                    )
                }
            }
        ...
        auditService.record(
            ...
            "offline_financial_record.verified",
    ...
    private fun insertRecord(
        ...
        auditService.record(
            currentUser.userId,
            organizationId,
            "offline_financial_record.created",
            "offline_financial_record",
            record.id,
    ...
    private fun findIdempotentOrDuplicate(
        organizationId: UUID,
        idempotencyKey: String,
        fingerprint: String,
    ): OfflineFinancialRecord? {
        ...
        if (byKey != null) {
            if (byKey.duplicateFingerprint != fingerprint) {
                throw ConflictException(
                    ...
                    "This idempotency key was already used for a different offline transaction.",
                )
            }
            return byKey
        }
    ...
    private fun ensureSponsorshipCapacity(
        packageId: UUID,
        maxQuantity: Int?,
    ) {
