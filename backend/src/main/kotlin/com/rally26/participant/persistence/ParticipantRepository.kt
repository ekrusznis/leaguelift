package com.rally26.participant.persistence
...
class ParticipantRepository(
    ...
    fun findById(
        ...
        jdbcClient
            .sql("select $P_COLS from participant where id = :id and organization_id = :organizationId")
            .param("id", id)
            .param("organizationId", organizationId)
            .query(::mapParticipant)
            .optional()
            .orElse(null)

    fun countActiveForOrganization(organizationId: UUID): Long =
        jdbcClient
            .sql("select count(*) from participant where organization_id = :organizationId and status = 'ACTIVE'")
            .param("organizationId", organizationId)
    ...
    fun countAllForOrganization(organizationId: UUID): Long =
        jdbcClient
            .sql("select count(*) from participant where organization_id = :organizationId and status != 'ARCHIVED'")
            .param("organizationId", organizationId)
            .query(Long::class.java)
            .single()

    fun countActiveForTeam(
        teamId: UUID,
        organizationId: UUID,
    ): Long =
        jdbcClient
