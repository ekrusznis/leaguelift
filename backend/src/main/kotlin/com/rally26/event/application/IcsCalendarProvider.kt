...
class IcsCalendarProvider : CalendarProvider {
    override fun buildIcs(
        event: Event,
        displayTitle: String,
    ): String = wrap(listOf(EventWithDisplayTitle(event, displayTitle)))
    ...
    private fun wrap(events: List<EventWithDisplayTitle>): String {
        val lines = mutableListOf("BEGIN:VCALENDAR", "VERSION:2.0", "PRODID:-//Rally26//Events//EN", "CALSCALE:GREGORIAN")
    ...
    private fun icsStatus(status: EventStatus): String =
        when (status) {
