...
class EventService(
    ...
    fun listForHousehold(
        ...
        return eventRepository
            .findByTeams(teamIds, organizationId, offset, limit)
     ...
     */
    private fun notifyMeaningfulChanges(
        before: Event,
        after: Event,
    ) {
    ...
    private fun writeChangeNotification(
        event: Event,
        eventType: String,
            ...
            payloadJson =
                objectMapper.writeValueAsString(
                    EventChangeNotificationPayload(
                        event.id,
                        displayTitleFor(event, event.organizationId),
                        changeSummary,
                        resolveFamilyRecipients(event),
                    ),
                ),
        )
    }
    ...
    private fun hasFamilyEventAccess(
        organizationId: UUID,
        teamId: UUID,
        currentUser: CurrentUser,
    ): Boolean =
