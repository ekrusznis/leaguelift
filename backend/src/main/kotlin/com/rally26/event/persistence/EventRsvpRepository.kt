...
@Repository
class EventRsvpRepository(private val jdbcClient: JdbcClient) {

	fun findByEventAndParticipant(eventId: UUID, participantId: UUID): EventRsvp? =
		jdbcClient.sql("select $COLUMNS from event_rsvp where event_id = :eventId and participant_id = :participantId")
			.param("eventId", eventId).param("participantId", participantId)
			.query(::mapRow).optional().orElse(null)

	fun findByEvent(eventId: UUID): List<EventRsvp> =
		jdbcClient.sql("select $COLUMNS from event_rsvp where event_id = :eventId")
	 ...
	 */
	fun upsert(eventId: UUID, participantId: UUID, response: RsvpResponse, note: String?, respondedByUserId: UUID, source: RsvpSource): EventRsvp {
		...
		jdbcClient.sql(
			...
			.param("id", id).param("eventId", eventId).param("participantId", participantId)
			.param("response", response.name).param("note", note).param("respondedByUserId", respondedByUserId)
			.param("source", source.name).param("now", java.sql.Timestamp.from(now))
			.update()
		return findByEventAndParticipant(eventId, participantId)!!
	}

	private fun mapRow(rs: java.sql.ResultSet, rowNum: Int): EventRsvp =
		EventRsvp(
