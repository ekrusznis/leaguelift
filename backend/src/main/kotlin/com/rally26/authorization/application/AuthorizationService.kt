...
class AuthorizationService(
    ...
    fun listContexts(currentUser: CurrentUser): List<AuthorizationContext> {
        ...
        val inheritedTeamOrgIds =
            memberships
                .filter {
                    it.role == MembershipRole.OWNER || it.role == MembershipRole.ADMINISTRATOR
        ...
        val tournamentAssignments =
            roleAssignmentRepository.findActiveForUserAndContext(
                currentUser.userId,
        ...
        for (assignment in roleAssignmentRepository.findActiveForUserAndContext(
            currentUser.userId,
    ...
    fun hasTeamCapability(
        ...
        val assignment =
            roleAssignmentRepository.findActiveForResource(currentUser.userId, RoleAssignmentContextType.TEAM, teamId) ?: return false
        return capability in CapabilityRegistry.teamCapabilities(assignment.role)
    }

    fun requireTeamCapability(
        organizationId: UUID,
        teamId: UUID,
        currentUser: CurrentUser,
        capability: String,
    ...
    fun listAccessibleTeamIds(
        ...
        return roleAssignmentRepository
            .findActiveForUserAndContext(currentUser.userId, RoleAssignmentContextType.TEAM)
    ...
    fun listTeamStaffUserIds(
        ...
        val explicit =
            roleAssignmentRepository
                .listActiveForResource(RoleAssignmentContextType.TEAM, teamId)
                .filter { capability in CapabilityRegistry.teamCapabilities(it.role) }
                .map { it.userId }
                .toSet()
        val inherited =
            if (capability in CapabilityRegistry.teamCapabilities(ResourceRole.TEAM_MANAGER)) {
    ...
    fun hasTournamentCapability(
        ...
        val assignment =
            roleAssignmentRepository.findActiveForResource(currentUser.userId, RoleAssignmentContextType.TOURNAMENT, tournamentId)
                ?: return false
        return capability in CapabilityRegistry.tournamentCapabilities(assignment.role)
    }

    fun requireTournamentCapability(
        organizationId: UUID,
        tournamentId: UUID,
        currentUser: CurrentUser,
        capability: String,
    ...
    fun listAccessibleTournamentIds(
        ...
        if (currentUser.platformAdministrator || organizationOwnerOrAdmin(organizationId, currentUser)) {
            if (currentUser.platformAdministrator ||
                capability in CapabilityRegistry.tournamentCapabilities(ResourceRole.TOURNAMENT_ADMINISTRATOR)
            ) {
    ...
    fun hasGuardianAdultRelationship(
        organizationId: UUID,
    ...
    fun hasParticipantCapability(
        ...
        if (currentUser.platformAdministrator) return true
        val assignment =
            roleAssignmentRepository.findActiveForResource(currentUser.userId, RoleAssignmentContextType.PARTICIPANT, participantId)
                ?: return false
    ...
    fun grantTeamRole(
        ...
        requireNotNull(teamRepository.findById(teamId, organizationId)) { "Team not found in this organization." }
        return roleAssignmentRepository.grant(
            organizationId,
            targetUserId,
    ...
    fun linkAthleteSelf(
        ...
        val isOrgManager =
            membershipRepository
                .findActiveMembership(organizationId, grantedByUserId)
                ?.role
                ?.let { it == MembershipRole.OWNER || it == MembershipRole.ADMINISTRATOR } == true
        require(
            isGuardian || isOrgManager,
        ) { "Only a guardian of this household or an organization manager can link an athlete self-login." }
        return roleAssignmentRepository.grant(
            organizationId,
            athleteUserId,
            RoleAssignmentContextType.PARTICIPANT,
    ...
    private fun organizationOwnerOrAdmin(
        organizationId: UUID,
        currentUser: CurrentUser,
    ): Boolean {
