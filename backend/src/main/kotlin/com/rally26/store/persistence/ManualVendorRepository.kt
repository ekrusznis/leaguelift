...
private const val VENDOR_COLUMNS = "id, organization_id, name, contact_name, contact_email, phone, website_url, notes, status, created_at, updated_at"

@Repository
class ManualVendorRepository(private val jdbcClient: JdbcClient) {
	fun findById(id: UUID, organizationId: UUID): ManualVendor? =
		jdbcClient.sql("select $VENDOR_COLUMNS from manual_vendor where id = :id and organization_id = :organizationId")
			.param("id", id)
	...
	fun findActiveByName(organizationId: UUID, name: String): ManualVendor? =
		jdbcClient.sql("select $VENDOR_COLUMNS from manual_vendor where organization_id = :organizationId and lower(name) = lower(:name) and status = 'ACTIVE'")
			.param("organizationId", organizationId)
			.param("name", name)
			.query(::mapRow)
			.optional()
	...
	fun list(organizationId: UUID, includeArchived: Boolean): List<ManualVendor> {
	...
	fun insert(
		...
		jdbcClient.sql(
			...
			.update()
		return ManualVendor(id, organizationId, name, contactName, contactEmail, phone, websiteUrl, notes, ManualVendorStatus.ACTIVE, now, now)
	}
	...
	fun archive(id: UUID, organizationId: UUID): Int = jdbcClient.sql(
	...
	private fun mapRow(rs: java.sql.ResultSet, _rowNum: Int): ManualVendor = ManualVendor(
