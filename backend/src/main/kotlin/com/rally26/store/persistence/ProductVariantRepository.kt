...
private const val VARIANT_COLUMNS = "id, organization_id, product_id, catalog_source, label, sku, size, color, printify_print_provider_id, printify_variant_id, currency, cost_minor, price_minor, is_active, created_at, updated_at"

@Repository
class ProductVariantRepository(private val jdbcClient: JdbcClient) {
	fun findById(id: UUID, organizationId: UUID): ProductVariant? =
		jdbcClient.sql("select $VARIANT_COLUMNS from product_variant where id = :id and organization_id = :organizationId")
			.param("id", id)
	...
	fun findActiveByProduct(productId: UUID): List<ProductVariant> =
		jdbcClient.sql("select $VARIANT_COLUMNS from product_variant where product_id = :productId and is_active = true order by created_at asc")
			.param("productId", productId)
			.query(::mapRow)
			.list()

	fun findByProduct(productId: UUID): List<ProductVariant> =
		jdbcClient.sql("select $VARIANT_COLUMNS from product_variant where product_id = :productId order by created_at asc")
			.param("productId", productId)
			.query(::mapRow)
			.list()
	...
	fun insertPrintify(
		...
		organizationId, productId, CatalogSource.PRINTIFY, label, null, null, null,
	...
	fun insertManual(
		...
		organizationId, productId, CatalogSource.MANUAL, label, sku, size, color,
		null, null, currency, costMinor, priceMinor,
	)
	...
	fun updateActive(id: UUID, organizationId: UUID, isActive: Boolean): Int = jdbcClient.sql(
	...
	private fun mapRow(rs: java.sql.ResultSet, _rowNum: Int): ProductVariant = ProductVariant(
