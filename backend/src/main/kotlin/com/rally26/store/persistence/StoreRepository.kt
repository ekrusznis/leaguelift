...
class StoreRepository(
    ...
    fun findById(
        ...
        jdbcClient
            .sql("select $COLUMNS from store where id = :id and organization_id = :organizationId")
            .param("id", id)
    ...
    fun findBySlug(slug: String): Store? =
        jdbcClient
            .sql("select $COLUMNS from store where lower(slug) = lower(:slug)")
            .param("slug", slug)
            .query(::mapRow)
            .optional()
            .orElse(null)
    ...
    fun countAll(organizationId: UUID): Long =
        jdbcClient
            .sql("select count(*) from store where organization_id = :organizationId")
            .param("organizationId", organizationId)
    ...
    fun updateStatus(
        ...
        val now = Instant.now()
        return jdbcClient
            .sql("update store set status = :status, updated_at = :now where id = :id and organization_id = :organizationId")
