...
class OutboxEventRepository(private val jdbcClient: JdbcClient) {
	...
	fun markProcessed(id: UUID) {
		jdbcClient.sql("update outbox_event set status = 'PROCESSED', processed_at = now() where id = :id")
			.param("id", id)
			.update()
	}

	fun markFailed(id: UUID, availableAt: Instant, lastError: String) {
		jdbcClient.sql(
	...
	fun markDeadLetter(id: UUID, lastError: String) {
		jdbcClient.sql("update outbox_event set status = 'DEAD_LETTER', last_error = :lastError where id = :id")
			.param("id", id)
			.param("lastError", lastError.take(2000))
			.update()
	}
	...
	fun findByStatus(status: String, limit: Int): List<OutboxEvent> =
		jdbcClient.sql("select $COLUMNS from outbox_event where status = :status order by created_at desc limit :limit")
			.param("status", status)
			.param("limit", limit)
			.query(::mapRow)
			.list()
	...
	private fun mapRow(rs: ResultSet, rowNum: Int): OutboxEvent =
		OutboxEvent(
