...
import com.leaguelift.tournament.domain.TournamentStatus
...
class MediaEntityAccessService(
	...
	fun requireAllowedSlot(target: ResolvedMediaTarget, usageSlot: MediaUsageSlot) {
	...
	private fun resolve(
		...
		currentUser: CurrentUser,
		manage: Boolean,
	): ResolvedMediaTarget = when (entityType) {
		MediaEntityType.ORGANIZATION -> {
			if (entityId != organizationId) {
				throw NotFoundException("MEDIA_TARGET_NOT_FOUND", "The media target could not be found.")
			}
			if (manage) membershipService.requireManagerRole(organizationId, currentUser)
			else membershipService.requireActiveMembership(organizationId, currentUser)
			ResolvedMediaTarget(
				organizationId,
				entityType,
				entityId,
		...
		MediaEntityType.TEAM -> {
			val team = teamRepository.findById(entityId, organizationId)
		...
		MediaEntityType.TOURNAMENT -> {
			val tournament = tournamentRepository.findById(entityId, organizationId)
				?: throw NotFoundException("TOURNAMENT_NOT_FOUND", "The tournament could not be found.")
			if (manage) {
				if (tournament.status != TournamentStatus.ACTIVE) throw ValidationException("An archived tournament cannot be rebranded.")
		...
		MediaEntityType.HOUSEHOLD_ADULT -> {
			val adult = householdRepository.findAdultById(entityId, organizationId)
			...
			if (manage) requireAdultProfileManageAccess(organizationId, adult.id, currentUser)
			else requireHouseholdProfileAccess(organizationId, adult.householdId, currentUser)
			ResolvedMediaTarget(
				organizationId,
				entityType,
				entityId,
		...
		MediaEntityType.PARTICIPANT -> {
			val participant = participantRepository.findById(entityId, organizationId)
			...
			val householdAccess = membershipService.hasManagerRole(organizationId, currentUser) ||
	...
	private fun requireAdultProfileManageAccess(organizationId: UUID, adultId: UUID, currentUser: CurrentUser) {
	...
	private fun requireHouseholdProfileAccess(organizationId: UUID, householdId: UUID, currentUser: CurrentUser) {
		if (membershipService.hasManagerRole(organizationId, currentUser)) return
		if (authorizationService.hasGuardianRelationship(organizationId, householdId, currentUser)) return
		throw ForbiddenException("PROFILE_PHOTO_ACCESS_DENIED", "You do not have access to this profile photo.")
	}

	private fun publishedPageVisibility(entityId: UUID, expectedType: PageType, privateVisibility: Visibility): Visibility {
