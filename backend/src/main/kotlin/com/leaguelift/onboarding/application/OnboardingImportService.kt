...
import com.leaguelift.onboarding.domain.ImportedOnboardingEntity
import com.leaguelift.onboarding.domain.OnboardingImportPreview
import com.leaguelift.onboarding.domain.OnboardingImportResult
import com.leaguelift.onboarding.domain.OnboardingImportRowError
import com.leaguelift.onboarding.domain.OnboardingPreviewAction
import com.leaguelift.onboarding.domain.OnboardingPreviewRow
import com.leaguelift.onboarding.domain.OnboardingRecordType
import com.leaguelift.onboarding.persistence.OnboardingImportIdentityRepository
...
class OnboardingImportService(
	...
	fun execute(
		...
		for (recordType in processingOrder) {
			for (row in validRows.filter { it.recordType == recordType }) {
				...
				val identity = identityRepository.insert(
				...
				imported += ImportedOnboardingEntity(
	...
	private fun analyze(organizationId: UUID, fileName: String?, csvContent: String): Analysis {
		...
		val header = parsed.first().mapIndexed { index, value ->
			value.trim().removePrefix(if (index == 0) "\uFEFF" else "").lowercase()
		}
		val duplicateHeaders = header.groupingBy { it }.eachCount().filterValues { it > 1 }.keys
		...
		val rows = parsed.drop(1).mapIndexedNotNull { index, raw ->
	...
	private fun parseIdentity(row: AnalysisRow) {
		val rawType = row.fields["record_type"].orEmpty().trim().uppercase()
		row.recordType = when (rawType) {
    ...
    private fun validateReferences(
        ...
        fun requireReference(
            ...
            val identity = identityRepository.find(
                organizationId,
                targetType,
                reference,
            )
            ...
            when {
                ...
                !entityExists(
                    ...
                    row.errors +=
                        "$field \"$reference\" points to a missing or inactive $targetType record. Contact LeagueLift support before importing."
                }
            }
        }
	...
	private fun validatePreviewDependencies(
		...
		fun requireValidFileReference(field: String, targetType: OnboardingRecordType) {
	...
	private fun resolvePreviewAction(
		...
		if (identity != null) {
			if (!entityExists(organizationId, recordType, identity.entityId)) {
				row.errors += "The saved external-id mapping points to a missing or inactive record. Contact LeagueLift support before importing."
				return
			}
		...
		when (recordType) {
			OnboardingRecordType.TEAM -> resolveSimpleNaturalMatch(
				organizationId,
				row,
				teamRepository.findNameMatches(organizationId, row.required("name")).map { it.id },
			)
			OnboardingRecordType.HOUSEHOLD -> resolveSimpleNaturalMatch(
				organizationId,
				row,
	...
	private fun resolveSimpleNaturalMatch(organizationId: UUID, row: AnalysisRow, naturalMatches: List<UUID>) {
		when {
			naturalMatches.size > 1 -> {
				row.errors += "More than one existing ${row.recordType!!.name.lowercase()} matches this row. Add or reuse a previously imported external_id instead of choosing automatically."
				row.action = OnboardingPreviewAction.ERROR
			}
			naturalMatches.size == 1 -> {
				...
				if (existingBinding != null) {
					row.errors += "This existing ${row.recordType!!.name.lowercase()} is already bound to external_id \"${existingBinding.externalId}\". Reuse that external_id instead of creating an alias."
					row.action = OnboardingPreviewAction.ERROR
				} else {
					row.existingEntityId = entityId
					row.action = OnboardingPreviewAction.MATCH_EXISTING
					row.warnings += "Matched an existing ${row.recordType!!.name.lowercase()} and will only bind this external_id; existing fields will not be overwritten."
				}
			}
	...
	private fun resolveGuardianNaturalMatch(
		...
		when {
			matches.size > 1 -> {
				row.errors += "More than one existing guardian uses this email. Review and merge the duplicate adult records before importing."
				row.action = OnboardingPreviewAction.ERROR
			}
			matches.size == 1 -> {
				val targetHouseholdId = resolveExistingReferenceId(
					organizationId,
					OnboardingRecordType.HOUSEHOLD,
				...
				if (targetHouseholdId == null || match.householdId != targetHouseholdId) {
					row.errors += "A guardian with this email already belongs to another household. Use the explicit account/profile review workflow instead of creating or moving the adult silently."
					row.action = OnboardingPreviewAction.ERROR
				} else {
					val existingBinding = identityRepository.findByEntity(organizationId, OnboardingRecordType.GUARDIAN, match.id)
					if (existingBinding != null) {
						row.errors += "This guardian is already bound to external_id \"${existingBinding.externalId}\". Reuse that external_id instead of creating an alias."
						row.action = OnboardingPreviewAction.ERROR
					} else {
						row.existingEntityId = match.id
						row.action = OnboardingPreviewAction.MATCH_EXISTING
						row.warnings += "Matched an existing guardian in the selected household and will only bind this external_id; existing fields will not be overwritten."
					}
				}
			}
			else -> row.action = OnboardingPreviewAction.CREATE
	...
	private fun validateExistingRelationship(
		...
		val targetHouseholdExternalId = when (row.recordType) {
		...
		val actualHouseholdId = when (row.recordType) {
		...
		if (targetHouseholdId == null || actualHouseholdId != targetHouseholdId) {
			row.errors += "The imported ${row.recordType!!.name.lowercase()} is already linked to a different household and cannot be moved by CSV import."
		}
	}
	...
	private fun upsertTeam(organizationId: UUID, row: AnalysisRow): UUID {
		...
		val existing = teamRepository.findById(existingId, organizationId)
	...
	private fun upsertHousehold(organizationId: UUID, row: AnalysisRow): UUID {
		...
		val existing = householdRepository.findById(existingId, organizationId)
	...
	private fun upsertGuardian(
		...
		val householdId = resolveReference(
			organizationId,
			OnboardingRecordType.HOUSEHOLD,
			row.requiredExternalReference("household_external_id"),
			resolved,
		...
		val existing = householdRepository.findAdultById(existingId, organizationId)
	...
	private data class ParticipantUpsertResult(val entityId: UUID, val assignmentCreated: Boolean)
	...
	private fun upsertParticipant(
		...
		val assignmentCreated = if (teamId != null) {
	...
	private fun resolveReference(
		...
		val entityId = resolved[recordType to externalId]
			?: identityRepository.find(organizationId, recordType, externalId)?.entityId
	...
	private fun entityExists(organizationId: UUID, type: OnboardingRecordType, entityId: UUID): Boolean = when (type) {
	...
	private fun parseBoolean(value: String?, field: String, row: AnalysisRow): Boolean? {
	...
	private fun parseDate(value: String?, field: String, row: AnalysisRow): LocalDate? {
	...
	private fun labelFor(row: AnalysisRow): String = when (row.recordType) {
	...
	private fun normalizeExternalId(value: String?): String? =
		value?.trim()?.lowercase()?.takeIf { it.isNotBlank() }

	private fun normalizedEmail(value: String?): String? =
		value?.trim()?.lowercase()?.takeIf { it.isNotBlank() }
	...
	private data class Analysis(val fileName: String?, val contentHash: String, val rows: List<AnalysisRow>) {
		fun toPreview(): OnboardingImportPreview {
			val previewRows = rows.map {
