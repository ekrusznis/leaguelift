package com.leaguelift.onboarding.persistence
...
@Repository
class OnboardingImportIdentityRepository(private val jdbcClient: JdbcClient) {

	fun find(organizationId: UUID, entityType: OnboardingRecordType, externalId: String): OnboardingImportIdentity? =
		jdbcClient.sql(
			"""
	...
	fun findByEntity(organizationId: UUID, entityType: OnboardingRecordType, entityId: UUID): OnboardingImportIdentity? =
		jdbcClient.sql(
			"""
			select id, organization_id, entity_type, external_id, entity_id, created_by_user_id, created_at
			from onboarding_import_identity
	...
	private fun mapRow(rs: java.sql.ResultSet, rowNum: Int) = OnboardingImportIdentity(
