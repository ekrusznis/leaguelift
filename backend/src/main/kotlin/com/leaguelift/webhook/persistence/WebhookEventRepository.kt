package com.leaguelift.webhook.persistence
...
private const val COLUMNS = "id, provider, external_event_id, event_type, payload, payload_hash, signature_verified, processing_status, attempt_count, related_entity_type, related_entity_id, received_at, processed_at, last_error"

@Repository
class WebhookEventRepository(private val jdbcClient: JdbcClient) {

	/** Null means this (provider, externalEventId) hasn't been recorded yet — safe to process. */
	fun findExisting(provider: String, externalEventId: String): WebhookEvent? =
		jdbcClient.sql("select $COLUMNS from webhook_event where provider = :provider and external_event_id = :externalEventId")
	...
	fun insert(
		...
		jdbcClient.sql(
			...
			.update()
		return WebhookEvent(
			id, provider, externalEventId, eventType, payload, payloadHash, signatureVerified,
			processingStatus, 1, relatedEntityType, relatedEntityId, receivedAt, processedAt, lastError,
		)
	}
