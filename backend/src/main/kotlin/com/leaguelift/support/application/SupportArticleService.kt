...
class SupportArticleService(
    ...
    fun listPublic(query: String?, category: String?) = repository.listPublished(setOf(SupportAudience.PUBLIC), normalizeSearch(query), normalizeCategoryFilter(category))

    fun getPublic(slug: String): SupportArticle = repository.findPublishedBySlug(slug.trim(), setOf(SupportAudience.PUBLIC))
        ?: throw NotFoundException("SUPPORT_ARTICLE_NOT_FOUND", "The help article could not be found.")

    fun listAuthenticated(currentUser: CurrentUser, query: String?, category: String?) =
        repository.listPublished(audiencesFor(currentUser), normalizeSearch(query), normalizeCategoryFilter(category))

    fun getAuthenticated(currentUser: CurrentUser, slug: String): SupportArticle =
        repository.findPublishedBySlug(slug.trim(), audiencesFor(currentUser))
            ?: throw NotFoundException("SUPPORT_ARTICLE_NOT_FOUND", "The help article could not be found.")

    fun listPlatform(
        currentUser: CurrentUser, query: String?, status: SupportArticleStatus?, audience: SupportAudience?, category: String?, page: PageRequest,
    ...
    @Transactional
    fun create(
        currentUser: CurrentUser, slug: String, title: String, summary: String, bodyMarkdown: String,
        category: String, audience: SupportAudience, sortOrder: Int,
    ): SupportArticle {
        authorizationService.requirePlatformCapability(currentUser, Capabilities.PLATFORM_HELP_MANAGE)
        val input = normalize(slug, title, summary, bodyMarkdown, category, sortOrder)
        val article = try {
            repository.insert(input.slug, input.title, input.summary, input.body, input.category, audience, input.sortOrder, currentUser.userId)
    ...
    @Transactional
    fun update(
        currentUser: CurrentUser, articleId: UUID, slug: String, title: String, summary: String, bodyMarkdown: String,
        category: String, audience: SupportAudience, sortOrder: Int,
    ): SupportArticle {
        authorizationService.requirePlatformCapability(currentUser, Capabilities.PLATFORM_HELP_MANAGE)
        val existing = requireArticle(articleId)
        if (existing.status == SupportArticleStatus.ARCHIVED) throw ConflictException("SUPPORT_ARTICLE_ARCHIVED", "Archived help articles cannot be edited.")
        val input = normalize(slug, title, summary, bodyMarkdown, category, sortOrder)
        try {
            if (repository.update(articleId, input.slug, input.title, input.summary, input.body, input.category, audience, input.sortOrder, currentUser.userId) == 0) {
    ...
    @Transactional
    fun publish(currentUser: CurrentUser, articleId: UUID): SupportArticle {
        authorizationService.requirePlatformCapability(currentUser, Capabilities.PLATFORM_HELP_MANAGE)
        val existing = requireArticle(articleId)
        if (existing.status == SupportArticleStatus.ARCHIVED) throw ConflictException("SUPPORT_ARTICLE_ARCHIVED", "Archived help articles cannot be published.")
    ...
    @Transactional
    fun archive(currentUser: CurrentUser, articleId: UUID): SupportArticle {
        authorizationService.requirePlatformCapability(currentUser, Capabilities.PLATFORM_HELP_MANAGE)
        val existing = requireArticle(articleId)
    ...
    private fun audiencesFor(currentUser: CurrentUser): Set<SupportAudience> {
        ...
        for (context in authorizationService.listContexts(currentUser)) {
            when (context.contextType) {
                ...
                ContextType.ORGANIZATION -> when (context.role) {
    ...
    private fun requireArticle(id: UUID) = repository.findById(id)
        ?: throw NotFoundException("SUPPORT_ARTICLE_NOT_FOUND", "The help article could not be found.")

    private fun normalize(slug: String, title: String, summary: String, body: String, category: String, sortOrder: Int): Input {
        val normalizedSlug = slug.trim().lowercase()
        if (!normalizedSlug.matches(Regex("^[a-z0-9]([a-z0-9-]{0,118}[a-z0-9])?$"))) throw ValidationException("Slug must use lowercase letters, numbers, and hyphens.")
    ...
    private fun normalizeSearch(value: String?) = value?.trim()?.takeIf { it.isNotEmpty() }?.also {
        if (it.length > 120) throw ValidationException("Search must not exceed 120 characters.")
    }
    private fun normalizeCategoryFilter(value: String?) = value?.trim()?.takeIf { it.isNotEmpty() }?.also {
        if (it.length > 80) throw ValidationException("Category must not exceed 80 characters.")
    }

    private fun audit(currentUser: CurrentUser, article: SupportArticle, action: String) = auditService.record(
        currentUser.userId, null, action, "SUPPORT_ARTICLE", article.id,
        objectMapper.writeValueAsString(mapOf("slug" to article.slug, "status" to article.status.name, "audience" to article.audience.name)),
    )

    private data class Input(val slug: String, val title: String, val summary: String, val body: String, val category: String, val sortOrder: Int)
}
