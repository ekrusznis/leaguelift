package com.leaguelift.platformadmin.application
...
import com.leaguelift.platformadmin.persistence.PlatformAdminConsoleRepository
import com.leaguelift.platformadmin.persistence.PlatformSupportAccessRepository
...
/**
 * LeagueLift employee console. This is deliberately separate from an organization
...
class PlatformAdminConsoleService(
	...
	fun getOrganization(currentUser: CurrentUser, organizationId: UUID): PlatformOrganizationDetail {
		authorizationService.requirePlatformCapability(currentUser, Capabilities.PLATFORM_ORG_VIEW)
	...
	@Transactional
	fun startSupportAccess(currentUser: CurrentUser, organizationId: UUID, reason: String): PlatformSupportAccess {
		authorizationService.requirePlatformCapability(currentUser, Capabilities.PLATFORM_SUPPORT_ACCESS)
		...
		val organization = consoleRepository.findOrganization(organizationId)
			?: throw NotFoundException("ORGANIZATION_NOT_FOUND", "Organization not found.")
		...
		val access = supportAccessRepository.create(currentUser.userId, organizationId, normalizedReason, now, now.plus(SUPPORT_ACCESS_DURATION))
		recordAccessEvent(
			currentUser,
			access,
	...
	@Transactional
	fun endSupportAccess(currentUser: CurrentUser, accessId: UUID) {
		authorizationService.requirePlatformCapability(currentUser, Capabilities.PLATFORM_SUPPORT_ACCESS)
		val access = supportAccessRepository.findById(accessId)
	...
	@Transactional
	fun requireActiveSupportAccess(currentUser: CurrentUser, accessId: UUID, organizationId: UUID): PlatformSupportAccess {
		authorizationService.requirePlatformCapability(currentUser, Capabilities.PLATFORM_SUPPORT_ACCESS)
		val access = supportAccessRepository.findById(accessId)
	...
	private fun validateExpiration(currentUser: CurrentUser, access: PlatformSupportAccess): PlatformSupportAccess? {
		val now = Instant.now(clock)
	...
	private fun normalizeStatus(value: String?, allowed: Set<String>, label: String): String? {
