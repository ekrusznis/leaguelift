package com.leaguelift.ledger.persistence
...
@Repository
class LedgerEntryRepository(private val jdbcClient: JdbcClient) {
	...
	fun insert(
		...
		return LedgerEntry(
			id, organizationId, accountCode, entryType, direction, amountMinor, currency,
			sourceType, sourceId, externalReference, description, null, now, now,
		)
	}

	/** ORGANIZATION_EARNING credits not yet paid out, whose holding period has elapsed — the actually-transferable pool. */
	fun findEligibleOrganizationEarningCredits(organizationId: UUID, cutoff: Instant): List<LedgerEntry> =
		findUnclaimedOrganizationEarnings(organizationId, "CREDIT", "effective_at <= :cutoff", cutoff)
	...
	fun findHeldOrganizationEarningCredits(organizationId: UUID, cutoff: Instant): List<LedgerEntry> =
		findUnclaimedOrganizationEarnings(organizationId, "CREDIT", "effective_at > :cutoff", cutoff)
	...
	private fun findUnclaimedOrganizationEarnings(organizationId: UUID, direction: String, cutoffPredicate: String, cutoff: Instant): List<LedgerEntry> =
		jdbcClient.sql(
			"""
			select $COLUMNS from ledger_entry
	...
	private fun mapRow(rs: java.sql.ResultSet, rowNum: Int): LedgerEntry =
		LedgerEntry(
