package com.leaguelift.financialcorrection.persistence
...
@Repository
class FinancialCorrectionRepository(private val jdbcClient: JdbcClient) {
    /** Serializes corrections for one source record without inventing a mutable financial row. */
    fun lockTarget(organizationId: UUID, targetType: FinancialCorrectionTargetType, targetId: UUID) {
    ...
    fun findByIdempotencyKey(organizationId: UUID, idempotencyKey: String): FinancialCorrection? =
        jdbcClient.sql("select $COLUMNS from financial_correction where organization_id = :organizationId and idempotency_key = :key")
            .param("organizationId", organizationId).param("key", idempotencyKey)
            .query(::mapRow).optional().orElse(null)

    fun sumByTarget(organizationId: UUID, targetType: FinancialCorrectionTargetType, targetId: UUID): Long =
        jdbcClient.sql(
            ...
            .param("organizationId", organizationId).param("targetType", targetType.name).param("targetId", targetId)
    ...
    fun insert(
        ...
        jdbcClient.sql(
            ...
            .param("id", id).param("organizationId", organizationId).param("correctionType", correctionType.name)
            .param("targetType", targetType.name).param("targetId", targetId).param("amountMinor", amountMinor)
            .param("currency", currency).param("reason", reason).param("providerReference", providerReference)
            .param("confirmationHash", confirmationHash).param("idempotencyKey", idempotencyKey)
            .param("createdByUserId", createdByUserId).param("now", Timestamp.from(now)).update()
    ...
    fun list(organizationId: UUID, offset: Int, limit: Int): List<FinancialCorrection> =
        jdbcClient.sql(
    ...
    fun count(organizationId: UUID): Long =
        jdbcClient.sql("select count(*) from financial_correction where organization_id = :organizationId")
            .param("organizationId", organizationId).query(Long::class.java).single()

    private fun mapRow(rs: ResultSet, rowNum: Int) = FinancialCorrection(
