...
@Repository
class FeeAdjustmentRepository(private val jdbcClient: JdbcClient) {

	fun findById(id: UUID, organizationId: UUID): FeeAdjustment? =
		jdbcClient.sql("select $COLUMNS from fee_adjustment where id = :id and organization_id = :organizationId")
			.param("id", id).param("organizationId", organizationId)
			.query(::mapRow).optional().orElse(null)

	fun findAllByAssignment(feeAssignmentId: UUID, organizationId: UUID): List<FeeAdjustment> =
		jdbcClient.sql(
			"""
	...
	fun insert(
		...
		jdbcClient.sql(
			...
			.param("householdId", householdId).param("adjustmentType", adjustmentType.name)
			.param("amountMinor", amountMinor).param("currency", currency).param("reason", reason)
			.param("createdByUserId", createdByUserId).param("now", Timestamp.from(now))
			.update()
		return FeeAdjustment(
			id, organizationId, feeAssignmentId, householdId, adjustmentType, amountMinor, currency,
			reason, createdByUserId, voidedAt = null, voidedByUserId = null, voidReason = null, createdAt = now,
		)
	}
	...
	private fun mapRow(rs: ResultSet, rowNum: Int): FeeAdjustment =
		FeeAdjustment(
