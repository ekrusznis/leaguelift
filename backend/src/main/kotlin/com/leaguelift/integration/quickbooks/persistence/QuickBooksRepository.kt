package com.leaguelift.integration.quickbooks.persistence

import com.leaguelift.integration.quickbooks.domain.QuickBooksAccountMapping
import com.leaguelift.integration.quickbooks.domain.QuickBooksAccountingBasis
import com.leaguelift.integration.quickbooks.domain.QuickBooksConnectionSetting
import com.leaguelift.integration.quickbooks.domain.QuickBooksEnvironment
import com.leaguelift.integration.quickbooks.domain.QuickBooksExportBatch
import com.leaguelift.integration.quickbooks.domain.QuickBooksExportCandidateCounts
import com.leaguelift.integration.quickbooks.domain.QuickBooksExportPolicy
import com.leaguelift.integration.quickbooks.domain.QuickBooksExportStatus
import com.leaguelift.integration.quickbooks.domain.QuickBooksMappingType
...
@Repository
class QuickBooksRepository(private val jdbcClient: JdbcClient) {
    fun findSetting(connectionId: UUID): QuickBooksConnectionSetting? =
        jdbcClient.sql("select $SETTING_COLUMNS from quickbooks_connection_setting where connection_id = :connectionId")
            .param("connectionId", connectionId).query(::mapSetting).optional().orElse(null)
    ...
    fun listMappings(connectionId: UUID): List<QuickBooksAccountMapping> =
        jdbcClient.sql("select $MAPPING_COLUMNS from quickbooks_account_mapping where connection_id = :connectionId and active order by mapping_type")
    ...
    fun replaceMapping(
        ...
        jdbcClient.sql("update quickbooks_account_mapping set active = false, updated_at = now() where connection_id = :connectionId and mapping_type = :mappingType and active")
            .param("connectionId", connectionId).param("mappingType", mappingType.name).update()
        ...
        jdbcClient.sql(
            ...
            .param("userId", userId)
            .update()
        return jdbcClient.sql("select $MAPPING_COLUMNS from quickbooks_account_mapping where id = :id")
            .param("id", id).query(::mapMapping).single()
    }

    fun countExportCandidates(organizationId: UUID, start: LocalDate, end: LocalDate): QuickBooksExportCandidateCounts =
        jdbcClient.sql(
            """
            select
            ...
            .param("start", start)
            .param("end", end)
            .query { rs, _ -> QuickBooksExportCandidateCounts(rs.getInt("contributions"), rs.getInt("sponsorships"), rs.getInt("orders"), rs.getInt("fee_payments"), rs.getInt("corrections")) }
            .single()
    ...
    fun listBatches(organizationId: UUID, limit: Int = 20): List<QuickBooksExportBatch> =
        jdbcClient.sql("select $BATCH_COLUMNS from quickbooks_export_batch where organization_id = :organizationId order by created_at desc limit :limit")
            .param("organizationId", organizationId).param("limit", limit).query(::mapBatch).list()

    private fun findBatch(id: UUID): QuickBooksExportBatch? =
        jdbcClient.sql("select $BATCH_COLUMNS from quickbooks_export_batch where id = :id")
            .param("id", id).query(::mapBatch).optional().orElse(null)

    private fun findBatchByKey(organizationId: UUID, key: String): QuickBooksExportBatch? =
        jdbcClient.sql("select $BATCH_COLUMNS from quickbooks_export_batch where organization_id = :organizationId and idempotency_key = :key")
            .param("organizationId", organizationId).param("key", key).query(::mapBatch).optional().orElse(null)

    private fun mapSetting(rs: ResultSet, rowNum: Int) = QuickBooksConnectionSetting(
    ...
    private fun mapMapping(rs: ResultSet, rowNum: Int) = QuickBooksAccountMapping(
    ...
    private fun mapBatch(rs: ResultSet, rowNum: Int) = QuickBooksExportBatch(
        rs.getObject("id", UUID::class.java),
        rs.getObject("connection_id", UUID::class.java),
    ...
    private companion object {
        const val SETTING_COLUMNS = "connection_id, realm_id, company_name, environment, export_policy, accounting_basis, default_currency, last_company_read_at, last_accounts_read_at, created_at, updated_at"
        const val MAPPING_COLUMNS = "id, connection_id, mapping_type, external_account_id, external_account_name, external_account_type, active, configured_by_user_id, created_at, updated_at"
        const val BATCH_COLUMNS = "id, connection_id, organization_id, sync_run_id, status, period_start, period_end, candidate_count, exported_count, failed_count, requested_by_user_id, created_at, completed_at"
    }
}
