...
data class PrintifyProductVariantCost(val printifyVariantId: Long, val costMinor: Long, val priceMinor: Long)

data class PrintifyProductResult(val printifyProductId: String, val variantCosts: List<PrintifyProductVariantCost>)

private data class CreateProductVariantDto(val id: Long, val price: Long, val is_enabled: Boolean)
private data class PositionedImageDto(val id: String, val x: Double = 0.5, val y: Double = 0.5, val scale: Double = 1.0, val angle: Int = 0)
private data class PlaceholderDto(val position: String, val images: List<PositionedImageDto>)
private data class PrintAreaDto(val variant_ids: List<Long>, val placeholders: List<PlaceholderDto>)
...
private data class ProductVariantResponseDto(val id: Long, val price: Long, val cost: Long)
private data class CreateProductResponseDto(val id: String, val variants: List<ProductVariantResponseDto>)
 ...
 */
@Component
class PrintifyProductClient(private val printifyRestClient: RestClient, private val printifyProperties: PrintifyProperties) {
		...
		val request = CreateProductRequestDto(
			...
			print_areas = listOf(
				PrintAreaDto(
					...
					placeholders = listOf(PlaceholderDto(position = printPosition, images = listOf(PositionedImageDto(id = printifyImageId)))),
				),
			),
		)
