...
class IcsFeedSyncJob(
	...
	@Scheduled(cron = "\${leaguelift.ics-feed.sync.cron:0 */30 * * * *}")
	fun syncAll() {
	...
	fun syncOne(connection: EventSourceConnection) {
		...
		if (failed == 0) {
			...
			eventSourceConnectionRepository.recordSyncResult(connection.id, EventSourceSyncStatus.FAILED, "$failed of ${parsedEvents.size} events failed to sync")
		}
		log.info("Synced ICS_FEED connection {} ({}): {} created, {} touched, {} failed.", connection.id, connection.label, created, touched, failed)
	}

	private fun recordFailure(connection: EventSourceConnection, message: String) {
	...
	private fun upsert(connection: EventSourceConnection, parsed: ParsedIcsEvent): Boolean {
		...
		if (existing == null) {
			eventRepository.insert(
				organizationId = connection.organizationId, teamId = connection.teamId, tournamentId = null, opponentTeamId = null,
				opponentName = null, eventType = EventType.OTHER, title = parsed.summary, description = parsed.description,
				startAt = parsed.startAt, endAt = parsed.endAt, arrivalAt = null, meetingAt = null, timezone = connection.timezone,
				venueName = parsed.location, address = null, latitude = null, longitude = null, area = null, meetingPoint = null,
				directionsNotes = null, visibility = if (connection.teamId != null) EventVisibility.TEAM else EventVisibility.ORGANIZATION,
				createdByUserId = connection.createdByUserId, sourceType = EventSourceType.ICS_FEED, provider = PROVIDER,
				connectionId = connectionId, externalEventId = parsed.uid, externalSyncHash = syncHash, sourceUpdatedAt = now,
		...
		eventRepository.update(
			id = existing.id, organizationId = connection.organizationId, title = parsed.summary, description = parsed.description,
			status = status, startAt = parsed.startAt, endAt = parsed.endAt, arrivalAt = null, meetingAt = null,
			venueName = parsed.location, address = null, latitude = null, longitude = null, area = null, meetingPoint = null,
			directionsNotes = null, opponentTeamId = null, opponentName = null, updatedByUserId = connection.createdByUserId,
			externalSyncHash = syncHash, sourceUpdatedAt = now,
		)
		return false
	}
	...
	private fun mapStatus(rawStatus: String?): EventStatus? = when (rawStatus?.uppercase()) {
