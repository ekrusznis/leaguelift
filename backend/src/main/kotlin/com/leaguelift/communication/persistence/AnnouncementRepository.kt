package com.leaguelift.communication.persistence

import com.leaguelift.common.web.PageRequest
...
import com.leaguelift.communication.domain.AnnouncementRecipient
import com.leaguelift.communication.domain.AnnouncementRecipientCandidate
import com.leaguelift.communication.domain.AnnouncementRecipientType
import com.leaguelift.communication.domain.AnnouncementScopeType
import com.leaguelift.communication.domain.AnnouncementStatus
import com.leaguelift.communication.domain.DeliveryStatus
import com.leaguelift.communication.domain.MyAnnouncement
...
@Repository
class AnnouncementRepository(private val jdbcClient: JdbcClient) {
    fun findById(id: UUID, organizationId: UUID): Announcement? = jdbcClient.sql(
        "$ANNOUNCEMENT_SELECT where a.id = :id and a.organization_id = :organizationId",
    ).param("id", id).param("organizationId", organizationId).query(::mapAnnouncement).optional().orElse(null)

    fun findBySourceKey(organizationId: UUID, sourceKey: String): Announcement? = jdbcClient.sql(
        "$ANNOUNCEMENT_SELECT where a.organization_id = :organizationId and a.source_key = :sourceKey",
    ).param("organizationId", organizationId).param("sourceKey", sourceKey).query(::mapAnnouncement).optional().orElse(null)
    ...
    fun insert(
        ...
        jdbcClient.sql(
            ...
            .param("id", id).param("organizationId", organizationId).param("scopeType", scopeType.name)
            .param("scopeId", scopeId).param("kind", kind.name).param("relatedEntityType", relatedEntityType)
            .param("relatedEntityId", relatedEntityId).param("targetHouseholdId", targetHouseholdId)
            .param("sourceKey", sourceKey).param("title", title).param("body", body).param("audience", audience.name)
            .param("emailEnabled", emailEnabled).param("smsEnabled", smsEnabled).param("createdByUserId", createdByUserId)
            .param("now", Timestamp.from(now)).update()
        return findById(id, organizationId)!!
    }

    fun updateDraft(
        id: UUID, organizationId: UUID, title: String, body: String, audience: AnnouncementAudience,
        ...
        .param("title", title).param("body", body).param("audience", audience.name)
        .param("emailEnabled", emailEnabled).param("smsEnabled", smsEnabled)
        .param("id", id).param("organizationId", organizationId).update()

    fun publish(id: UUID, organizationId: UUID, userId: UUID, now: Instant): Int = jdbcClient.sql(
        """
        update announcement set status = 'PUBLISHED', published_by_user_id = :userId,
            published_at = :now, updated_at = :now
        where id = :id and organization_id = :organizationId and status = 'DRAFT'
        """.trimIndent(),
    ).param("userId", userId).param("now", Timestamp.from(now)).param("id", id).param("organizationId", organizationId).update()

    fun archive(id: UUID, organizationId: UUID, now: Instant): Int = jdbcClient.sql(
        """
    ...
    fun listForManagement(
    ...
    ): List<Announcement> = jdbcClient.sql(
        """
        $ANNOUNCEMENT_SELECT
        ...
        .param("organizationId", organizationId).param("scopeType", scopeType?.name).param("scopeId", scopeId)
        .param("status", status?.name).param("offset", page.offset).param("limit", page.size)
        .query(::mapAnnouncement).list()

    fun countForManagement(organizationId: UUID, scopeType: AnnouncementScopeType?, scopeId: UUID?, status: AnnouncementStatus?): Long =
        jdbcClient.sql(
            """
            ...
            .param("organizationId", organizationId).param("scopeType", scopeType?.name).param("scopeId", scopeId)
            .param("status", status?.name).query(Long::class.java).single()

    fun insertRecipient(
        announcementId: UUID, organizationId: UUID, recipientKey: String, candidate: AnnouncementRecipientCandidate,
        inAppVisible: Boolean, emailStatus: DeliveryStatus, smsStatus: DeliveryStatus,
    ) {
        jdbcClient.sql(
            """
            ...
            .param("id", UUID.randomUUID()).param("announcementId", announcementId).param("organizationId", organizationId)
            .param("recipientKey", recipientKey).param("recipientType", candidate.recipientType.name).param("userId", candidate.userId)
            .param("householdId", candidate.householdId).param("displayName", candidate.displayName).param("email", candidate.email)
            .param("phone", candidate.phone).param("inAppVisible", inAppVisible).param("emailStatus", emailStatus.name)
            .param("smsStatus", smsStatus.name).update()
    }

    fun listDeliveries(announcementId: UUID): List<AnnouncementRecipient> = jdbcClient.sql(
        """
        ...
        """.trimIndent(),
    ).param("announcementId", announcementId).query(::mapRecipient).list()

    fun markEmailSent(recipientId: UUID, now: Instant): Int = jdbcClient.sql(
        "update announcement_recipient set email_status = 'SENT', email_sent_at = :now, last_error = null, updated_at = :now where id = :id",
    ).param("now", Timestamp.from(now)).param("id", recipientId).update()

    fun markSmsSent(recipientId: UUID, now: Instant): Int = jdbcClient.sql(
        "update announcement_recipient set sms_status = 'SENT', sms_sent_at = :now, last_error = null, updated_at = :now where id = :id",
    ).param("now", Timestamp.from(now)).param("id", recipientId).update()

    fun markEmailFailed(recipientId: UUID, error: String): Int = jdbcClient.sql(
        "update announcement_recipient set email_status = 'FAILED', last_error = :error, updated_at = now() where id = :id",
    ).param("error", error.take(1000)).param("id", recipientId).update()

    fun markSmsFailed(recipientId: UUID, error: String): Int = jdbcClient.sql(
        "update announcement_recipient set sms_status = 'FAILED', last_error = :error, updated_at = now() where id = :id",
    ).param("error", error.take(1000)).param("id", recipientId).update()

    fun listMineSafe(userId: UUID, page: PageRequest): List<MyAnnouncement> = jdbcClient.sql(
        """
        ...
        .param("userId", userId).param("offset", page.offset).param("limit", page.size)
        .query { rs, _ -> MyAnnouncement(mapAnnouncement(rs, 0), rs.getObject("recipient_id", UUID::class.java), rs.getTimestamp("read_at")?.toInstant()) }
        .list()

    fun countMine(userId: UUID): Long = jdbcClient.sql(
    ...
    fun markRead(announcementId: UUID, userId: UUID, now: Instant): Int = jdbcClient.sql(
    ...
    ).param("now", Timestamp.from(now)).param("announcementId", announcementId).param("userId", userId).update()

    fun listOrganizationStaff(organizationId: UUID): List<AnnouncementRecipientCandidate> = jdbcClient.sql(
        """
        ...
        """.trimIndent(),
    ).param("organizationId", organizationId).query { rs, _ -> candidate(rs, AnnouncementRecipientType.STAFF) }.list()

    fun listTeamStaff(organizationId: UUID, teamId: UUID): List<AnnouncementRecipientCandidate> = jdbcClient.sql(
        """
        select distinct au.id as user_id, au.display_name, au.email
        from app_user au
        ...
        """.trimIndent(),
    ).param("organizationId", organizationId).param("teamId", teamId).query { rs, _ -> candidate(rs, AnnouncementRecipientType.STAFF) }.list()

    fun listTournamentStaff(organizationId: UUID, tournamentId: UUID): List<AnnouncementRecipientCandidate> = jdbcClient.sql(
        """
        select distinct au.id as user_id, au.display_name, au.email
        from app_user au
        where au.status = 'ACTIVE' and (
        ...
        """.trimIndent(),
    ).param("organizationId", organizationId).param("tournamentId", tournamentId).query { rs, _ -> candidate(rs, AnnouncementRecipientType.STAFF) }.list()
    ...
    fun listTeamGuardians(organizationId: UUID, teamId: UUID): List<AnnouncementRecipientCandidate> = guardiansQuery(organizationId, teamId, null)
    fun listHouseholdGuardians(organizationId: UUID, householdId: UUID): List<AnnouncementRecipientCandidate> = guardiansQuery(organizationId, null, householdId)
    fun listUnacknowledgedDocumentGuardians(
        organizationId: UUID, householdId: UUID, mediaAssignmentId: UUID,
    ): List<AnnouncementRecipientCandidate> = jdbcClient.sql(
        """
        ...
        .param("organizationId", organizationId).param("householdId", householdId)
    ...
    private fun guardiansQuery(organizationId: UUID, teamId: UUID?, householdId: UUID?): List<AnnouncementRecipientCandidate> {
        val teamClause = if (teamId == null) "" else """
            and exists (
    ...
    fun listTeamAthletes(organizationId: UUID, teamId: UUID): List<AnnouncementRecipientCandidate> = athletesQuery(organizationId, teamId)

    private fun athletesQuery(organizationId: UUID, teamId: UUID?): List<AnnouncementRecipientCandidate> {
        val teamJoin = if (teamId == null) "" else "join participant_team pt on pt.participant_id = p.id and pt.status = 'ACTIVE' and pt.team_id = :teamId"
        var statement = jdbcClient.sql(
            """
            select distinct au.id as user_id, au.display_name, au.email
    ...
    private fun candidate(rs: java.sql.ResultSet, type: AnnouncementRecipientType) = AnnouncementRecipientCandidate(
        recipientType = type,
        userId = rs.getObject("user_id", UUID::class.java),
        householdId = null,
        displayName = rs.getString("display_name"),
    ...
    private fun mapAnnouncement(rs: java.sql.ResultSet, _rowNum: Int): Announcement = Announcement(
        ...
        title = rs.getString("title"), body = rs.getString("body"),
        audience = AnnouncementAudience.valueOf(rs.getString("audience")),
        status = AnnouncementStatus.valueOf(rs.getString("status")),
        emailEnabled = rs.getBoolean("email_enabled"), smsEnabled = rs.getBoolean("sms_enabled"),
        ...
        publishedAt = rs.getTimestamp("published_at")?.toInstant(), archivedAt = rs.getTimestamp("archived_at")?.toInstant(),
        recipientCount = rs.getLong("recipient_count"), emailSentCount = rs.getLong("email_sent_count"),
        emailFailedCount = rs.getLong("email_failed_count"), smsSentCount = rs.getLong("sms_sent_count"),
    ...
    private fun mapRecipient(rs: java.sql.ResultSet, _rowNum: Int) = AnnouncementRecipient(
        id = rs.getObject("id", UUID::class.java), announcementId = rs.getObject("announcement_id", UUID::class.java),
        recipientKey = rs.getString("recipient_key"), recipientType = AnnouncementRecipientType.valueOf(rs.getString("recipient_type")),
        userId = rs.getObject("user_id", UUID::class.java), householdId = rs.getObject("household_id", UUID::class.java),
        displayName = rs.getString("display_name"), email = rs.getString("email"), phone = rs.getString("phone"),
        inAppVisible = rs.getBoolean("in_app_visible"), emailStatus = DeliveryStatus.valueOf(rs.getString("email_status")),
        smsStatus = DeliveryStatus.valueOf(rs.getString("sms_status")), readAt = rs.getTimestamp("read_at")?.toInstant(),
