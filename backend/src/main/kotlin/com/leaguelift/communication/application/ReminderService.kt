...
class ReminderService(
    ...
    fun send(
        organizationId: UUID, resourceType: ReminderResourceType, resourceId: UUID, idempotencyKey: String,
        emailEnabled: Boolean, smsEnabled: Boolean, currentUser: CurrentUser,
    ): Announcement = when (resourceType) {
    ...
    private fun campaign(
        organizationId: UUID, id: UUID, key: String, email: Boolean, sms: Boolean, currentUser: CurrentUser,
    ): Announcement {
        membershipService.requireManagerRole(organizationId, currentUser)
        val campaign = sources.findCampaign(organizationId, id)
            ?: throw NotFoundException("CAMPAIGN_NOT_FOUND", "The campaign could not be found.")
        if (campaign.status != "ACTIVE") throw ConflictException("CAMPAIGN_NOT_ACTIVE", "Only an active campaign can send a launch reminder.")
        ...
        return announcements.createSystemAndPublish(
            organizationId, scopeType, scopeId, AnnouncementKind.CAMPAIGN_LAUNCH, "CAMPAIGN", campaign.id, null,
            sourceKey("CAMPAIGN", campaign.id, key), "Campaign launched: ${campaign.name}",
            "${campaign.name} is now open. View and share the campaign at /campaigns/${campaign.slug}.",
            AnnouncementAudience.ALL, email, sms, currentUser,
        )
    }
    ...
    private fun event(
        ...
        val event = sources.findEvent(organizationId, id)
        ...
        if (event.status == "DRAFT" || event.status == "CANCELLED" || event.status == "COMPLETED" ||
            event.startAt == null || !event.startAt.isAfter(Instant.now(clock))) {
        ...
        when {
            ...
            event.tournamentId != null -> {
                ...
                authorizationService.requireTournamentCapability(organizationId, scopeId, currentUser, Capabilities.TOURNAMENT_COMMUNICATION_MANAGE)
            }
            else -> {
        ...
        return announcements.createSystemAndPublish(
            organizationId, scopeType, scopeId, AnnouncementKind.EVENT_REMINDER, "EVENT", event.id, null,
            sourceKey("EVENT", event.id, key), "Event reminder: ${event.title}",
            "${event.title} is scheduled for $whenText$whereText. Open LeagueLift for the latest details and RSVP status.",
            audience, email, sms, currentUser,
        )
    }
    ...
    private fun fee(
        ...
        membershipService.requireManagerRole(organizationId, currentUser)
        val fee = sources.findFee(organizationId, id)
        ...
        return announcements.createSystemAndPublish(
            organizationId, AnnouncementScopeType.ORGANIZATION, organizationId, AnnouncementKind.FEE_REMINDER,
            "FEE_ASSIGNMENT", fee.id, fee.householdId, sourceKey("FEE", fee.id, key),
            ...
            AnnouncementAudience.GUARDIANS, email, sms, currentUser,
        )
    }
    ...
    private fun document(
        ...
        membershipService.requireManagerRole(organizationId, currentUser)
        val document = sources.findDocument(organizationId, id)
            ?: throw NotFoundException("DOCUMENT_NOT_FOUND", "The household document could not be found.")
        return announcements.createSystemAndPublish(
            organizationId, AnnouncementScopeType.ORGANIZATION, organizationId, AnnouncementKind.DOCUMENT_REMINDER,
            "DOCUMENT", document.id, document.householdId, sourceKey("DOCUMENT", document.id, key),
            "Document reminder: ${document.title}",
            "Please review and acknowledge ${document.title} in your LeagueLift household documents.",
            AnnouncementAudience.GUARDIANS, email, sms, currentUser,
        )
    }

    private fun sourceKey(prefix: String, id: UUID, key: String): String {
