...
@Repository
class EmailVerificationTokenRepository(private val jdbcClient: JdbcClient) {
	...
	fun replaceActiveToken(userId: UUID, tokenHash: String, expiresAt: Instant): EmailVerificationTokenRecord {
		val now = Instant.now()
		jdbcClient.sql("delete from email_verification_token where user_id = :userId and consumed_at is null")
			.param("userId", userId)
			.update()
	...
	private fun mapRow(rs: java.sql.ResultSet, row: Int): EmailVerificationTokenRecord =
