package com.leaguelift.profilecorrection.persistence
...
@Repository
class ProfileCorrectionRepository(private val jdbcClient: JdbcClient) {
    ...
    fun findById(id: UUID, organizationId: UUID): ProfileCorrectionRequest? =
        jdbcClient.sql(
            """
    ...
    fun countForOrganization(organizationId: UUID, status: ProfileCorrectionStatus?): Long =
        jdbcClient.sql(
            """
            select count(*)
    ...
    fun listForHousehold(organizationId: UUID, householdId: UUID): List<ProfileCorrectionRequest> =
        jdbcClient.sql(
            """
            select $SELECT_COLUMNS
            from profile_correction_request r
    ...
    fun withdraw(id: UUID, organizationId: UUID, requestedBy: UUID): Int {
        val now = Instant.now()
        return jdbcClient.sql(
            """
            update profile_correction_request
    ...
    private fun mapRow(rs: java.sql.ResultSet, row: Int) = ProfileCorrectionRequest(
