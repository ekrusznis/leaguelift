package com.leaguelift.reconciliation.persistence
...
private const val RUN_COLUMNS = "id, organization_id, status, issue_count, high_count, medium_count, low_count, started_by_user_id, started_at, completed_at"
private const val ISSUE_COLUMNS = "id, reconciliation_run_id, organization_id, issue_type, severity, resource_type, resource_id, title, detail, action_path, created_at"

@Repository
class ReconciliationRepository(private val jdbcClient: JdbcClient) {
    fun insertRun(organizationId: UUID, startedByUserId: UUID): ReconciliationRun {
        ...
        jdbcClient.sql(
            ...
            .param("startedByUserId", startedByUserId).param("startedAt", Timestamp.from(now)).update()
        return ReconciliationRun(id, organizationId, ReconciliationRunStatus.RUNNING, 0, 0, 0, 0, startedByUserId, now, null)
    }

    fun completeRun(runId: UUID, organizationId: UUID, high: Int, medium: Int, low: Int): ReconciliationRun {
        ...
        jdbcClient.sql(
            ...
            .param("issueCount", high + medium + low).param("highCount", high)
            .param("mediumCount", medium).param("lowCount", low)
            .param("completedAt", Timestamp.from(completedAt)).param("runId", runId)
    ...
    fun findRun(organizationId: UUID, runId: UUID): ReconciliationRun? = jdbcClient.sql(
        "select $RUN_COLUMNS from reconciliation_run where organization_id = :organizationId and id = :runId",
    ).param("organizationId", organizationId).param("runId", runId).query(::mapRun).optional().orElse(null)

    fun latestRun(organizationId: UUID): ReconciliationRun? = jdbcClient.sql(
        "select $RUN_COLUMNS from reconciliation_run where organization_id = :organizationId order by started_at desc limit 1",
    ).param("organizationId", organizationId).query(::mapRun).optional().orElse(null)

    fun listRuns(organizationId: UUID, offset: Int, limit: Int): List<ReconciliationRun> = jdbcClient.sql(
    ...
    ).param("organizationId", organizationId).param("offset", offset).param("limit", limit).query(::mapRun).list()

    fun countRuns(organizationId: UUID): Long = jdbcClient.sql(
    ...
    fun insertIssue(runId: UUID, organizationId: UUID, issue: NewReconciliationIssue): ReconciliationIssue {
        val id = UUID.randomUUID()
        val now = Instant.now()
        jdbcClient.sql(
            """
            ...
            .param("id", id).param("runId", runId).param("organizationId", organizationId)
            .param("issueType", issue.issueType).param("severity", issue.severity.name)
            .param("resourceType", issue.resourceType).param("resourceId", issue.resourceId)
            .param("title", issue.title).param("detail", issue.detail).param("actionPath", issue.actionPath)
            .param("createdAt", Timestamp.from(now)).update()
        return ReconciliationIssue(
            id, runId, organizationId, issue.issueType, issue.severity, issue.resourceType,
            issue.resourceId, issue.title, issue.detail, issue.actionPath, now,
        )
    }

    fun listIssues(organizationId: UUID, runId: UUID): List<ReconciliationIssue> = jdbcClient.sql(
    ...
    ).param("organizationId", organizationId).param("runId", runId).query(::mapIssue).list()

    fun countLatestIssues(organizationId: UUID): Long = jdbcClient.sql(
        """
    ...
    fun detectIssues(organizationId: UUID): List<NewReconciliationIssue> = buildList {
    ...
    private fun pendingOfflineRecords(organizationId: UUID): List<NewReconciliationIssue> = jdbcClient.sql(
        """
        ...
        """.trimIndent(),
    ).param("organizationId", organizationId).query { rs, _ ->
        val id = rs.getObject("id", UUID::class.java)
        NewReconciliationIssue(
            "OFFLINE_PENDING_VERIFICATION", ReconciliationSeverity.HIGH, "OFFLINE_FINANCIAL_RECORD", id,
            "Offline transaction needs verification", rs.getString("display_label"),
            "/app/organizations/$organizationId/financial-operations",
        )
    }.list()

    private fun missingProviderReferences(organizationId: UUID): List<NewReconciliationIssue> = jdbcClient.sql(
        """
        ...
        val id = rs.getObject("id", UUID::class.java)
        NewReconciliationIssue(
            "MISSING_PROVIDER_REFERENCE", ReconciliationSeverity.HIGH, type, id,
    ...
    private fun missingLedgerEntries(organizationId: UUID): List<NewReconciliationIssue> = jdbcClient.sql(
        """
        ...
        NewReconciliationIssue(
            "MISSING_LEDGER_ENTRIES", ReconciliationSeverity.HIGH, type, id,
    ...
    private fun missingFulfillments(organizationId: UUID): List<NewReconciliationIssue> = jdbcClient.sql(
        """
        select o.id
        from "order" o
        ...
        val id = rs.getObject("id", UUID::class.java)
        NewReconciliationIssue(
            "MISSING_FULFILLMENT", ReconciliationSeverity.HIGH, "ORDER", id,
    ...
    private fun fulfillmentExceptions(organizationId: UUID): List<NewReconciliationIssue> = jdbcClient.sql(
        """
        ...
        NewReconciliationIssue(
            "FULFILLMENT_EXCEPTION", ReconciliationSeverity.HIGH, "FULFILLMENT",
            rs.getObject("id", UUID::class.java), "Fulfillment ${rs.getString("status")} requires attention",
            rs.getString("detail"), "/app/organizations/$organizationId/stores",
        )
    }.list()

    private fun overdueInstallments(organizationId: UUID): List<NewReconciliationIssue> = jdbcClient.sql(
        """
        ...
        NewReconciliationIssue(
            "OVERDUE_INSTALLMENT", ReconciliationSeverity.MEDIUM, "FEE_INSTALLMENT",
            rs.getObject("id", UUID::class.java), "Payment-plan installment is overdue",
    ...
    private fun paymentPlanBalanceMismatches(organizationId: UUID): List<NewReconciliationIssue> = jdbcClient.sql(
        """
        ...
        val householdId = rs.getObject("household_id", UUID::class.java)
        NewReconciliationIssue(
            "PAYMENT_PLAN_BALANCE_MISMATCH", ReconciliationSeverity.HIGH, "FEE_PAYMENT_PLAN",
            rs.getObject("id", UUID::class.java), "Payment plan does not match the fee balance",
            "The fee balance is ${rs.getLong("current_balance")} minor units while the plan shows ${rs.getLong("plan_remaining")} remaining.",
            "/app/organizations/$organizationId/households/$householdId/fees",
        )
    }.list()

    private fun mapRun(rs: ResultSet, rowNum: Int) = ReconciliationRun(
        rs.getObject("id", UUID::class.java), rs.getObject("organization_id", UUID::class.java),
        ReconciliationRunStatus.valueOf(rs.getString("status")), rs.getInt("issue_count"),
        rs.getInt("high_count"), rs.getInt("medium_count"), rs.getInt("low_count"),
        rs.getObject("started_by_user_id", UUID::class.java), rs.getTimestamp("started_at").toInstant(),
        rs.getTimestamp("completed_at")?.toInstant(),
    )

    private fun mapIssue(rs: ResultSet, rowNum: Int) = ReconciliationIssue(
        rs.getObject("id", UUID::class.java), rs.getObject("reconciliation_run_id", UUID::class.java),
        rs.getObject("organization_id", UUID::class.java), rs.getString("issue_type"),
        ReconciliationSeverity.valueOf(rs.getString("severity")), rs.getString("resource_type"),
        rs.getObject("resource_id", UUID::class.java), rs.getString("title"), rs.getString("detail"),
        rs.getString("action_path"), rs.getTimestamp("created_at").toInstant(),
    )
}
