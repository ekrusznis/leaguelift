...
@Repository
class SponsorshipPackageRepository(private val jdbcClient: JdbcClient) {

	fun findById(id: UUID, organizationId: UUID): SponsorshipPackage? =
		jdbcClient.sql("select $COLUMNS from sponsorship_package where id = :id and organization_id = :organizationId")
			.param("id", id)
	...
	fun findById(id: UUID): SponsorshipPackage? =
		jdbcClient.sql("select $COLUMNS from sponsorship_package where id = :id")
			.param("id", id)
			.query(::mapRow)
			.optional()
			.orElse(null)

	fun findAll(organizationId: UUID, offset: Int, limit: Int): List<SponsorshipPackage> =
		jdbcClient.sql(
			"""
	...
	fun countAll(organizationId: UUID): Long =
		jdbcClient.sql("select count(*) from sponsorship_package where organization_id = :organizationId")
			.param("organizationId", organizationId)
	...
	fun insert(
		...
		return SponsorshipPackage(
			id, organizationId, name, description, priceMinor, currency, maxQuantity, exclusive,
			placementStartDate, placementEndDate, SponsorshipPackageStatus.DRAFT, now, now,
		)
	}
	...
	fun updateStatus(id: UUID, organizationId: UUID, status: SponsorshipPackageStatus): Int {
		val now = Instant.now()
		return jdbcClient.sql(
	...
	private fun mapRow(rs: java.sql.ResultSet, rowNum: Int): SponsorshipPackage =
		SponsorshipPackage(
