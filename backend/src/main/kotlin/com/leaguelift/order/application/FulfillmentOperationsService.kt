...
import com.leaguelift.order.persistence.FulfillmentReprintRepository
import com.leaguelift.order.persistence.FulfillmentRepository
import com.leaguelift.order.persistence.OrderRepository
import com.leaguelift.store.application.ManualVendorService
...
class FulfillmentOperationsService(
	...
	fun get(organizationId: UUID, orderId: UUID, currentUser: CurrentUser): Fulfillment? {
		membershipService.requireManagerRole(organizationId, currentUser)
		requireOrder(organizationId, orderId)
		return fulfillmentRepository.findByOrder(orderId)
	}

	fun listHistory(organizationId: UUID, orderId: UUID, currentUser: CurrentUser): List<FulfillmentHistory> {
		membershipService.requireManagerRole(organizationId, currentUser)
		requireOrder(organizationId, orderId)
	...
	fun listReprints(organizationId: UUID, orderId: UUID, currentUser: CurrentUser): List<FulfillmentReprint> {
		membershipService.requireManagerRole(organizationId, currentUser)
		requireOrder(organizationId, orderId)
	...
	fun update(
		...
		membershipService.requireManagerRole(organizationId, currentUser)
		requireOrder(organizationId, orderId)
		val existing = fulfillmentRepository.findByOrder(orderId)
		...
		val normalizedVendorId = when (existing.source) {
	...
	fun requestReprint(
		...
		membershipService.requireManagerRole(organizationId, currentUser)
		requireOrder(organizationId, orderId)
		val fulfillment = fulfillmentRepository.findByOrder(orderId)
			?: throw NotFoundException("FULFILLMENT_NOT_FOUND", "The fulfillment record could not be found.")
		if (fulfillment.status == FulfillmentStatus.CANCELED) throw ValidationException("A canceled fulfillment cannot receive a reprint request.")
		...
		val reprint = reprintRepository.insert(
			organizationId, fulfillment.id, orderId, reason.trim(), vendorOrderReference.normalized(),
			internalNotes.normalized(), currentUser.userId,
		)
	...
	fun updateReprint(
		...
		membershipService.requireManagerRole(organizationId, currentUser)
		requireOrder(organizationId, orderId)
		val existing = reprintRepository.findById(reprintId, organizationId)?.takeIf { it.orderId == orderId }
		...
		reprintRepository.update(
			reprintId, organizationId, status, vendorOrderReference.normalized(), carrier.normalized(),
			trackingNumber.normalized(), trackingUrl.normalized(), internalNotes.normalized(),
		)
	...
	private fun requireOrder(organizationId: UUID, orderId: UUID) =
		orderRepository.findById(orderId, organizationId)
			?: throw NotFoundException("ORDER_NOT_FOUND", "The order could not be found.")

	private fun validateTransition(from: FulfillmentStatus, to: FulfillmentStatus) {
		if (from == to) return
		val allowed = when (from) {
			FulfillmentStatus.NOT_SUBMITTED -> setOf(FulfillmentStatus.READY, FulfillmentStatus.NEEDS_ATTENTION, FulfillmentStatus.CANCELED)
			FulfillmentStatus.DRAFT_CREATED -> setOf(FulfillmentStatus.READY, FulfillmentStatus.IN_PRODUCTION, FulfillmentStatus.SHIPPED, FulfillmentStatus.NEEDS_ATTENTION, FulfillmentStatus.CANCELED)
			FulfillmentStatus.FAILED -> setOf(FulfillmentStatus.READY, FulfillmentStatus.NEEDS_ATTENTION, FulfillmentStatus.CANCELED)
			FulfillmentStatus.READY -> setOf(FulfillmentStatus.IN_PRODUCTION, FulfillmentStatus.SHIPPED, FulfillmentStatus.NEEDS_ATTENTION, FulfillmentStatus.CANCELED)
			FulfillmentStatus.IN_PRODUCTION -> setOf(FulfillmentStatus.SHIPPED, FulfillmentStatus.NEEDS_ATTENTION, FulfillmentStatus.CANCELED)
			FulfillmentStatus.SHIPPED -> setOf(FulfillmentStatus.DELIVERED, FulfillmentStatus.NEEDS_ATTENTION)
			FulfillmentStatus.NEEDS_ATTENTION -> setOf(FulfillmentStatus.READY, FulfillmentStatus.IN_PRODUCTION, FulfillmentStatus.SHIPPED, FulfillmentStatus.CANCELED)
	...
	private fun validateReprintTransition(from: FulfillmentReprintStatus, to: FulfillmentReprintStatus) {
		if (from == to) return
		val allowed = when (from) {
			FulfillmentReprintStatus.REQUESTED -> setOf(FulfillmentReprintStatus.IN_PRODUCTION, FulfillmentReprintStatus.SHIPPED, FulfillmentReprintStatus.CANCELED)
