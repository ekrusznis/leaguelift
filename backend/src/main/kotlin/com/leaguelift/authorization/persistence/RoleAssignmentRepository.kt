...
@Repository
class RoleAssignmentRepository(private val jdbcClient: JdbcClient) {

	fun findActiveForUser(userId: UUID): List<RoleAssignment> =
		jdbcClient.sql("select $COLUMNS from role_assignment where user_id = :userId and status = 'ACTIVE'")
			.param("userId", userId)
			.query(::mapRow)
			.list()

	fun findActiveForUserAndContext(userId: UUID, contextType: RoleAssignmentContextType): List<RoleAssignment> =
		jdbcClient.sql(
	...
	fun findActiveForResource(userId: UUID, contextType: RoleAssignmentContextType, resourceId: UUID): RoleAssignment? =
		jdbcClient.sql(
			"""
	...
	fun listActiveForResource(contextType: RoleAssignmentContextType, resourceId: UUID): List<RoleAssignment> =
		jdbcClient.sql(
	...
	fun revoke(id: UUID): Int {
		val now = Instant.now()
		return jdbcClient.sql("update role_assignment set status = 'REVOKED', updated_at = :now where id = :id")
			.param("now", Timestamp.from(now))
			.param("id", id)
			.update()
	}

	private fun mapRow(rs: java.sql.ResultSet, rowNum: Int): RoleAssignment =
		RoleAssignment(
