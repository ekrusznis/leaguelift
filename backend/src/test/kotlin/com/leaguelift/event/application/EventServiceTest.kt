...
class EventServiceTest {
	...
	fun `listForHousehold resolves events across every linked participant's teams`() {
		val householdId = UUID.randomUUID()
		val participantA = com.leaguelift.participant.domain.Participant(
		...
		val teamAssignment = com.leaguelift.participant.domain.ParticipantTeamAssignment(UUID.randomUUID(), participantA.id, teamId, orgId, "ACTIVE", null, Instant.now(), Instant.now())
		every { householdRepository.findById(householdId, orgId) } returns mockk()
