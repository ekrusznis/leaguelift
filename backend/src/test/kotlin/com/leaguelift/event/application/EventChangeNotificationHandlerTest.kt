...
class EventChangeNotificationHandlerTest {
	...
	private fun eventWithPayload(payloadJson: String): OutboxEvent {
		val now = Instant.now()
		return OutboxEvent(
			id = UUID.randomUUID(), aggregateType = "event", aggregateId = UUID.randomUUID(), organizationId = UUID.randomUUID(),
			eventType = "event.time_changed", schemaVersion = 1, payload = payloadJson, status = OutboxEventStatus.PROCESSING,
	...
	fun `emails and texts every recipient with an available channel`() {
		val payload = objectMapper.writeValueAsString(
			EventChangeNotificationPayload(
				UUID.randomUUID(), "Varsity Soccer Practice", "The time for \"Varsity Soccer Practice\" has changed.",
