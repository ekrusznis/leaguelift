...
class EventRsvpChangeNotificationHandlerTest {
	...
	private fun eventWithPayload(payloadJson: String): OutboxEvent {
		val now = Instant.now()
		return OutboxEvent(
			id = UUID.randomUUID(), aggregateType = "event_rsvp", aggregateId = UUID.randomUUID(), organizationId = UUID.randomUUID(),
			eventType = "event.rsvp_changed", schemaVersion = 1, payload = payloadJson, status = OutboxEventStatus.PROCESSING,
	...
	fun `emails every team staff recipient`() {
		val payload = objectMapper.writeValueAsString(
			RsvpChangeNotificationPayload(
				UUID.randomUUID(), "Varsity Soccer Practice", "Jamie Lee", "NO_RESPONSE", "ATTENDING",
