...
import com.leaguelift.config.FeePaymentReminderProperties
import com.leaguelift.fee.persistence.FeePaymentReminderCandidate
...
class FeePaymentReminderScannerTest {
	...
	fun `enqueues an outbox event and marks a due candidate reminded`() {
		...
		val candidate = FeePaymentReminderCandidate(
			feeAssignmentId = UUID.randomUUID(), organizationId = UUID.randomUUID(), householdId = UUID.randomUUID(),
			householdContactEmail = "family@example.test", householdContactPhone = null, householdSmsOptIn = false,
			participantName = "Jamie Lee", description = "Fall registration",
			currency = "USD", dueDate = LocalDate.now().plusDays(3), balanceMinor = 10_000L,
		)
