...
class StripeWebhookControllerTest {
	...
	fun `a valid checkout_session_completed event confirms the contribution and records a PROCESSED webhook_event`() {
		...
		every {
			webhookEventRepository.insert(
				...
				WebhookProcessingStatus.PROCESSED, "contribution", contribution.id, null,
			)
	...
	fun `an unrecognized event type is stored as IGNORED without touching the contribution service`() {
		val eventId = "evt_${UUID.randomUUID()}"
		val payload = """{"id":"$eventId","object":"event","api_version":"2025-03-31.basil","type":"customer.created","data":{"object":{"id":"cus_test_1","object":"customer"}}}"""
		val signature = sign(payload, webhookSecret)
		every { webhookEventRepository.findExisting("stripe", eventId) } returns null
		every {
			webhookEventRepository.insert("stripe", eventId, "customer.created", payload, any(), true, WebhookProcessingStatus.IGNORED, null, null, null)
		} returns sampleWebhookEvent()
	...
	private fun sampleWebhookEvent() = WebhookEvent(
		id = UUID.randomUUID(), provider = "stripe", externalEventId = "evt_x", eventType = "checkout.session.completed",
		payload = "{}", payloadHash = "hash", signatureVerified = true, processingStatus = WebhookProcessingStatus.PROCESSED,
		attemptCount = 1, relatedEntityType = null, relatedEntityId = null, receivedAt = Instant.now(), processedAt = Instant.now(), lastError = null,
	)
}
