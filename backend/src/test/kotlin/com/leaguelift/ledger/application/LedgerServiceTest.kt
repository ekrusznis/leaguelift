...
class LedgerServiceTest {
	...
	fun `recordConfirmedOrder writes gross sale, production cost, platform fee, and a net earning credit when profitable`() {
		...
		assertEquals(LedgerEntryType.LEAGUELIFT_PLATFORM_FEE to LedgerDirection.DEBIT, captured[2].entryType to captured[2].direction)
	...
	fun `getPayoutSummary nets eligible credits against pending debits`() {
		every { ledgerEntryRepository.findEligibleOrganizationEarningCredits(orgId, any()) } returns listOf(
			earningEntry(LedgerDirection.CREDIT, 5_000L),
			earningEntry(LedgerDirection.CREDIT, 2_000L),
		)
		every { ledgerEntryRepository.findHeldOrganizationEarningCredits(orgId, any()) } returns listOf(
			earningEntry(LedgerDirection.CREDIT, 1_000L),
		)
		every { ledgerEntryRepository.findPendingOrganizationEarningDebits(orgId) } returns listOf(
	...
	fun `getPayoutSummary reports a negative netAvailableMinor honestly when pending debits exceed eligible credits`() {
		every { ledgerEntryRepository.findEligibleOrganizationEarningCredits(orgId, any()) } returns listOf(earningEntry(LedgerDirection.CREDIT, 1_000L))
		every { ledgerEntryRepository.findHeldOrganizationEarningCredits(orgId, any()) } returns emptyList()
		every { ledgerEntryRepository.findPendingOrganizationEarningDebits(orgId) } returns listOf(earningEntry(LedgerDirection.DEBIT, 4_000L))
	...
	fun `recordTransfer writes a TRANSFER debit and marks the included entries`() {
		...
		every {
			ledgerEntryRepository.insert(
				organizationId = orgId, accountCode = LedgerEntryType.TRANSFER.name, entryType = LedgerEntryType.TRANSFER,
				direction = LedgerDirection.DEBIT, amountMinor = 4_000L, currency = "usd", sourceType = LedgerSourceType.TRANSFER,
				sourceId = any(), externalReference = "tr_test_123", description = any(),
			)
	...
	private fun stubInsert(captured: MutableList<InsertCall>) {
		every {
			...
			captured += call
			LedgerEntry(
				id = UUID.randomUUID(), organizationId = call.organizationId, accountCode = call.accountCode,
				entryType = call.entryType, direction = call.direction, amountMinor = call.amountMinor, currency = call.currency,
				sourceType = call.sourceType, sourceId = call.sourceId, externalReference = call.externalReference,
				description = call.description, includedInTransferEntryId = null, effectiveAt = Instant.now(), createdAt = Instant.now(),
			)
		}
	}

	private fun earningEntry(direction: LedgerDirection, amountMinor: Long) = LedgerEntry(
		id = UUID.randomUUID(), organizationId = orgId, accountCode = LedgerEntryType.ORGANIZATION_EARNING.name,
		entryType = LedgerEntryType.ORGANIZATION_EARNING, direction = direction, amountMinor = amountMinor, currency = "usd",
		sourceType = LedgerSourceType.CONTRIBUTION, sourceId = UUID.randomUUID(), externalReference = null, description = null,
		includedInTransferEntryId = null, effectiveAt = Instant.now(), createdAt = Instant.now(),
	)

	private fun confirmedContribution(amountMinor: Long) = Contribution(
		id = UUID.randomUUID(), organizationId = orgId, campaignId = UUID.randomUUID(), amountMinor = amountMinor,
	...
	private fun confirmedOrder() = Order(
		id = UUID.randomUUID(), organizationId = orgId, storeId = UUID.randomUUID(), status = OrderStatus.CONFIRMED,
		currency = "usd", supporterName = "Jane Doe", supporterEmail = null, shippingAddress = null,
		stripeCheckoutSessionId = "cs_test_2", stripePaymentIntentId = "pi_test_2", confirmedAt = Instant.now(),
		refundedAt = null, createdAt = Instant.now(),
	)

	private fun orderItem(orderId: UUID, quantity: Int, unitPriceMinor: Long, unitCostMinor: Long) = OrderItem(
		id = UUID.randomUUID(), orderId = orderId, productVariantId = UUID.randomUUID(), quantity = quantity,
		unitPriceMinor = unitPriceMinor, unitCostMinor = unitCostMinor,
	)
}
