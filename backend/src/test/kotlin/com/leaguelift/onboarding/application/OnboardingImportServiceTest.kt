...
import com.leaguelift.onboarding.persistence.OnboardingImportIdentity
...
class OnboardingImportServiceTest {
	...
	private val service = OnboardingImportService(
		identityRepository,
		teamRepository,
		householdRepository,
	...
	fun `preview refuses to choose between ambiguous natural matches`() {
		...
		every { identityRepository.find(organizationId, any(), any()) } returns null
		val now = Instant.now()
		every { teamRepository.findNameMatches(organizationId, "U14 Blue") } returns listOf(
		...
		assertTrue(preview.rows.single().errors.single().contains("More than one existing team"))
	}
	...
	fun `preview propagates an invalid referenced team to the participant row`() {
		...
		assertEquals(2, preview.errorRows)
		assertTrue(preview.rows.last().errors.any { it.contains("team_external_id") && it.contains("has errors") })
	}
	...
	fun `preview rejects an existing guardian email that belongs to another household`() {
		...
		every { householdRepository.findContactEmailMatches(organizationId, any()) } returns emptyList()
		val now = Instant.now()
		every { householdRepository.findActiveAdultEmailMatches(organizationId, "jamie@example.com") } returns listOf(
			HouseholdAdult(
				UUID.randomUUID(), UUID.randomUUID(), organizationId, "Jamie", "Smith",
				"jamie@example.com", null, "Parent", true, AdultStatus.ACTIVE, now, now,
			),
		)
		val csv = """
		...
		assertEquals(1, preview.errorRows)
		assertTrue(preview.rows.last().errors.single().contains("already belongs to another household"))
	}
	...
	fun `execute aborts when a matched record was concurrently bound to another external id`() {
		...
		} returns OnboardingImportIdentity(
			UUID.randomUUID(), organizationId, OnboardingRecordType.TEAM, "other-source-id", team.id, user.userId, now,
		)
	...
	fun `execute creates a team and persists its stable external identity`() {
		...
		every {
			...
			UUID.randomUUID(), organizationId, OnboardingRecordType.TEAM, "team-blue", team.id, user.userId, now,
		)
