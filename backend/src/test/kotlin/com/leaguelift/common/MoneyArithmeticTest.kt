package com.leaguelift.common

import com.leaguelift.fee.domain.FeeAdjustment
import com.leaguelift.fee.domain.FeeBalance
...
class MoneyArithmeticTest {
	...
	private val moneyBearingClasses: List<KClass<*>> = listOf(
		LedgerEntry::class, FeeBalance::class, FeeAdjustment::class, FeePayment::class,
		Order::class, OrderItem::class, Contribution::class, Sponsorship::class,
		Campaign::class, ProductVariant::class,
	)

	@Test
	fun `no money domain class stores an amount as Double or Float`() {
		val offenders = moneyBearingClasses.flatMap { klass ->
	...
	fun `LedgerEntryRepository exposes no method that mutates an existing entry's core fields`() {
		...
		val mutatingMethodNames = LedgerEntryRepository::class.members
