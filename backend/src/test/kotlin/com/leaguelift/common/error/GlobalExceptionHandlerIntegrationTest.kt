...
class GlobalExceptionHandlerIntegrationTest : AbstractIntegrationTest() {
	...
	private data class AuthedOrg(val bearerToken: String, val organizationId: UUID)

	private fun authedOrg(): AuthedOrg {
		val appUser = passwordAuthenticationService.register(
			"exception-handler-${System.nanoTime()}@example.com", "password1234", "Test User",
		)
		...
		val organization = organizationService.create(
			"Exception Handler Test Org", "exception-handler-org-${System.nanoTime()}", OrganizationType.RECREATIONAL_LEAGUE, currentUser,
		)
		return AuthedOrg(token.accessToken, organization.id)
	}

	private fun postFeeTemplate(organizationId: UUID, bearerToken: String, jsonBody: String): HttpResponse<String> {
	...
	fun `a request body Jackson cannot bind returns 400 with the standard error envelope, not 500`() {
		...
		assert(!body.contains("com.fasterxml") && !body.contains("com.leaguelift")) {
	...
	fun `an omitted optional field with a Kotlin default is accepted, not rejected as malformed`() {
		...
		assertEquals(201, response.statusCode(), "omitting a field with a Kotlin default should use that default, not fail: ${response.body()}")
