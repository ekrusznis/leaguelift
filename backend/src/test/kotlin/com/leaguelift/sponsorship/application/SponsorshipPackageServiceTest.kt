...
class SponsorshipPackageServiceTest {
	...
	fun `create requires manager role and records audit`() {
		...
		verify(exactly = 1) { auditService.record(currentUser.userId, orgId, "sponsorship_package.created", "sponsorship_package", created.id, any()) }
	}
	...
	fun `create rejects a placement end date before the start date`() {
		...
		assertFailsWith<ValidationException> {
			service.create(
				orgId, "Gold Sponsor", null, 50_000L, "USD", null, false,
	...
	fun `listPublic resolves the organization by slug and returns only published packages`() {
		...
		every { sponsorshipPackageRepository.findPublished(org.id) } returns listOf(samplePackage(status = SponsorshipPackageStatus.PUBLISHED))
	...
	fun `publish transitions a draft package to published and records audit`() {
		...
		verify(exactly = 1) { auditService.record(currentUser.userId, orgId, "sponsorship_package.published", "sponsorship_package", draft.id, any()) }
	}
