...
/**
 * Security review (2026-08): this endpoint backs the org-manager-only "Teams" page and
 * previously only required active membership, letting any org member (e.g. a Viewer or a
 * team-scoped TEAM_ADMINISTRATOR) pull every team in the org. See
 * [com.rally26.search.application.SearchService]'s doc comment for the full finding.
 */
class TeamSearchServiceTest {
    private val repository = mockk<TeamSearchRepository>()
    private val membershipService = mockk<MembershipService>()
    private val service = TeamSearchService(repository, membershipService)

    private val orgId = UUID.randomUUID()
    private val currentUser = CurrentUser(UUID.randomUUID(), "viewer@example.com", "Viewer")
    private val criteria = TeamSearchCriteria()
