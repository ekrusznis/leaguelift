...
class FeePaymentReminderHandlerTest {
	...
	private fun eventWithPayload(payloadJson: String): OutboxEvent {
		val now = Instant.now()
		return OutboxEvent(
			id = UUID.randomUUID(), aggregateType = "fee_assignment", aggregateId = UUID.randomUUID(), organizationId = UUID.randomUUID(),
			eventType = "fee.payment_reminder_due", schemaVersion = 1, payload = payloadJson, status = OutboxEventStatus.PROCESSING,
	...
	fun `sends only an email when only a contact email is available`() {
		val payload = """{"feeAssignmentId":"${UUID.randomUUID()}","householdContactEmail":"family@example.test","householdContactPhone":null,"participantName":"Jamie Lee","description":"Fall registration","currency":"USD","dueDate":"2026-08-10","balanceMinor":10000}"""
	...
	fun `sends only an SMS when only a contact phone is available`() {
		val payload = """{"feeAssignmentId":"${UUID.randomUUID()}","householdContactEmail":null,"householdContactPhone":"+15550100","participantName":"Jamie Lee","description":"Fall registration","currency":"USD","dueDate":"2026-08-10","balanceMinor":10000}"""
	...
	fun `sends both channels when both are available`() {
		val payload = """{"feeAssignmentId":"${UUID.randomUUID()}","householdContactEmail":"family@example.test","householdContactPhone":"+15550100","participantName":null,"description":"Fall registration","currency":"USD","dueDate":"2026-08-10","balanceMinor":10000}"""
	...
	fun `skips both channels when neither is available`() {
		val payload = """{"feeAssignmentId":"${UUID.randomUUID()}","householdContactEmail":null,"householdContactPhone":null,"participantName":null,"description":"Fall registration","currency":"USD","dueDate":"2026-08-10","balanceMinor":10000}"""
