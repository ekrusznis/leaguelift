...
class DocumentServiceTest {
    ...
    private fun asset(
        ...
        slot: MediaUsageSlot = MediaUsageSlot.DOCUMENT,
    ) = MediaAsset(
        id,
        orgId,
        currentUser.userId,
    ...
    private fun household() =
        Household(householdId, orgId, "Smith Family", null, null, null, false, false, HouseholdStatus.ACTIVE, Instant.now(), Instant.now())
    ...
    fun `assignToAllHouseholds creates one assignment per household sharing the same asset`() {
        ...
        } returns assignment(entityId = householdA.id, assetId = doc.id)
        every {
            mediaAssignmentRepository.insert(
                orgId,
                doc.id,
        ...
        } returns assignment(entityId = householdB.id, assetId = doc.id)
    ...
    fun `acknowledge records a new acknowledgment for a real guardian`() {
        ...
        val recorded =
            com.rally26.document.domain.DocumentAcknowledgment(
                UUID.randomUUID(),
                orgId,
                a.id,
                householdId,
                relationship.householdAdultId,
                currentUser.userId,
                Instant.now(),
                Instant.now(),
            )
