...
class DocumentServiceTest {
	...
	fun `acknowledge records a new acknowledgment for a real guardian`() {
		...
		val recorded = com.rally26.document.domain.DocumentAcknowledgment(UUID.randomUUID(), orgId, a.id, householdId, relationship.householdAdultId, currentUser.userId, Instant.now(), Instant.now())
		every {
	...
	fun `acknowledge is idempotent — returns the existing record instead of inserting again`() {
		...
		every { guardianRelationshipRepository.findActiveForHousehold(currentUser.userId, householdId) } returns relationship
		every { mediaAssignmentRepository.findById(a.id, orgId) } returns a
		val existing = com.rally26.document.domain.DocumentAcknowledgment(UUID.randomUUID(), orgId, a.id, householdId, relationship.householdAdultId, currentUser.userId, Instant.now(), Instant.now())
