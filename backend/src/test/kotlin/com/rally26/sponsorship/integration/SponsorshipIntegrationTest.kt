...
class SponsorshipIntegrationTest : AbstractIntegrationTest() {
    ...
    fun `a confirmed sponsorship shows CONFIRMED with the package's confirmed count updated, and a replayed webhook doesn't double-count it`() {
        ...
        val organization =
            organizationService.create(
                "Riverside Soccer",
                "riverside-soccer-sponsorship-${System.nanoTime()}",
    ...
    fun `rejecting a confirmed sponsorship atomically refunds it via Stripe and the ledger, and it never reaches the public directory`() {
        ...
        val organization =
            organizationService.create(
                "Eastside Baseball",
                "eastside-baseball-sponsorship-${System.nanoTime()}",
                OrganizationType.RECREATIONAL_LEAGUE,
                owner,
            )
        activateClubPlan(organization.id)
    ...
    fun `a general refund of an approved sponsorship removes it from the public directory's future listings but preserves its own record`() {
        ...
        val organization =
            organizationService.create(
                "Northshore Swim",
                "northshore-swim-sponsorship-${System.nanoTime()}",
                OrganizationType.RECREATIONAL_LEAGUE,
                owner,
            )
        activateClubPlan(organization.id)
        ...
        assertEquals(
            0,
            sponsorshipService.listPublicDirectory(organization.slug).size,
            "a refunded sponsorship is no longer CONFIRMED, so it drops off the directory",
        )
    }
    ...
    fun `a sold-out package rejects a further checkout attempt`() {
        ...
        val organization =
            organizationService.create(
                "Lakeside Hockey",
                "lakeside-hockey-sponsorship-${System.nanoTime()}",
                OrganizationType.RECREATIONAL_LEAGUE,
                owner,
            )
        activateClubPlan(organization.id)
    ...
    fun `a second organization cannot see or manage the first organization's sponsorship packages`() {
        ...
        val organizationA =
            organizationService.create(
                "Org A",
                "org-a-sponsorship-${System.nanoTime()}",
                OrganizationType.RECREATIONAL_LEAGUE,
                ownerA,
            )
        ...
        val organizationB =
            organizationService.create(
                "Org B",
                "org-b-sponsorship-${System.nanoTime()}",
                OrganizationType.RECREATIONAL_LEAGUE,
                ownerB,
            )
