...
class SponsorshipRenewalReminderHandlerTest {
    ...
    fun `sends an email when the candidate has a contact email`() {
        val payload =
            """{"sponsorshipId":"${UUID.randomUUID()}","sponsorName":"Acme Co",""" +
                """"sponsorContactEmail":"sponsor@acme.test","packageName":"Gold Sponsor","placementEndDate":"2026-08-10"}"""
    ...
    fun `skips sending when the candidate has no contact email`() {
        val payload =
            """{"sponsorshipId":"${UUID.randomUUID()}","sponsorName":"No Email Co",""" +
                """"sponsorContactEmail":null,"packageName":"Silver Sponsor","placementEndDate":"2026-08-01"}"""
