...
class ReportingServiceTest {
    ...
    fun `getRevenueReport totalMinor sums across every source type`() {
        every { membershipService.requireReportingRole(orgId, manager) } returns managerMembership()
        every { reportingRepository.revenueBySourceType(orgId, any(), any()) } returns
            listOf(
                SourceTypeRevenueRow("CONTRIBUTION", 3_000L),
                SourceTypeRevenueRow("ORDER", 2_000L),
            )
        every { reportingRepository.revenueByTeam(orgId, any(), any()) } returns
            listOf(TeamRevenueRow(UUID.randomUUID(), "Varsity Soccer", 1_500L))
    ...
    fun `getFeeCollectionsReport combines in-range payments with the point-in-time outstanding balance`() {
        ...
        every { reportingRepository.feeCollections(orgId, any(), any()) } returns
            listOf(
    ...
    fun `getPlatformReport requires the platform audit capability`() {
        every { authorizationService.requirePlatformCapability(manager, Capabilities.PLATFORM_AUDIT_VIEW) } throws
