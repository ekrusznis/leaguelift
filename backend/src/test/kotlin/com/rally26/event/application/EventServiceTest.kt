...
class EventServiceTest {
	...
	private val service = EventService(
		eventRepository, teamRepository, tournamentRepository, householdRepository, participantRepository,
		membershipService, authorizationService, auditService, calendarProvider, mapsProvider, outboxWriter, ObjectMapper(),
	)
	...
	private fun team(id: UUID = teamId) = Team(id, orgId, "Varsity Soccer", "Soccer", "Fall 2026", TeamStatus.ACTIVE, null, Instant.now(), Instant.now())
	...
	private fun sampleEvent(id: UUID = UUID.randomUUID(), teamId: UUID? = this.teamId, tournamentId: UUID? = null, status: EventStatus = EventStatus.DRAFT) = Event(
		id = id, organizationId = orgId, teamId = teamId, tournamentId = tournamentId, opponentTeamId = null, opponentName = null,
		eventType = EventType.PRACTICE, title = null, description = null, status = status, startAt = null, endAt = null,
		arrivalAt = null, meetingAt = null, timezone = "America/New_York", venueName = null, address = null, latitude = null,
		longitude = null, area = null, meetingPoint = null, directionsNotes = null, visibility = EventVisibility.TEAM,
		sourceType = EventSourceType.MANUAL, provider = null, connectionId = null, externalEventId = null, externalSyncHash = null,
		sourceUpdatedAt = null, createdByUserId = currentUser.userId, updatedByUserId = currentUser.userId,
	...
	fun `create rejects an invalid timezone`() {
		assertFailsWith<ValidationException> {
			service.create(
				orgId, teamId, null, null, null, EventType.PRACTICE, null, null, null, null, null, null,
				"Not/AZone", null, null, null, null, null, null, null, EventVisibility.TEAM, currentUser,
			)
		}
	}
	...
	fun `create rejects an opponent team equal to the owning team`() {
		assertFailsWith<ValidationException> {
			service.create(
				orgId, teamId, null, teamId, null, EventType.COMPETITION, null, null, null, null, null, null,
				"America/New_York", null, null, null, null, null, null, null, EventVisibility.TEAM, currentUser,
			)
		}
	}
	...
	fun `create requires manager role for an org-wide event with no team or tournament`() {
		...
		assertFailsWith<ForbiddenException> {
			service.create(
				orgId, null, null, null, null, EventType.MEETING, "Annual Meeting", null, null, null, null, null,
				"America/New_York", null, null, null, null, null, null, null, EventVisibility.ORGANIZATION, currentUser,
			)
		}
	}
	...
	fun `create succeeds for a team-scoped event and records an audit event`() {
		...
		every {
			eventRepository.insert(
				orgId, teamId, null, null, null, EventType.PRACTICE, null, null, null, null, null, null,
				"America/New_York", null, null, null, null, null, null, null, EventVisibility.TEAM, currentUser.userId,
			)
		} returns created
	...
	fun `listForTeam requires the event-read capability at team scope`() {
		every { authorizationService.requireTeamCapability(orgId, teamId, currentUser, Capabilities.EVENT_READ) } throws ForbiddenException("DENIED", "no")
	...
	fun `listForHousehold resolves events across every linked participant's teams`() {
		val householdId = UUID.randomUUID()
		val participantA = com.rally26.participant.domain.Participant(
			UUID.randomUUID(), householdId, orgId, "Jamie", "Lee", null, null,
			com.rally26.participant.domain.ParticipantStatus.ACTIVE, Instant.now(), Instant.now(),
		)
		val teamAssignment = com.rally26.participant.domain.ParticipantTeamAssignment(UUID.randomUUID(), participantA.id, teamId, orgId, "ACTIVE", null, Instant.now(), Instant.now())
		every { householdRepository.findById(householdId, orgId) } returns mockk()
	...
	fun `listForParticipant allows the linked athlete via ATHLETE_SCHEDULE_VIEW`() {
		val participant = com.rally26.participant.domain.Participant(
			participantId2, UUID.randomUUID(), orgId, "Jamie", "Lee", null, null,
			com.rally26.participant.domain.ParticipantStatus.ACTIVE, Instant.now(), Instant.now(),
		)
		every { participantRepository.findById(participantId2, orgId) } returns participant
		every { authorizationService.hasParticipantCapability(currentUser, participantId2, Capabilities.ATHLETE_SCHEDULE_VIEW) } returns true
	...
	fun `listForParticipant denies an athlete's self-link from viewing a sibling's schedule`() {
		...
		val sibling = com.rally26.participant.domain.Participant(
	...
	fun `update does not notify when the event is still DRAFT`() {
		...
		service.update(
			orgId, before.id, null, null, null, after.startAt, null, null, null,
	...
	fun `update on a tournament child event with no team collapses every changed field into one tournament_event_updated notification`() {
		...
		val before = sampleEvent(teamId = null, tournamentId = tournamentId, status = EventStatus.TENTATIVE)
		...
		service.update(
			orgId, before.id, null, null, null, after.startAt, null, null, null,
			null, null, null, null, after.area, null, null, null, null, currentUser,
		)
