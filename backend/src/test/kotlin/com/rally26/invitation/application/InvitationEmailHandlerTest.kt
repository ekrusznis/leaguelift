...
class InvitationEmailHandlerTest {
    ...
    private fun outboxEventFor(invitationId: UUID): OutboxEvent {
        ...
        return OutboxEvent(
            ...
            schemaVersion = 1,
            payload =
                """{"invitationId":"$invitationId","email":"new@example.com",""" +
                    """"role":"ADMINISTRATOR","acceptToken":"token-from-event"}""",
    ...
    private fun pendingInvitation(id: UUID) =
        Invitation(
            id = id,
            organizationId = UUID.randomUUID(),
    ...
    fun `does nothing when the invitation is no longer pending`() {
        val invitationId = UUID.randomUUID()
        every { invitationRepository.findById(invitationId) } returns
            pendingInvitation(invitationId).copy(status = InvitationStatus.REVOKED)
