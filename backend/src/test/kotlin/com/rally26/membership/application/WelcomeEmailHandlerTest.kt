...
class WelcomeEmailHandlerTest {
	...
	private fun handlerWith(resendTemplateProperties: ResendTemplateProperties) = WelcomeEmailHandler(
	...
	private fun eventFor(userId: UUID, organizationId: UUID, role: MembershipRole): OutboxEvent {
		val now = Instant.now()
		val membershipId = UUID.randomUUID()
		return OutboxEvent(
			id = UUID.randomUUID(), aggregateType = "organization_membership", aggregateId = membershipId, organizationId = organizationId,
			eventType = "membership.first_granted", schemaVersion = 1,
	...
	private fun activeMembership(organizationId: UUID, userId: UUID, role: MembershipRole) = OrganizationMembership(
		id = UUID.randomUUID(), organizationId = organizationId, userId = userId, role = role,
		status = MembershipStatus.ACTIVE, createdAt = Instant.now(), updatedAt = Instant.now(),
	)

	private fun activeUser(id: UUID) = AppUser(
		id = id, email = "new-owner@example.com", displayName = "New Owner", status = AppUserStatus.ACTIVE,
		passwordHash = "hash", createdAt = Instant.now(), updatedAt = Instant.now(),
	)

	private fun organization(id: UUID) = Organization(
		id = id, name = "Rally26 Youth League", slug = "rally26-youth-league",
		organizationType = OrganizationType.RECREATIONAL_LEAGUE, status = OrganizationStatus.ACTIVE,
		sports = listOf("soccer"), contactEmail = null, contactPhone = null,
	...
	fun `sends a welcome email for a newly active membership`() {
		...
		every { membershipRepository.findActiveMembership(organizationId, userId) } returns activeMembership(organizationId, userId, MembershipRole.OWNER)
	...
	fun `sends via the Resend template with a role-scoped feature list when a template id is configured`() {
		val userId = UUID.randomUUID()
		val organizationId = UUID.randomUUID()
		every { membershipRepository.findActiveMembership(organizationId, userId) } returns activeMembership(organizationId, userId, MembershipRole.TEAM_ADMINISTRATOR)
		every { appUserRepository.findById(userId) } returns activeUser(userId)
		every { organizationRepository.findById(organizationId) } returns organization(organizationId)
		val messageSlot = slot<EmailMessage>()
		every { emailProvider.send(capture(messageSlot)) } just runs

		handlerWith(ResendTemplateProperties(welcomeId = "template-welcome")).handle(eventFor(userId, organizationId, MembershipRole.TEAM_ADMINISTRATOR))
