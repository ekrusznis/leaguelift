...
class SeasonRolloverServiceTest {
	...
	private val source = Team(
		UUID.randomUUID(), organizationId, "U14 Blue", "Volleyball", "2026-2027",
		TeamStatus.ACTIVE, "coach@example.com", Instant.parse("2026-07-01T12:00:00Z"), Instant.parse("2026-07-31T12:00:00Z"),
	)
	private val destination = Team(
		UUID.randomUUID(), organizationId, "U15 Blue", "Volleyball", "2027-2028",
		TeamStatus.ACTIVE, "coach@example.com", Instant.parse("2026-08-01T12:00:00Z"), Instant.parse("2026-08-01T12:00:00Z"),
	)
	...
	fun `execute creates the team copies selected setup archives source and records the run`() {
		...
		val run = SeasonRolloverRun(
			UUID.randomUUID(), organizationId, source.id, destination.id, preview.confirmationHash,
			true, true, true, true, 1, 1, 1, user.userId, Instant.parse("2026-08-01T12:30:00Z"),
		)
		every {
			repository.insertRun(
				organizationId, source.id, destination.id, preview.confirmationHash,
				true, true, true, true, 1, 1, 1, user.userId,
			)
		} returns run
		...
		verify(exactly = 1) { auditService.record(user.userId, organizationId, "season_rollover.executed", "season_rollover_run", run.id, any()) }
	}
	...
	fun `completed confirmation is idempotent and does not copy again`() {
		...
		val run = SeasonRolloverRun(
			UUID.randomUUID(), organizationId, source.id, destination.id, hash,
			true, true, true, true, 12, 2, 2, user.userId, Instant.parse("2026-08-01T12:30:00Z"),
		)
	...
	private fun stubSnapshot() {
		...
		every { repository.listRoster(organizationId, source.id) } returns listOf(
			SeasonRolloverRosterItem(
				UUID.randomUUID(), "Avery Morgan", LocalDate.of(2026, 8, 1),
				Instant.parse("2026-07-30T12:00:00Z"), Instant.parse("2026-07-31T12:00:00Z"),
			),
		)
		every { repository.listStaff(organizationId, source.id) } returns listOf(
			SeasonRolloverStaffItem(
				UUID.randomUUID(), UUID.randomUUID(), "Taylor Coach", "coach@example.com", ResourceRole.TEAM_MANAGER,
				Instant.parse("2026-07-31T12:00:00Z"), Instant.parse("2026-07-30T12:00:00Z"),
			),
		)
		every { repository.listBranding(organizationId, source.id) } returns listOf(
			SeasonRolloverBrandingItem(
				UUID.randomUUID(), UUID.randomUUID(), "LOGO", "u14-blue-logo.png", PublicationStatus.PUBLISHED,
				Visibility.PUBLIC, "U14 Blue logo", Instant.parse("2026-07-31T12:00:00Z"), Instant.parse("2026-07-30T12:00:00Z"),
			),
		)
	}
}
