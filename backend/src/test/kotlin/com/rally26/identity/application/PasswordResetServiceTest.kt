...
class PasswordResetServiceTest {
    ...
    fun `request stores token and writes outbox event for active user`() {
        ...
        every { passwordResetTokenRepository.replaceActiveToken(user.id, any(), any()) } returns
    ...
    fun `complete rejects expired token`() {
        every { passwordResetTokenRepository.findByTokenHash(any()) } returns
            PasswordResetTokenRecord(
                id = UUID.randomUUID(),
    ...
    private fun activeUser(email: String) =
        AppUser(
            id = UUID.randomUUID(),
            email = email,
            displayName = "Owner",
