...
class AthleteDashboardServiceTest {
	...
	private fun selfLink() = RoleAssignment(
		UUID.randomUUID(), orgId, currentUser.userId, RoleAssignmentContextType.PARTICIPANT, participantId,
		ResourceRole.ATHLETE_SELF, RoleAssignmentStatus.ACTIVE, UUID.randomUUID(), Instant.now(), Instant.now(),
	)

	private fun participant() = Participant(participantId, householdId, orgId, "Maya", "Johnson", null, null, ParticipantStatus.ACTIVE, Instant.now(), Instant.now())
	...
	fun `getTeams returns the linked participant's real team assignments and coach identity`() {
		...
		val coach = AppUser(UUID.randomUUID(), "coach@example.com", "Jordan Ellis", AppUserStatus.ACTIVE, null, Instant.now(), Instant.now())
		val coachGrant = RoleAssignment(
			UUID.randomUUID(), orgId, coach.id, RoleAssignmentContextType.TEAM, team.id,
			ResourceRole.TEAM_MANAGER, RoleAssignmentStatus.ACTIVE, null, Instant.now(), Instant.now(),
		)
		every { authorizationService.findAthleteSelfLink(currentUser) } returns selfLink()
		every { participantRepository.findById(participantId, orgId) } returns participant()
		every { participantRepository.listTeamAssignments(participantId, orgId) } returns listOf(
	...
	fun `getGuardians returns real household adults for the linked participant`() {
		every { authorizationService.findAthleteSelfLink(currentUser) } returns selfLink()
		every { participantRepository.findById(participantId, orgId) } returns participant()
		every { householdRepository.listAdults(householdId, orgId) } returns listOf(
			HouseholdAdult(UUID.randomUUID(), householdId, orgId, "Sarah", "Johnson", "sarah@example.com", "555-0198", "Parent", true, AdultStatus.ACTIVE, Instant.now(), Instant.now()),
		)
	...
	fun `getWeekEvents returns the linked participant's real upcoming events`() {
		val team = Team(UUID.randomUUID(), orgId, "Varsity Soccer", "Soccer", "2024", TeamStatus.ACTIVE, null, Instant.now(), Instant.now())
		val event = com.rally26.event.domain.Event(
			id = UUID.randomUUID(), organizationId = orgId, teamId = team.id, tournamentId = null, opponentTeamId = null, opponentName = "Northside FC",
			eventType = com.rally26.event.domain.EventType.COMPETITION, title = null, description = null,
			status = com.rally26.event.domain.EventStatus.SCHEDULED, startAt = Instant.now().plusSeconds(3600), endAt = null,
			arrivalAt = null, meetingAt = null, timezone = "America/New_York", venueName = "Home Field", address = null, latitude = null,
			longitude = null, area = null, meetingPoint = null, directionsNotes = null, visibility = com.rally26.event.domain.EventVisibility.TEAM,
			sourceType = com.rally26.event.domain.EventSourceType.MANUAL, provider = null, connectionId = null, externalEventId = null,
