...
class PlatformAdminDashboardServiceTest {
    ...
    private val service =
        PlatformAdminDashboardService(
            ...
            organizationPayoutAccountRepository,
        )
    ...
    fun `getOrdersSummary requires the platform audit capability`() {
        every { authorizationService.requirePlatformCapability(currentUser, Capabilities.PLATFORM_AUDIT_VIEW) } throws
    ...
    fun `getPaymentsSummary sums gross sale and contribution credits as the gross processed total`() {
        ...
        every { ledgerEntryRepository.sumAllByTypeAndDirection(LedgerEntryType.RALLY26_PLATFORM_FEE, LedgerDirection.DEBIT) } returns 7_500L
