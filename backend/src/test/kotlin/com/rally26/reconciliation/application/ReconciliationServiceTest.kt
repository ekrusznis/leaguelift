...
class ReconciliationServiceTest {
    ...
    fun `run persists a durable issue snapshot and severity counts`() {
        val running =
            ReconciliationRun(UUID.randomUUID(), organizationId, ReconciliationRunStatus.RUNNING, 0, 0, 0, 0, user.userId, now, null)
        val detected =
            listOf(
        ...
        every { repository.insertIssue(running.id, organizationId, any()) } returnsMany
            detected.map { issue ->
                ReconciliationIssue(
                    UUID.randomUUID(),
                    running.id,
